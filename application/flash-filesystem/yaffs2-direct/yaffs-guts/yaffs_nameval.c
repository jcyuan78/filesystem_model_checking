...
static int nval_find(struct yaffs_dev *dev,
		     const YCHAR *xb, int xb_size, const YCHAR *name,
		     int *exist_size)
{
	int pos = 0;
	s32 size;
...
static int nval_used(struct yaffs_dev *dev, const YCHAR *xb, int xb_size)
{
	int pos = 0;
	s32 size;
	     ...
	     int bsize, int flags)
{
	...
	yaffs_strncpy((YCHAR *) (xb + pos), xb_size-pos, name, reclen);
	pos += (namelen + 1);
...
int nval_list(struct yaffs_dev *dev, const YCHAR *xb, int xb_size, YCHAR *buf, int bsize)
{
	int pos = 0;
	s32 size;
	int name_len;
