				    ...
				    const YCHAR *oh_name, int buff_size)
{
	...
	{
#endif
		yaffs_strcpy_(name, oh_name, buff_size - 1);
	}
}
				    ...
				    const YCHAR *name)
{
	...
	{
#endif
		yaffs_strcpy_(oh_name, name, YAFFS_MAX_NAME_LENGTH - 1);
	}
}
...
void yaffs_set_obj_name(struct yaffs_obj *obj, const YCHAR * name)
{
	...
	if (name && !name[0]) {
		...
		YAFFS_SHORT_NAME_LENGTH)  {
		yaffs_strcpy_(obj->short_name, name, 16);
	}
...
void yaffs_block_became_dirty(struct yaffs_dev *dev, int block_no)
{
	...
	yaffs_trace(YAFFS_TRACE_GC | YAFFS_TRACE_ERASE,
		...
		(bi->needs_retiring) ? L"needs retiring" : L"");
				    ...
				    int aggressive, int background)
{
	...
	if (selected) {
		...
		yaffs_trace(YAFFS_TRACE_GC,
			...
			dev->oldest_dirty_block, background ? L" bg" : L"");
	}
...
static int yaffs_apply_xattrib_mod(struct yaffs_obj *obj, YCHAR *buffer,
				   struct yaffs_xattr_mod *xmod)
{
	int retval = 0;
				  ...
				  void *value, int size)
{
	...
	buffer = (YCHAR *)yaffs_get_temp_buffer(dev);
	if (!buffer)
		return -ENOMEM;
...
int yaffs_list_xattrib(struct yaffs_obj *obj, YCHAR *buffer, int size)
{
		    ...
		    int is_shrink, int shadows, struct yaffs_xattr_mod *xmod)
{
	...
	yaffs_strcpy_(old_name, _Y("silly old name"), YAFFS_MAX_NAME_LENGTH);
	...
	case YAFFS_OBJECT_TYPE_SYMLINK:
		...
		if (!alias)
			alias = _Y("no alias");
		yaffs_strcpy_(oh->alias, alias, YAFFS_MAX_ALIAS_LENGTH);
	...
	if (xmod)
		yaffs_apply_xattrib_mod(in, (YCHAR *)buffer, xmod);
				...
				int buffer_size)
{
	...
	if (yaffs_strnlen(name, YAFFS_MAX_NAME_LENGTH) == 0) {
		...
		/* make up a name */
		yaffs_strcpy_(local_name, YAFFS_LOSTNFOUND_PREFIX,20);
		yaffs_strcat(local_name, 20, x);
		yaffs_strcpy_(name, local_name, buffer_size - 1);
	}
}
...
int yaffs_get_obj_name(struct yaffs_obj *obj, YCHAR *name, int buffer_size)
{
	...
	if (obj->obj_id == YAFFS_OBJECTID_LOSTNFOUND) {
		yaffs_strcpy_(name, YAFFS_LOSTNFOUND_NAME, buffer_size - 1);
	} else if (obj->short_name[0]) {
		yaffs_strcpy_(name, obj->short_name, buffer_size);
