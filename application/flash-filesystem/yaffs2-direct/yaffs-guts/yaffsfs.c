...
static int yaffsfs_alt_dir_path(const YCHAR *path, YCHAR **ret_path)
{
	...
	*ret_path = NULL;
	...
	if (path_length > 0 && yaffsfs_IsPathDivider(path[path_length - 1])) {
		...
		if (!alt_path)
			return -1;
		yaffs_strcpy(alt_path, path_length, path);
		for (i = path_length - 1;
...
static int yaffs_do_setxattr_reldir(struct yaffs_obj *reldir, const YCHAR *path,
				  const YCHAR *name, const void *data, int size,
...
int yaffs_setxattr_reldir(struct yaffs_obj *reldir, const YCHAR *path,
			const YCHAR *name, const void *data, int size, int flags)
{
...
int yaffs_setxattr(const YCHAR *path, const YCHAR *name,
		   const void *data, int size, int flags)
{
	return yaffs_setxattr_reldir(NULL, path, name, data, size, flags);
}

int yaffs_lsetxattr_reldir(struct yaffs_obj *reldir, const YCHAR *path, const YCHAR *name,
		    const void *data, int size, int flags)
{
	return yaffs_do_setxattr_reldir(reldir, path, name, data, size, flags, 0);
}

int yaffs_lsetxattr_reldev(struct yaffs_dev *dev, const YCHAR *path, const YCHAR *name,
		   const void *data, int size, int flags)
{
	return yaffs_lsetxattr_reldir(ROOT_DIR(dev), path, name, data, size, flags);
}

int yaffs_lsetxattr(const YCHAR *path, const YCHAR *name,
		   const void *data, int size, int flags)
{
	return yaffs_lsetxattr_reldir(NULL, path, name, data, size, flags);
}

int yaffs_fsetxattr(int fd, const YCHAR *name,
		    const void *data, int size, int flags)
{
	struct yaffs_obj *obj;
...
static int yaffs_do_getxattr_reldir(struct yaffs_obj *reldir, const YCHAR *path,
			const YCHAR *name, void *data, int size, int follow)
{
	struct yaffs_obj *obj;
	struct yaffs_obj *dir;
	int retVal = -1;
...
int yaffs_getxattr_reldir(struct yaffs_obj *reldir, const YCHAR *path,
			const YCHAR *name, void *data, int size)
{
	return yaffs_do_getxattr_reldir(reldir, path, name, data, size, 1);
}

int yaffs_getxattr_reldev(struct yaffs_dev *dev, const YCHAR *path, const YCHAR *name, void *data, int size)
{
	return yaffs_getxattr_reldir(ROOT_DIR(dev), path, name, data, size);
}

int yaffs_getxattr(const YCHAR *path, const YCHAR *name, void *data, int size)
{
...
int yaffs_lgetxattr_reldev(struct yaffs_dev *dev, const YCHAR *path, const YCHAR *name, void *data, int size)
{
	return yaffs_lgetxattr_reldir(ROOT_DIR(dev), path, name, data, size);
}

int yaffs_lgetxattr(const YCHAR *path, const YCHAR *name, void *data, int size)
{
	return yaffs_lgetxattr_reldir(NULL, path, name, data, size);
}

int yaffs_fgetxattr(int fd, const YCHAR *name, void *data, int size)
{
	struct yaffs_obj *obj;
...
static int yaffs_do_listxattr_reldir(struct yaffs_obj *reldir, const YCHAR *path,
				YCHAR *data, int size, int follow)
{
	struct yaffs_obj *obj = NULL;
	struct yaffs_obj *dir = NULL;
	int retVal = -1;
...
int yaffs_listxattr_reldev(struct yaffs_dev *dev, const YCHAR *path, YCHAR *data, int size)
{
	return yaffs_listxattr_reldir(ROOT_DIR(dev), path, data, size);
}

int yaffs_listxattr(const YCHAR *path, YCHAR *data, int size)
{
...
int yaffs_llistxattr_reldev(struct yaffs_dev *dev, const YCHAR *path, YCHAR *data, int size)
{
	return yaffs_llistxattr_reldir(ROOT_DIR(dev), path, data, size);
}

int yaffs_llistxattr(const YCHAR *path, YCHAR *data, int size)
{
	return yaffs_llistxattr_reldir(NULL, path, data, size);
}

int yaffs_flistxattr(int fd, YCHAR *data, int size)
{
	struct yaffs_obj *obj;
...
int yaffs_removexattr_reldev(struct yaffs_dev *dev, const YCHAR *path, const YCHAR *name)
{
	return yaffs_removexattr_reldir(ROOT_DIR(dev), path, name);
}

int yaffs_removexattr(const YCHAR *path, const YCHAR *name)
{
...
int yaffs_lremovexattr_reldev(struct yaffs_dev *dev, const YCHAR *path, const YCHAR *name)
{
	return yaffs_lremovexattr_reldir(ROOT_DIR(dev), path, name);
}

int yaffs_lremovexattr(const YCHAR *path, const YCHAR *name)
{
	return yaffs_lremovexattr_reldir(NULL, path, name);
}

int yaffs_fremovexattr(int fd, const YCHAR *name)
{
	struct yaffs_obj *obj;
					...
					const YCHAR *dirname)
{
	...
	if (!obj && notDir)
		...
		if (dsc) {
			...
			dsc->inUse = 1;
			dsc->dirObj = obj;
			yaffs_strncpy(dsc->name, NAME_MAX, dirname, NAME_MAX);
	...
	if (dsc && dsc->inUse) {
		...
		if (dsc->nextReturn) {
			...
			if (yaffs_strnlen(dsc->de.d_name, NAME_MAX + 1) == 0) {
				...
				yaffs_strcpy(dsc->de.d_name, NAME_MAX, _Y("zz"));
			}
			...
			YCHAR *buf, int bufsiz)
{
	...
	if (!dir && notDir)
		...
		YCHAR *alias = obj->variant.symlink_variant.alias;
		memset(buf, 0, bufsiz);
		yaffs_strncpy(buf, bufsiz, alias, bufsiz - 1);
		retVal = 0;
	}
	yaffsfs_Unlock();
	return retVal;
