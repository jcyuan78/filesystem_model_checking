...
#include "../include/buffer_head.h" /* for inode_has_buffers */
...
LOCAL_LOGGER_ENABLE(L"linux.inode", LOGGER_LEVEL_DEBUGINFO);
...
//	mapping->i_mmap = RB_ROOT_CACHED;
	mapping->i_mmap.rb_root.rb_node = NULL;
	mapping->i_mmap.rb_leftmost = NULL;
}
...
/*
 * These are initializations that only need to be done once, because the fields are idempotent across use
...
//	INIT_HLIST_NODE(&inode->i_hash);
	...
	// 由于i_mapping是虚类指针，移到对象初始化后初始化。
//	__address_space_init_once(inode->i_mapping);
...
static void inode_lru_list_add(inode *iinode)
{
	//if (list_lru_add(&inode->i_sb->s_inode_lru, &inode->i_lru))
	//	this_cpu_inc(nr_unused);
	if (iinode->i_sb->s_inode_lru.list_lru_add(iinode)) {}
	else	/*iinode->i_state |= I_REFERENCED*/iinode->SetState(I_REFERENCED);
}
/* Add inode to LRU if needed (inode is unused and clean).
 * Needs inode->i_lock held. */
void inode_add_lru(inode *iinode)
{
//	if (!(iinode->i_state & (I_DIRTY_ALL | I_SYNC |	I_FREEING | I_WILL_FREE)) &&
	if (!iinode->TestState((I_DIRTY_ALL | I_SYNC | I_FREEING | I_WILL_FREE)) &&
			!atomic_read(&iinode->i_count) && iinode->i_sb->s_flags & SB_ACTIVE)
		inode_lru_list_add(iinode);
}


static void inode_lru_list_del(inode *iinode)
{
...
static void evict(inode *iinode)
{
...
//	BUG_ON(!(iinode->i_state & I_FREEING));
	...
	if (!list_empty(&iinode->i_io_list))
		inode_io_list_del(iinode);

	inode_sb_list_del(iinode);
...
static void dispose_list(list_head *head)
{
	...
	{
		inode *iinode;
		iinode = list_first_entry(head, inode, i_lru);
		list_del_init(&iinode->i_lru);
		evict(iinode);
//		cond_resched();
	}
}
...
//		if (i_node->i_state & (I_FREEING|I_WILL_FREE))
		if (i_node->TestState(I_FREEING|I_WILL_FREE))
		{
			__wait_on_freeing_inode(i_node);
			goto repeat;
		}
//		if (unlikely(i_node->i_state & I_CREATING)) {
		if (unlikely(i_node->TestState(I_CREATING))) 
		{
...
int generic_update_time(struct inode *inode, timespec64 *time, int flags)
{
	int dirty_flags = 0;
 ...
 */
static int update_time(struct inode *inode, timespec64 *time, int flags)
{
...
int file_remove_privs(struct file *file)
{
	...
	/* Fast path for nothing security related. As well for non-regular files, e.g. blkdev inodes. For example,
	blkdev_write_iter() might get here trying to remove privs which it is not allowed to. */
	if (IS_NOSEC(inode) || !S_ISREG(inode->i_mode))	return 0;

	kill = dentry_needs_remove_privs(dentry);
	if (kill < 0)	return kill;
	if (kill)		error = __remove_privs(file_mnt_user_ns(file), dentry, kill);
...
int file_update_time(struct file *file)
{
	...
	now = current_time(inode);
	if (!timespec64_equal(&inode->i_mtime, &now))	sync_it = S_MTIME;

	if (!timespec64_equal(&inode->i_ctime, &now))	sync_it |= S_CTIME;

	if (IS_I_VERSION(inode) && inode_iversion_need_inc(inode))	sync_it |= S_VERSION;

	if (!sync_it)	return 0;

	/* Finally allowed to write? Takes lock. */
	if (__mnt_want_write_file(file))	return 0;
 ...
 */
void inode_init_owner(user_namespace *mnt_userns, inode *inode, const struct inode *dir, umode_t mode)
{
#if 0 //not support uid/gid
...
static void __inode_dio_wait(struct inode *inode)
{
	...
	do
	{
		...
		if (atomic_read(&inode->i_dio_count))		schedule();
...
{
	if (atomic_read(&inode->i_dio_count))	__inode_dio_wait(inode);
}
 ...
 *
 * Note: the caller should be holding i_mutex, or else be sure that they have exclusive access to the inode structure
	(i.e., while the inode is being instantiated).  The reason for the cmpxchg() loop --- which wouldn't be necessary 
	if all code paths which modify i_flags actually followed this rule, is that there is at least one code path which
	doesn't today so we use cmpxchg() out of an abundance of caution.
 *
 * In the long run, i_mutex is overkill, and we should probably look at using the i_lock spinlock to protect i_flags, 
	and then make sure it is so documented in include/linux/fs.h and that all code follows the locking convention!! */
void inode_set_flags(inode *inode, unsigned int flags, unsigned int mask)
{
	//WARN_ON_ONCE(flags & ~mask);
	JCASSERT((flags & ~mask) == 0);
	set_mask_bits(&inode->i_flags, mask, flags);
}
//EXPORT_SYMBOL(inode_set_flags);

//void inode_nohighmem(inode *inode)
void inode::inode_nohighmem(void)
{
	mapping_set_gfp_mask(i_mapping, GFP_USER);
}
...
void inode::make_bad_inode(void)
{
	...
	i_mode = S_IFREG;
	//TODO
//	i_atime = i_mtime = i_ctime = current_time(static_cast<inode*>(this) );
	i_opflags &= ~IOP_XATTR;
	m_is_bad = true;
}
...
inode::~inode(void) 
{
	JCASSERT(i_mapping == NULL);
	CloseHandle(m_event_state);
}

void inode::SetStateNotify(unsigned long state)
{
	unsigned long ss = i_state;
	i_state |= state;
	if (ss != i_state) SetEvent(m_event_state);
}

void inode::ClearStateNotify(unsigned long state)
{
	unsigned long ss = i_state;
	i_state &= ~state;
	if (ss != i_state) SetEvent(m_event_state);
