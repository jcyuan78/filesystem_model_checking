...
#include "../include/buffer_head.h" /* __set_page_dirty_buffers */
//#include "../include/pagevec.h"
//#include "../include/timer.h"
//#include "../include/sched/rt.h"
//#include "../include/sched/signal.h"
//#include "../include/mm_inline.h"
//#include "../include/events/writeback.h"
...
#endif
	bdi_writeback	*wb;
//	fprop_local_percpu *wb_completions;
...
static void wb_domain_writeout_inc(wb_domain *dom, fprop_local_percpu *completions, unsigned int max_prop_frac)
{
...
static inline void __wb_writeout_inc(bdi_writeback *wb)
{
	...
	if (cgdom) wb_domain_writeout_inc(cgdom, wb_memcg_completions(wb), wb->bdi->max_prop_frac);
#endif
}
...
{
#if 0
	dirty_throttle_control gdtc_stor; // = { GDTC_INIT(wb) };
	gdtc_stor.wb = wb;

	dirty_throttle_control mdtc_stor;	// = { MDTC_INIT(wb, &gdtc_stor) };
...
free_running:
		...
		if (unlikely(!writeback_in_progress(wb)))		wb_start_background_writeback(wb);
		...
		if (!strictlimit)
		{
			...
			//    gdtc->wb_dirty < dirty_freerun_ceiling(gdtc->wb_thresh,  gdtc->wb_bg_thresh))
			//	/* LOCAL_THROTTLE tasks must not be throttled when below the per-wb freerun ceiling. */
			//	goto free_running;
		}
		...
		if (mdtc) {
			/* If memcg domain is in effect, calculate its pos_ratio.  @wb should satisfy constraints from
			 * both global and memcg domains.  Choose the one w/ lower pos_ratio.	 */
			if (!strictlimit)
			{
				wb_dirty_limits(mdtc);

				//if ((current->flags & PF_LOCAL_THROTTLE) &&
				//    mdtc->wb_dirty < dirty_freerun_ceiling(mdtc->wb_thresh,  mdtc->wb_bg_thresh))
				//	/* LOCAL_THROTTLE tasks must not be throttled when below the per-wb freerun ceiling. */
				//	goto free_running;
			}
...
pause:
		//trace_balance_dirty_pages(wb, sdtc->thresh,
		//			  sdtc->bg_thresh,  sdtc->dirty,
		//			  sdtc->wb_thresh,  sdtc->wb_dirty,
		//			  dirty_ratelimit,  task_ratelimit,
		//			  pages_dirtied,  period,
		//			  pause,		  start_time);
...
void balance_dirty_pages_ratelimited(address_space *mapping)
{
	...
	backing_dev_info *bdi = inode_to_bdi(inode);
	bdi_writeback *wb = NULL;
//	int ratelimit;
//	int *p=nullptr;

	if (!(bdi->capabilities & BDI_CAP_WRITEBACK))	return;
...
#endif
//	if (unlikely(current->nr_dirtied >= ratelimit))	balance_dirty_pages(wb, current->nr_dirtied);
//	balance_dirty_pages(wb, current->nr_dirtied);
	balance_dirty_pages(wb, 0);
	wb_put(wb);
}
 ...
 * This function scans the page range from @start to @end (inclusive) and tags all pages that have DIRTY tag set with a
   special TOWRITE tag. The idea is that write_cache_pages (or whoever calls this function) will then use TOWRITE tag 
   to identify pages eligible for writeback.  This mechanism is used to avoid livelocking of writeback by a process 
   steadily creating new dirty pages in the file (thus it is important for this function to be quick so that it can tag
   pages faster than a dirtying process can create them). */
void tag_pages_for_writeback(address_space *mapping, pgoff_t start, pgoff_t end)
{
...
//	xas_for_each_marked(&xas, page, end, PAGECACHE_TAG_DIRTY) 
	for (page = xas_find_marked(&xas, end, PAGECACHE_TAG_DIRTY); page; page = xas_next_marked(&xas, end, PAGECACHE_TAG_DIRTY))
	{
		xas_set_mark(&xas, PAGECACHE_TAG_TOWRITE);
		if (++tagged % XA_CHECK_SCHED)		continue;
...
#endif //TODO
//int do_writepages(address_space *mapping, writeback_control *wbc)

int address_space::do_writepages(writeback_control *wbc)
{
	int ret;
...
// 应该仅仅是计数功能
void account_page_dirtied(page *ppage, address_space *mapping)
{
...
//	trace_writeback_dirty_page(page, mapping);
	...
	{
		bdi_writeback *wb;
		inode_attach_wb(iinode, ppage);
		wb = inode_to_wb(iinode);

		__inc_lruvec_page_state(ppage, NR_FILE_DIRTY);

//		__inc_zone_page_state(ppage, NR_ZONE_WRITE_PENDING);
//		__inc_node_page_state(ppage, NR_DIRTIED);

		inc_wb_stat(wb, WB_RECLAIMABLE);
		inc_wb_stat(wb, WB_DIRTIED);
//		task_io_account_write(PAGE_SIZE);				//可以不支持
//		current->nr_dirtied++;
//		this_cpu_inc(bdp_ratelimits);
//		mem_cgroup_track_foreign_dirty(ppage, wb);		//可以不支持
	}
#else
	// TODO：根据IVirtualDisk的方式进行处理。
	JCASSERT(0);
#endif
}
...
/* When a writepage implementation decides that it doesn't want to write this page for some reason, it should redirty 
   the locked page via redirty_page_for_writepage() and it should then unlock the page and return 0 */
int redirty_page_for_writepage(writeback_control *wbc, page *ppage)
{
	int ret;
	wbc->pages_skipped++;
	ret = __set_page_dirty_nobuffers(ppage);
	account_page_redirty(ppage);
	return ret;
}
...
int test_clear_page_writeback(page *ppage)
{
	...
	{
		LOG_DEBUG_(1,L"mapping use writeback tags, mapping=%llX, flag=%X", mapping, mapping->flags);
		inode *iinode = mapping->host;
		backing_dev_info *bdi = inode_to_bdi(iinode);
		unsigned long flags=0;

		xa_lock_irqsave(&mapping->i_pages, flags);
		ret = TestClearPageWriteback(ppage);
		LOG_DEBUG_(1,L"check page writeback=%d, page=%llX, index=%d", ret, ppage, ppage->index);
		if (ret)
		{
			__xa_clear_mark(&mapping->i_pages, page_index(ppage), PAGECACHE_TAG_WRITEBACK);
		...
		if (mapping->host && !mapping->mapping_tagged(PAGECACHE_TAG_WRITEBACK))
	...
	if (ret) 
	{
		dec_lruvec_page_state(ppage, NR_WRITEBACK);
//		dec_zone_page_state(ppage, NR_ZONE_WRITE_PENDING);
//		inc_node_page_state(ppage, NR_WRITTEN);
	}
...
//	if (wbc->sync_mode == WB_SYNC_ALL)		flags |= REQ_SYNC;
//	else if (wbc->for_kupdate || wbc->for_background)	flags |= REQ_BACKGROUND;
//	return flags;
//}
