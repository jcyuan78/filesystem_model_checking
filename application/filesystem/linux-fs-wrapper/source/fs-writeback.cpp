...
/* If an inode is constantly having its pages dirtied, but then the updates stop dirtytime_expire_interval seconds in the past, it's possible for the worst case time between when an inode has its timestamps updated and when they finally get written out to be two dirtytime_expire_intervals.  We set the default to 12 hours (in seconds), which means most of the time inodes will have their timestamps written to disk after 12 hours, but in the worst case a few inodes might not their timestamps updated for 24 hours. */
 ...
 *
 * Remove @inode which may be on one of @wb->b_{dirty|io|more_io} lists and clear %WB_has_dirty_io if all are empty afterwards.*/
static void inode_io_list_del_locked(struct inode *inode, bdi_writeback *wb)
{
//	assert_spin_locked(&wb->list_lock);
//	assert_spin_locked(&inode->i_lock);

	//inode->i_state &= ~I_SYNC_QUEUED;
	inode->ClearState(I_SYNC_QUEUED);
...
static bdi_writeback *inode_to_wb_and_lock_list(inode *iinode)
//	__acquires(&wb->list_lock)
{
	bdi_writeback *wb = inode_to_wb(iinode);
	spin_lock(&wb->list_lock);
 ...
 *
 * Description:
 *   This makes sure WB_SYNC_NONE background writeback happens. When this function returns, it is only guaranteed that 
 for given wb some IO is happening if we are over background dirty threshold. Caller need not hold sb s_umount semaphore. */
...
#if 0	// wake up在ClearState()中实现
	smp_mb();
	wake_up_bit(&inode->i_state, __I_SYNC);
#endif
	inode->ClearStateNotify(I_SYNC);
}
...
static int write_inode(inode *iinode, writeback_control *wbc)
{
	int ret;

	if (/*iinode->i_sb->s_op->write_inode && */!iinode->is_bad_inode()) 
	{
//		trace_writeback_write_inode_start(iinode, wbc);
//		ret = iinode->i_sb->s_op->write_inode(iinode, wbc);
		ret = iinode->i_sb->write_inode(iinode, wbc);
//		trace_writeback_write_inode(iinode, wbc);
		return ret;
	}
	return 0;
}
...
static void __inode_wait_for_writeback(inode *iinode)
	//__releases(iinode->i_lock)
	//__acquires(iinode->i_lock)
{
	//DEFINE_WAIT_BIT(wq, &iinode->i_state, __I_SYNC);
	//wait_queue_head_t *wqh;

	//wqh = bit_waitqueue(&iinode->i_state, __I_SYNC);
	//while (iinode->i_state & I_SYNC) 
	//{
	//	spin_unlock(&iinode->i_lock);
...
/* Write out an inode and its dirty pages (or some of its dirty pages, depending on @wbc->nr_to_write), and clear the relevant dirty flags from i_state.
 *
 * This doesn't remove the inode from the writeback list it is on, except potentially to move it from b_dirty_time to b_dirty due to timestamp expiration.  The caller is otherwise responsible for writeback list handling.
 *
 * The caller is also responsible for setting the I_SYNC flag beforehand and calling inode_sync_complete() to clear it afterwards.*/
static int __writeback_single_inode(inode *iinode, writeback_control *wbc)
{
...
//	WARN_ON(!(iinode->i_state & I_SYNC));
	JCASSERT(iinode->TestState(I_SYNC));
//	trace_writeback_single_inode_start(iinode, wbc, nr_to_write);
	ret = mapping->do_writepages( wbc);
	/* Make sure to wait on the data before writing out the metadata. 
	 * This is important for filesystems that modify metadata on data I/O completion. We don't do it for sync(2) writeback because it has a separate, external IO completion path and ->sync_fs for guaranteeing iinode metadata is written back correctly.	 */
	...
	/* If the iinode has dirty timestamps and we need to write them, call mark_inode_dirty_sync() to notify the filesystem about it and to change I_DIRTY_TIME into I_DIRTY_SYNC. */
	if (/*(iinode->i_state & I_DIRTY_TIME)*/iinode->TestState(I_DIRTY_TIME) &&   (wbc->sync_mode == WB_SYNC_ALL ||
...
//	iinode->lock();
	...
	/* Paired with smp_mb() in __mark_inode_dirty().  This allows __mark_inode_dirty() to test i_state without grabbing i_lock - either they see the I_DIRTY bits cleared or we see the dirtied inode.
	 * I_DIRTY_PAGES is always cleared together above even if @mapping still has dirty pages.  The flag is reinstated after smp_mb() if necessary.  This guarantees that either __mark_inode_dirty() sees clear I_DIRTY_PAGES or we see PAGECACHE_TAG_DIRTY. */
//	smp_mb();
...
//	spin_unlock(&iinode->i_lock);
	...
	{
		int err = write_inode(iinode, wbc);
		if (ret == 0)	ret = err;
	}
//	trace_writeback_single_inode(iinode, wbc, nr_to_write);
	return ret;
}

/* Write out an inode's dirty data and metadata on-demand, i.e. separately from the regular batched writeback done by the flusher threads in writeback_sb_inodes().  @wbc controls various aspects of the write, such as whether it is a data-integrity sync (%WB_SYNC_ALL) or not (%WB_SYNC_NONE).
 * To prevent the inode from going away, either the caller must have a reference to the inode, or the inode must have I_WILL_FREE or I_FREEING set. */
static int writeback_single_inode(inode *iinode, writeback_control *wbc)
{
	bdi_writeback *wb;
	int ret = 0;
...
//		__inode_wait_for_writeback(inode);
	...
	ret = __writeback_single_inode(iinode, wbc);
 ...
 *
 * @inode: inode to mark
 * @flags: what kind of dirty, e.g. I_DIRTY_SYNC.  This can be a combination of multiple I_DIRTY_* flags, except that
           I_DIRTY_TIME can't be combined with I_DIRTY_PAGES.
 *
 * Mark an inode as dirty.  We notify the filesystem, then update the inode's dirty flags.  Then, if needed we add the
...
//		trace_writeback_dirty_inode(iinode, flags);
	...
	{
		/* Else it's either I_DIRTY_PAGES, I_DIRTY_TIME, or nothing. (We don't support setting both I_DIRTY_PAGES and
		   I_DIRTY_TIME in one call to __mark_inode_dirty().)	 */
		dirtytime = flags & I_DIRTY_TIME;
		JCASSERT(!(dirtytime && flags != I_DIRTY_TIME));
	}
...
static void wait_sb_inodes(struct super_block *sb)
{
	...
	while (!list_empty(&sync_list)) {
		 ...
		 * will remove it.
		 */
		if (!mapping->mapping_tagged(PAGECACHE_TAG_WRITEBACK))
			continue;
