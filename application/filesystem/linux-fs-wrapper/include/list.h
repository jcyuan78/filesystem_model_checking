...
//#define WRITE_ONCE(var, val) (((volatile)((var))) = (val))
...
# define POISON_POINTER_DELTA 0
  ...
  */
#define LIST_POISON1  ((list_head *) 0x100 + POISON_POINTER_DELTA)
#define LIST_POISON2  ((list_head *) 0x122 + POISON_POINTER_DELTA)
...
extern bool __list_add_valid(struct list_head *new_var,
...
static inline void __list_add(list_head *new_var, list_head *prev, list_head *next)
{
	...
	new_var->next = next;
	new_var->prev = prev;
	WRITE_ONCE(prev->next, new_var);
}

/**
 * list_add - add a new entry
 * @new_var: new entry to be added
 * @head: list head to add it after
 *
 * Insert a new entry after the specified head.
 * This is good for implementing stacks.
 */
static inline void list_add(struct list_head *new_var, struct list_head *head)
{
	__list_add(new_var, head, head->next);
}
 ...
 */
static inline void list_add_tail(struct list_head *new_var, struct list_head *head)
{
	__list_add(new_var, head->prev, head);
}
 ...
 * @new_var : the new element to insert
 *
 * If @old was empty, it will be overwritten.
 */
static inline void list_replace(struct list_head *old,
				struct list_head *new_var)
{
	new_var->next = old->next;
	new_var->next->prev = new_var;
	new_var->prev = old->prev;
 ...
 */
#define list_entry(ptr, type, member) 	container_of<type>(ptr, offsetof(type, member))
 ...
 * @new_var: hlist_head for new list.
 *
 * Move a list from one list head to another. Fixup the pprev
 * reference of the first entry if it exists.
 */
static inline void hlist_move_list(struct hlist_head *old,
				   struct hlist_head *new_var)
{
	new_var->first = old->first;
	if (new_var->first)
		new_var->first->pprev = &new_var->first;
	old->first = NULL;
}

//#define hlist_entry(ptr, type, member) container_of(ptr,type,member)
#define hlist_entry(ptr, type, member) container_of<type>(ptr, offsetof(type, member) )
