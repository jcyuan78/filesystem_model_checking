...
static inline bool mapping_empty(address_space *mapping)
{
...
static inline void mapping_set_error(address_space *mapping, int error)
{
...
static inline gfp_t mapping_gfp_mask(address_space * mapping)
{
	return mapping->gfp_mask;
}
#if 0 //<TODO>
/* Restricts the given gfp_mask to what the mapping allows. */
static inline gfp_t mapping_gfp_constraint(address_space *mapping, gfp_t gfp_mask)
{
 ...
 */
static inline int __page_cache_add_speculative(page *ppage, int count)
{
//#ifdef CONFIG_TINY_RCU
//# ifdef CONFIG_PREEMPT_COUNT
//	VM_BUG_ON(!in_atomic() && !irqs_disabled());
//# endif
//	/*Preempt must be disabled here - we rely on rcu_read_lock doing this for us.
//	 *
//	 * Pagecache won't be truncated from interrupt context, so if we have found a page in the radix tree here, we have pinned its refcount by disabling preempt, and hence no need for the "speculative get" that SMP requires. */
	//VM_BUG_ON_PAGE(page_count(ppage) == 0, ppage);
	//page_ref_add(ppage, count);
	JCASSERT(ppage->ref_count() > 0);
	ppage->ref_add(count);
//
//#else
//	if (unlikely(!page_ref_add_unless(ppage, count, 0))) {
//		/* Either the page has been freed, or will be freed. In either case, retry here and the caller should do the right thing (see comments above).	 */
//		return 0;
//	}
//#endif
...
static inline gfp_t readahead_gfp_mask(address_space *x)
{
...
#endif //<TODO>
/* Given the page we found in the page cache, return the page corresponding to this index in the file */
static inline struct page *find_subpage(struct page *head, pgoff_t index)
{
	/* HugeTLBfs wants the head page regardless */
	if (PageHuge(head))		return head;
...
inline int __get_user(char c, const char* uaddr) { return 0; }
...
static inline int fault_in_pages_readable(const char __user *uaddr, int size)
{
	...
	if (unlikely(uaddr > end))		return -EFAULT;

	do 
	{
		if (unlikely(__get_user(c, uaddr) != 0))	return -EFAULT;
		uaddr += PAGE_SIZE;
	} while (uaddr <= end);

	/* Check whether the range spilled into the next page. */
	if (((UINT64)uaddr & PAGE_MASK) == ((UINT64)end & PAGE_MASK)) 
	{
...
int add_to_page_cache_locked(struct page *page, address_space *mapping, pgoff_t index, gfp_t gfp_mask);
int add_to_page_cache_lru(struct page *page, address_space *mapping, pgoff_t index, gfp_t gfp_mask);
extern void delete_from_page_cache(struct page *page);
extern void __delete_from_page_cache(struct page *page, void *shadow);
void replace_page_cache_page(struct page *old, struct page *new_page);
void delete_from_page_cache_batch(address_space *mapping,  struct pagevec *pvec);
loff_t mapping_seek_hole_data(address_space *, loff_t start, loff_t end, int whence);
...
void page_cache_ra_unbounded(readahead_control *, unsigned long nr_to_read, unsigned long lookahead_count);
void page_cache_sync_ra(readahead_control *, unsigned long req_count);
void page_cache_async_ra(readahead_control *, struct page *, unsigned long req_count);
void readahead_expand(readahead_control *ractl, loff_t new_start, size_t new_len);
...
static inline unsigned int __readahead_batch(readahead_control *rac, page **array, unsigned int array_sz)
{
	unsigned int i = 0;
...
//	rcu_read_lock();
	...
	{
		if (xas_retry(&xas, page))		continue;
//		VM_BUG_ON_PAGE(!PageLocked(page), page);
//		VM_BUG_ON_PAGE(PageTail(page), page);
		array[i++] = page;
		rac->_batch_count += thp_nr_pages(page);

		/* The page cache isn't using multi-index entries yet, so the xas cursor needs to be manually moved to the
		 * next index.  This can be removed once the page cache is converted.	 */
		if (PageHead(page))		xas_set(&xas, rac->_index + rac->_batch_count);
...
static inline int page_mkwrite_check_truncate(struct page *page,   struct inode *inode)
{
	...
	pgoff_t index = (pgoff_t)(size >> PAGE_SHIFT);
	int offset = offset_in_page(size);

//	if (page->mapping != inode->i_mapping)	return -EFAULT;
	if (!inode->mapping_equal(page->mapping)) return -EFAULT;

	/* page is wholly inside EOF */
	if (page->index < index)		return PAGE_SIZE;
	/* page is wholly past EOF */
	if (page->index > index || !offset)		return -EFAULT;
