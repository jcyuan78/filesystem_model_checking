...
static inline void __rb_change_child_rcu(rb_node *old, rb_node *new_node, rb_node *parent, rb_root *root)
{
	if (parent) 
	{
		if (parent->rb_left == old)			rcu_assign_pointer(parent->rb_left, new_node);
		else			rcu_assign_pointer(parent->rb_right, new_node);
	} 
	else		rcu_assign_pointer(root->rb_node, new_node);
}
