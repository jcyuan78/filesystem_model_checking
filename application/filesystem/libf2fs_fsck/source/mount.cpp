...
static int get_device_idx(fsck_f2fs_sb_info *sbi, u_int32_t segno)
{
	block_t seg_start_blkaddr;
	int i;

	seg_start_blkaddr = sbi->SM_I()->main_blkaddr +	segno * DEFAULT_BLOCKS_PER_SEGMENT;
	for (i = 0; i < c.ndevs; i++)
	{
		if (c.devices[i].start_blkaddr <= seg_start_blkaddr && c.devices[i].end_blkaddr > seg_start_blkaddr)
			return i;
	}
	return 0;
}
...
static int get_zone_idx_from_dev(fsck_f2fs_sb_info *sbi,
					u_int32_t segno, u_int32_t dev_idx)
{
	block_t seg_start_blkaddr = FSCK_START_BLOCK(sbi, segno);

	return (seg_start_blkaddr - c.devices[dev_idx].start_blkaddr) >>
			log_base_2(sbi->segs_per_sec * sbi->blocks_per_seg);
}

bool is_usable_seg(fsck_f2fs_sb_info *sbi, unsigned int segno)
{
	unsigned int secno = segno / sbi->segs_per_sec;
	block_t seg_start = FSCK_START_BLOCK(sbi, segno);
	...
	unsigned int sec_off = sbi->SM_I()->main_blkaddr >>
...
unsigned int get_usable_seg_count(fsck_f2fs_sb_info *sbi)
{
...
bool is_usable_seg(fsck_f2fs_sb_info *UNUSED(sbi), unsigned int UNUSED(segno))
{
	return true;
}
...
u32 get_free_segments(fsck_f2fs_sb_info *sbi)
{
	u32 i, free_segs = 0;

	for (i = 0; i < TOTAL_SEGS(sbi); i++) {
		fsck_seg_entry *se = get_seg_entry(sbi, i);
		if (se->valid_blocks == 0x0 && ! IS_CUR_SEGNO(sbi, i) && is_usable_seg(sbi, i))
			free_segs++;
	}
	return free_segs;
}

void update_free_segments(fsck_f2fs_sb_info *sbi)
{
	const char *progress = "-*|*-";
	static int i = 0;
...
static void print_xattr_entry(const struct f2fs_xattr_entry *ent)
{
	...
	for (i = 0; i < ent->e_name_len; i++)	MSG(0, "%c", ent->e_name[i]);
...
void print_inode_info(fsck_f2fs_sb_info *sbi, f2fs_node *node, int name)
{
	f2fs_inode *inode = &node->i;
	void *xattr_addr;
	struct f2fs_xattr_entry *ent;
	char en[F2FS_PRINT_NAMELEN];
	unsigned int i = 0;
	u32 namelen = le32_to_cpu(inode->i_namelen);
	int enc_name = 0;// file_enc_name(inode);
	int ofs = get_extra_isize(inode);

	pretty_print_filename(inode->i_name, namelen, en, enc_name);
	if (name && en[0]) {
		MSG(0, " - File name         : %s%s\n", en,	enc_name ? " <encrypted>" : "");
		setlocale(LC_ALL, "");
	...
	printf("i_ext: fofs:%x blkaddr:%x len:%x\n", le32_to_cpu(inode->i_ext.fofs), le32_to_cpu(inode->i_ext.blk),	le32_to_cpu(inode->i_ext.len));

	if (c.feature & cpu_to_le32(F2FS_FEATURE_EXTRA_ATTR)) {
		DISP_u16(inode, _u._s.i_extra_isize);
		if (c.feature & cpu_to_le32(F2FS_FEATURE_FLEXIBLE_INLINE_XATTR))	DISP_u16(inode, _u._s.i_inline_xattr_size);
		if (c.feature & cpu_to_le32(F2FS_FEATURE_PRJQUOTA))					DISP_u32(inode, _u._s.i_projid);
		if (c.feature & cpu_to_le32(F2FS_FEATURE_INODE_CHKSUM))				DISP_u32(inode, _u._s.i_inode_checksum);
		if (c.feature & cpu_to_le32(F2FS_FEATURE_INODE_CRTIME)) {
			DISP_u64(inode, _u._s.i_crtime);
			DISP_u32(inode, _u._s.i_crtime_nsec);
		}
		if (c.feature & cpu_to_le32(F2FS_FEATURE_COMPRESSION)) {
			DISP_u64(inode, _u._s.i_compr_blocks);
//			DISP_u32(inode, _u._s.i_compress_algrithm);
			DISP_u32(inode, _u._s.i_log_cluster_size);
//			DISP_u32(inode, _u._s.i_padding);
		}
	}
	...
	for (i = 0; i < ADDRS_PER_INODE(inode); i++) {
		...
		if (i + ofs >= DEF_ADDRS_PER_INODE)		break;

		blkaddr = le32_to_cpu(inode->_u.i_addr[i + ofs]);

		if (blkaddr == 0x0)				continue;
		if (blkaddr == COMPRESS_ADDR)	flag = "cluster flag";
		else if (blkaddr == NEW_ADDR)	flag = "reserved flag";
...
void print_node_info(fsck_f2fs_sb_info *sbi,	f2fs_node *node_block, int verbose)
{
...
//	utf16_to_utf8(buffer, name, MAX_VOLUME_NAME, MAX_VOLUME_NAME);
	jcvos::UnicodeToUtf8(buffer, MAX_VOLUME_NAME, (wchar_t*)name, wcslen((wchar_t*)name));
	if (c.layout)		printf("%-30s %s\n", "Filesystem volume name:", buffer);
	else				printf("%-30s" "\t\t[%s]\n", "volum_name", buffer);
}

void print_raw_sb_info(f2fs_super_block *sb)
{
	if (c.layout)		goto printout;
	if (!c.dbg_lv)		return;
...
printout:
	...
	DISP_label((u_int16_t*)sb->volume_name);
...
void print_ckpt_info(fsck_f2fs_sb_info *sbi)
{
	f2fs_checkpoint *cp = sbi->F2FS_CKPT();
...
{
	if (c.show_file_map)		return;

	MSG(0, "Info: checkpoint state = %x : ", flag);
	if (flag & CP_QUOTA_NEED_FSCK_FLAG)		MSG(0, "%s", " quota_need_fsck");
	if (flag & CP_LARGE_NAT_BITMAP_FLAG)	MSG(0, "%s", " large_nat_bitmap");
	if (flag & CP_NOCRC_RECOVERY_FLAG)		MSG(0, "%s", " allow_nocrc");
	if (flag & CP_TRIMMED_FLAG)				MSG(0, "%s", " trimmed");
	if (flag & CP_NAT_BITS_FLAG)			MSG(0, "%s", " nat_bits");
	if (flag & CP_CRC_RECOVERY_FLAG)		MSG(0, "%s", " crc");
	if (flag & CP_FASTBOOT_FLAG)			MSG(0, "%s", " fastboot");
	if (flag & CP_FSCK_FLAG)				MSG(0, "%s", " fsck");
	if (flag & CP_ERROR_FLAG)				MSG(0, "%s", " error");
	if (flag & CP_COMPACT_SUM_FLAG)			MSG(0, "%s", " compacted_summary");
	if (flag & CP_ORPHAN_PRESENT_FLAG)		MSG(0, "%s", " orphan_inodes");
	if (flag & CP_DISABLED_FLAG)			MSG(0, "%s", " disabled");
	if (flag & CP_RESIZEFS_FLAG)			MSG(0, "%s", " resizefs");
	if (flag & CP_UMOUNT_FLAG)				MSG(0, "%s", " unmount");
	else									MSG(0, "%s", " sudden-power-off");
	MSG(0, "\n");
}

void print_sb_state(f2fs_super_block *sb)
{
	__le32 f = sb->feature;
	int i;

	MSG(0, "Info: superblock features = %x : ", f);
	if (f & cpu_to_le32(F2FS_FEATURE_ENCRYPT))					{	MSG(0, "%s", " encrypt");	}
	if (f & cpu_to_le32(F2FS_FEATURE_VERITY))					{	MSG(0, "%s", " verity");	}
	if (f & cpu_to_le32(F2FS_FEATURE_BLKZONED))					{	MSG(0, "%s", " blkzoned");	}
	if (f & cpu_to_le32(F2FS_FEATURE_EXTRA_ATTR))				{	MSG(0, "%s", " extra_attr");	}
	if (f & cpu_to_le32(F2FS_FEATURE_PRJQUOTA))					{	MSG(0, "%s", " project_quota");	}
	if (f & cpu_to_le32(F2FS_FEATURE_INODE_CHKSUM))				{	MSG(0, "%s", " inode_checksum");	}
	if (f & cpu_to_le32(F2FS_FEATURE_FLEXIBLE_INLINE_XATTR))	{	MSG(0, "%s", " flexible_inline_xattr");	}
	if (f & cpu_to_le32(F2FS_FEATURE_QUOTA_INO))				{	MSG(0, "%s", " quota_ino");	}
	if (f & cpu_to_le32(F2FS_FEATURE_INODE_CRTIME))				{	MSG(0, "%s", " inode_crtime");	}
	if (f & cpu_to_le32(F2FS_FEATURE_LOST_FOUND))				{	MSG(0, "%s", " lost_found");	}
	if (f & cpu_to_le32(F2FS_FEATURE_SB_CHKSUM))				{	MSG(0, "%s", " sb_checksum");	}
	if (f & cpu_to_le32(F2FS_FEATURE_CASEFOLD))					{	MSG(0, "%s", " casefold");	}
	if (f & cpu_to_le32(F2FS_FEATURE_COMPRESSION))				{	MSG(0, "%s", " compression");	}
	if (f & cpu_to_le32(F2FS_FEATURE_RO))						{	MSG(0, "%s", " ro");	}
	MSG(0, "\n");
	MSG(0, "Info: superblock encrypt level = %d, salt = ",	sb->encryption_level);
	for (i = 0; i < 16; i++)		MSG(0, "%02x", sb->encrypt_pw_salt[i]);
	MSG(0, "\n");
}

bool f2fs_is_valid_blkaddr(fsck_f2fs_sb_info *sbi, block_t blkaddr, int type)
{
	switch (type) {
	case META_NAT:
		break;
	case META_SIT:
		if (blkaddr >= SIT_BLK_CNT(sbi))											return 0;
		break;
	case META_SSA:
		if (blkaddr >= MAIN_BLKADDR(sbi) ||	blkaddr < sbi->SM_I()->ssa_blkaddr)		return 0;
		break;
	case META_CP:
		if (blkaddr >= SIT_I(sbi)->sit_base_addr ||	blkaddr < __start_cp_addr(sbi))	return 0;
		break;
	case META_POR:
		if (blkaddr >= MAX_BLKADDR(sbi) || blkaddr < MAIN_BLKADDR(sbi))				return 0;
		break;
	default:
		ASSERT(0);
	}
...
static inline block_t current_sit_addr(fsck_f2fs_sb_info *sbi, unsigned int start);

/* Readahead CP/NAT/SIT/SSA pages */
int f2fs_ra_meta_pages(fsck_f2fs_sb_info *sbi, block_t start, int nrpages, int type)
{
	block_t blkno = start;
	...
	for (; nrpages-- > 0; blkno++) {
		...
		switch (type) {
		case META_NAT:
			if (blkno >= NAT_BLOCK_OFFSET(sbi->NM_I()->max_nid))			blkno = 0;
			/* get nat block addr */
			blkaddr = current_nat_addr(sbi,	blkno * NAT_ENTRY_PER_BLOCK, NULL);
			break;
		case META_SIT:
			/* get sit block addr */
			blkaddr = current_sit_addr(sbi,	blkno * SIT_ENTRY_PER_BLOCK);
			break;
		case META_SSA:
		case META_CP:
		case META_POR:
		...
		if (!len) {
			...
			fs->dev_readahead(start_blk << F2FS_BLKSIZE_BITS, len << F2FS_BLKSIZE_BITS);
		}
	}
out:
	if (len)
...
void update_superblock(CF2fsFileSystem* fs, f2fs_super_block *sb, int sb_mask)
{
	int addr, ret;
	u_int8_t *buf;
	u32 old_crc, new_crc;

	buf = (u_int8_t*)calloc(BLOCK_SZ, 1);
	ASSERT(buf);
	...
	for (addr = SB0_ADDR; addr < SB_MAX_ADDR; addr++) {
		if (SB_MASK(addr) & sb_mask) {
			ret = fs->dev_write_block((BYTE*)buf, addr);
			ASSERT(ret >= 0);
		}
	}
	free(buf);
...
static inline int sanity_check_area_boundary(CF2fsFileSystem * fs, f2fs_super_block *sb, enum SB_ADDR sb_addr)
{
	...
	if (main_end_blkaddr > seg_end_blkaddr) {
		...
		set_sb(segment_count, (main_end_blkaddr -segment0_blkaddr) >> log_blocks_per_seg);

		update_superblock(fs, sb, SB_MASK(sb_addr));
		MSG(0, "Info: Fix alignment: start(%u) end(%u) block(%u)\n",
			main_blkaddr, segment0_blkaddr + (segment_count << log_blocks_per_seg),segment_count_main << log_blocks_per_seg);
	}
	return 0;
}

static int verify_sb_chksum(f2fs_super_block *sb)
{
...
static int sanity_check_raw_super(CF2fsFileSystem * fs, f2fs_super_block *sb, enum SB_ADDR sb_addr)
{
	...
	if (sb->devs[0].path[0]) {
		...
		if (segment_count != dev_segs / segs_per_zone * segs_per_zone) {
			MSG(0, "Segment count (%u) mismatch with total segments from devices (%u)",	segment_count, dev_segs);
			return 1;
		}
	}
	...
	if (get_sb(extension_count) > F2FS_MAX_EXTENSION ||
			...
			get_sb(extension_count) + sb->hot_ext_count > F2FS_MAX_EXTENSION) {
	...
	if (sanity_check_area_boundary(fs, sb, sb_addr)) return -1;
	return 0;
}

int validate_super_block(fsck_f2fs_sb_info *sbi, enum SB_ADDR sb_addr)
{
	char buf[F2FS_BLKSIZE];
	CF2fsFileSystem* fs = sbi->GetFs();

	sbi->raw_super = (f2fs_super_block*)malloc(sizeof(f2fs_super_block));
	if (!sbi->raw_super)
		return -ENOMEM;

	if (fs->dev_read_block(buf, sb_addr))	return -1;

	memcpy(sbi->raw_super, buf + F2FS_SUPER_OFFSET,	sizeof(f2fs_super_block));

	if (!sanity_check_raw_super(fs, sbi->raw_super, sb_addr)) {
		/* get kernel version */
		if (c.kd >= 0) {
			fs->dev_read_version(c.version, 0, VERSION_LEN);
		...
		get_kernel_version((char*)c.sb_version);
		memcpy(c.init_version, sbi->raw_super->init_version, VERSION_LEN);
		get_kernel_version((char*)c.init_version);

		MSG(0, "Info: MKFS version\n  \"%s\"\n", c.init_version);
		MSG(0, "Info: FSCK version\n  from \"%s\"\n    to \"%s\"\n", c.sb_version, c.version);
		if (!c.no_kernel_check && memcmp(c.sb_version, c.version, VERSION_LEN)) {
			c.auto_fix = 0;
			c.fix_on = 1;
		}
		if (c.fix_on) {
			memcpy(sbi->raw_super->version, c.version, VERSION_LEN);
			update_superblock(fs, sbi->raw_super, SB_MASK(sb_addr));
		}
...
int init_sb_info(fsck_f2fs_sb_info *sbi)
{
	f2fs_super_block *sb = sbi->F2FS_RAW_SUPER();
	u64 total_sectors;
	//int i;
	...
	sbi->total_node_count = (get_sb(segment_count_nat) / 2) *	sbi->blocks_per_seg * NAT_ENTRY_PER_BLOCK;
	sbi->root_ino_num = get_sb(root_ino);
	sbi->node_ino_num = get_sb(node_ino);
	sbi->meta_ino_num = get_sb(meta_ino);
	sbi->cur_victim_sec = NULL_SEGNO;
	
	// config device在CF2fsFileSystem中调用Connect Device实现
#if 0
	for (i = 0; i < MAX_DEVICES; i++) {
		if (!sb->devs[i].path[0])	break;

		if (i) {
			c.devices[i].path = strdup((char *)sb->devs[i].path);
			if (get_device_info(i))	ASSERT(0);
		}
		else {
			ASSERT(!strcmp((char *)sb->devs[i].path, (char *)c.devices[i].path));
		}

		c.devices[i].total_segments = le32_to_cpu(sb->devs[i].total_segments);
		if (i)	c.devices[i].start_blkaddr = c.devices[i - 1].end_blkaddr + 1;
		c.devices[i].end_blkaddr = c.devices[i].start_blkaddr +	c.devices[i].total_segments * c.blks_per_seg - 1;
		if (i == 0)	c.devices[i].end_blkaddr += get_sb(segment0_blkaddr);

		c.ndevs = i + 1;
		MSG(0, "Info: Device[%d] : %s blkaddr = %lld--%lld\n", i, c.devices[i].path, c.devices[i].start_blkaddr, c.devices[i].end_blkaddr);
	}
#endif
	total_sectors = get_sb(block_count) << sbi->log_sectors_per_block;
	MSG(0, "Info: total FS sectors = %lld (%lld MB)\n", total_sectors, total_sectors >> (20 - get_sb(log_sectorsize)));
	return 0;
}

static int verify_checksum_chksum(f2fs_checkpoint *cp)
{
...
static void *get_checkpoint_version(CF2fsFileSystem * fs, block_t cp_addr)
{
	void *cp_page;
	...
	if (fs->dev_read_block(cp_page, cp_addr) < 0)		ASSERT(0);

	if (verify_checksum_chksum((f2fs_checkpoint *)cp_page))		goto out;
	return cp_page;
out:
	free(cp_page);
	return NULL;
...
void *validate_checkpoint(fsck_f2fs_sb_info *sbi, block_t cp_addr, unsigned long long *version)
{
	...
	cp_page_1 = get_checkpoint_version(fs, cp_addr);
	if (!cp_page_1)		return NULL;

	cp = (f2fs_checkpoint *)cp_page_1;
	...
	cp_page_2 = get_checkpoint_version(fs, cp_addr);
	if (!cp_page_2)		goto invalid_cp1;

	cp = (f2fs_checkpoint *)cp_page_2;
...
int get_valid_checkpoint(fsck_f2fs_sb_info *sbi)
{
	f2fs_super_block *sb = sbi->F2FS_RAW_SUPER();
	CF2fsFileSystem* fs = sbi->GetFs();
	...
	if (cp_payload > F2FS_BLK_ALIGN(MAX_SIT_BITMAP_SIZE))	return -EINVAL;

	cp_blks = 1 + cp_payload;
	sbi->ckpt = (f2fs_checkpoint*)malloc(cp_blks * blk_size);
	if (!sbi->ckpt)	return -ENOMEM;
	/* Finding out valid cp block involves read both sets( cp pack1 and cp pack 2)	 */
	...
	else goto fail_no_cp;

	MSG(0, "Info: CKPT version = %llx, CKPT id=%d\n", version, sbi->cur_cp);
	...
	if (cp_blks > 1) {
		...
		if (cur_page == cp2)			cp_blk_no += 1 << get_sb(log_blocks_per_seg);
		/* copy sit bitmap */
		for (i = 1; i < cp_blks; i++) {
			unsigned char *ckpt = (unsigned char *)sbi->ckpt;
			ret = fs->dev_read_block(cur_page, cp_blk_no + i);
			ASSERT(ret >= 0);
...
/* For a return value of 1, caller should further check for c.fix_on state and take appropriate action. */
static int f2fs_should_proceed(f2fs_super_block *sb, u32 flag)
{
	if (!c.fix_on && (c.auto_fix || c.preen_mode)) {
		if (flag & CP_FSCK_FLAG ||	flag & CP_QUOTA_NEED_FSCK_FLAG || (exist_qf_ino(sb) && (flag & CP_ERROR_FLAG))) {
			c.fix_on = 1;
		} 
...
int sanity_check_ckpt(fsck_f2fs_sb_info *sbi)
{
	CF2fsFileSystem* fs = sbi->GetFs();
	...
	if (!(get_sb(feature) & cpu_to_le32(F2FS_FEATURE_RO)) &&
		(fsmeta < F2FS_MIN_SEGMENT || ovp_segments == 0 || reserved_segments == 0)) {
		MSG(0, "\tWrong layout: check mkfs.f2fs version\n");
		return 1;
	}

	user_block_count = get_cp(user_block_count);
	segment_count_main = get_sb(segment_count_main) + (cpu_to_le32(F2FS_FEATURE_RO) ? 1 : 0);
	...
	if (!user_block_count || user_block_count >= segment_count_main << log_blocks_per_seg) {
		...
		if (!f2fs_should_proceed(sb, flag))		return 1;
		if (!c.fix_on)							return 1;
		...
		if (flag & (CP_FSCK_FLAG | CP_RESIZEFS_FLAG)) {
			...
			valid_user_block_cnt = ((get_sb(segment_count_main) - get_cp(overprov_segment_count)) * c.blks_per_seg);
	...
	for (i = 0; i < NR_CURSEG_NODE_TYPE; i++) {
		if (get_cp(cur_node_segno[i]) >= main_segs || get_cp(cur_node_blkoff[i]) >= blocks_per_seg)		return 1;
	}
	for (i = 0; i < NR_CURSEG_DATA_TYPE; i++) {
		if (get_cp(cur_data_segno[i]) >= main_segs || get_cp(cur_data_blkoff[i]) >= blocks_per_seg)		return 1;
	}
	...
	if (cp_pack_start_sum < cp_payload + 1 || cp_pack_start_sum > blocks_per_seg - 1 - NR_CURSEG_TYPE) {
		MSG(0, "\tWrong cp_pack_start_sum(%u) or cp_payload(%u)\n", cp_pack_start_sum, cp_payload);
		if ((get_sb(feature) & F2FS_FEATURE_SB_CHKSUM))
			return 1;
		set_sb(cp_payload, cp_pack_start_sum - 1);
		update_superblock(fs, sb, SB_MASK_ALL);
	}

	return 0;
}

pgoff_t current_nat_addr(fsck_f2fs_sb_info *sbi, nid_t start, int *pack)
{
	fsck_f2fs_nm_info *nm_i = sbi->NM_I();
	pgoff_t block_off;
	pgoff_t block_addr;
	int seg_off;
...
static int f2fs_early_init_nid_bitmap(fsck_f2fs_sb_info *sbi)
{
	fsck_f2fs_nm_info *nm_i = sbi->NM_I();
	int nid_bitmap_size = (nm_i->max_nid + BITS_PER_BYTE - 1) / BITS_PER_BYTE;
	fsck_curseg_info *curseg = CURSEG_I(sbi, CURSEG_HOT_DATA);
	f2fs_summary_block *sum = curseg->sum_blk;
	f2fs_journal *journal = &sum->journal;
	nid_t nid;
	int i;

	if (!(c.func == SLOAD || c.func == FSCK))	return 0;

	nm_i->nid_bitmap = (char *)calloc(nid_bitmap_size, 1);
	if (!nm_i->nid_bitmap)	return -ENOMEM;

	/* arbitrarily set 0 bit */
	f2fs_set_bit(0, nm_i->nid_bitmap);

	if (nats_in_cursum(journal) > NAT_JOURNAL_ENTRIES) {
		MSG(0, "\tError: f2fs_init_nid_bitmap truncate n_nats(%u) to NAT_JOURNAL_ENTRIES(%lld)\n",	nats_in_cursum(journal), NAT_JOURNAL_ENTRIES);
...
static int f2fs_late_init_nid_bitmap(fsck_f2fs_sb_info *sbi)
{
	CF2fsFileSystem* fs = sbi->GetFs();
	...
	nat_block = (f2fs_nat_block*)malloc(F2FS_BLKSIZE);
	if (!nat_block) {
		free(nm_i->nid_bitmap);
		return -ENOMEM;
	}

	f2fs_ra_meta_pages(sbi, 0, NAT_BLOCK_OFFSET(nm_i->max_nid), META_NAT);
	for (nid = 0; nid < nm_i->max_nid; nid++) {
		if (!(nid % NAT_ENTRY_PER_BLOCK)) {
			int ret;

			start_blk = current_nat_addr(sbi, nid, NULL);
			ret = fs->dev_read_block(nat_block, start_blk);
			ASSERT(ret >= 0);
		}
...
u32 update_nat_bits_flags(f2fs_super_block *sb,	f2fs_checkpoint *cp, u32 flags)
{
...
void write_nat_bits(fsck_f2fs_sb_info *sbi, f2fs_super_block *sb, f2fs_checkpoint *cp, int set)
{
	CF2fsFileSystem* fs = sbi->GetFs();

	fsck_f2fs_nm_info *nm_i = sbi->NM_I();
	u_int32_t nat_blocks = get_sb(segment_count_nat) <<	(get_sb(log_blocks_per_seg) - 1);
	u_int32_t nat_bits_bytes = nat_blocks >> 3;
	u_int32_t nat_bits_blocks = F2FS_BYTES_TO_BLK((nat_bits_bytes << 1) + 8 + F2FS_BLKSIZE - 1);
	...
	nat_bits = (unsigned char*)calloc(F2FS_BLKSIZE, nat_bits_blocks);
	ASSERT(nat_bits);
	...
	for (i = 0; i < nat_blocks; i++) {
		...
		/* Should consider new nat_blocks is larger than old nm_i->nat_blocks, since nm_i->nat_bitmap is based on old one. */
		if (i < nm_i->nat_blocks && f2fs_test_bit(i, nm_i->nat_bitmap))
			blkaddr += (1 << get_sb(log_blocks_per_seg));

		ret = fs->dev_read_block(nat_block, blkaddr);
		ASSERT(ret >= 0);
		...
		if (valid == 0)								test_and_set_bit_le(i, empty_nat_bits);
		else if (valid == NAT_ENTRY_PER_BLOCK)		test_and_set_bit_le(i, full_nat_bits);
	}
	*(__le64 *)nat_bits = get_cp_crc(cp);
	free(nat_block);

	blkaddr = get_sb(segment0_blkaddr) + (set << get_sb(log_blocks_per_seg)) - nat_bits_blocks;

	DBG(1, "\tWriting NAT bits pages, at offset 0x%08x\n", blkaddr);

	for (i = 0; i < nat_bits_blocks; i++) {
		if (fs->dev_write_block((BYTE*)nat_bits + i * F2FS_BLKSIZE, blkaddr + i)) {
...
static int check_nat_bits(fsck_f2fs_sb_info *sbi, f2fs_super_block *sb, f2fs_checkpoint *cp)
{
	CF2fsFileSystem* fs = sbi->GetFs();
	...
	u_int32_t nat_bits_blocks = F2FS_BYTES_TO_BLK((nat_bits_bytes << 1) + 8 + F2FS_BLKSIZE - 1);
	unsigned char *nat_bits, *full_nat_bits, *empty_nat_bits;
	struct fsck_curseg_info *curseg = CURSEG_I(sbi, CURSEG_HOT_DATA);
	...
	blkaddr = get_sb(segment0_blkaddr) + (sbi->cur_cp << get_sb(log_blocks_per_seg)) - nat_bits_blocks;

	for (i = 0; i < nat_bits_blocks; i++) {
		if (fs->dev_read_block(nat_bits + i * F2FS_BLKSIZE, blkaddr + i))
			ASSERT_MSG("\tError: read NAT bits to disk!!!\n");
	}

	if (*(__le64 *)nat_bits != get_cp_crc(cp) || nats_in_cursum(journal)) {
		/* if there is a journal, f2fs was not shutdown cleanly. Let's flush them with nat_bits. */
		if (c.fix_on)			err = -1;
	...
	for (i = 0; i < nat_blocks; i++) {
		...
		for (j = 0; j < NAT_ENTRY_PER_BLOCK; j++) {
			if (f2fs_test_bit(start_nid + j, nm_i->nid_bitmap))		valid++;
		}
		if (valid == 0) {
			if (!empty || full) {
				err = -1;
...
int init_node_manager(fsck_f2fs_sb_info *sbi)
{
	f2fs_super_block *sb = sbi->F2FS_RAW_SUPER();
	f2fs_checkpoint *cp = sbi->F2FS_CKPT();
	fsck_f2fs_nm_info *nm_i = sbi->NM_I();
	...
	nm_i->bitmap_size = sbi->__bitmap_size( NAT_BITMAP);

	nm_i->nat_bitmap = (char*)malloc(nm_i->bitmap_size);
	if (!nm_i->nat_bitmap)		return -ENOMEM;
	version_bitmap = (unsigned char*)sbi->__bitmap_ptr(NAT_BITMAP);
	if (!version_bitmap)		return -EFAULT;
...
int build_node_manager(fsck_f2fs_sb_info *sbi)
{
	int err;
	sbi->nm_info = (fsck_f2fs_nm_info*)malloc(sizeof(fsck_f2fs_nm_info));
	if (!sbi->nm_info)		return -ENOMEM;
...
int build_sit_info(fsck_f2fs_sb_info *sbi)
{
	f2fs_super_block *sb = sbi->F2FS_RAW_SUPER();
	f2fs_checkpoint *cp = sbi->F2FS_CKPT();
	fsck_sit_info *sit_i;
	...
	//sit_i = (fsck_sit_info*)malloc(sizeof(fsck_sit_info));
	sit_i = new fsck_sit_info;
	if (!sit_i) {
		MSG(1, "\tError: Malloc failed for build_sit_info!\n");
		return -ENOMEM;
	}

	sbi->SM_I()->sit_info = sit_i;

//	sit_i->sentries = (fsck_seg_entry*)calloc(TOTAL_SEGS(sbi) * sizeof(fsck_seg_entry), 1);
	// segment count是包括meta data部分的segment，但不包含superblock
	sit_i->sentries = new fsck_seg_entry[TOTAL_SEGS(sbi)];
	...
	if (need_fsync_data_record(sbi))		bitmap_size += bitmap_size;

	LOG_DEBUG(L"total seg=%d, vblock map size=%d, sit bitmap size=%d", TOTAL_SEGS(sbi), SIT_VBLOCK_MAP_SIZE, bitmap_size);

	//sit_i->bitmap = (BYTE*)calloc(bitmap_size, 1);
	sit_i->bitmap = new BYTE[bitmap_size];
	if (!sit_i->bitmap) {
		MSG(1, "\tError: Calloc failed for build_sit_info!!\n");
		goto free_sentries;
	}
	...
	bitmap_size = sbi->__bitmap_size( SIT_BITMAP);
	src_bitmap = (char*) sbi->__bitmap_ptr( SIT_BITMAP);

	//dst_bitmap = (char*)malloc(bitmap_size);
	dst_bitmap = new char[bitmap_size];
...
void reset_curseg(fsck_f2fs_sb_info *sbi, int type)
{
	struct fsck_curseg_info *curseg = CURSEG_I(sbi, type);
	struct summary_footer *sum_footer;
	fsck_seg_entry *se;

	sum_footer = &(curseg->sum_blk->footer);
	memset(sum_footer, 0, sizeof(struct summary_footer));
	if (IS_DATASEG(type))		SET_SUM_TYPE(sum_footer, SUM_TYPE_DATA);
	if (IS_NODESEG(type))		SET_SUM_TYPE(sum_footer, SUM_TYPE_NODE);
	se = get_seg_entry(sbi, curseg->segno);
	se->type = type;
	se->dirty = 1;
}

static void read_compacted_summaries(fsck_f2fs_sb_info *sbi)
{
	CF2fsFileSystem* fs = sbi->GetFs();

	struct fsck_curseg_info *curseg;
	unsigned int i, j, offset;
	block_t start;
	char *kaddr;
	int ret;

	start = sbi->start_sum_block();

	kaddr = (char *)malloc(PAGE_SIZE);
	ASSERT(kaddr);

	ret = fs->dev_read_block(kaddr, start++);
	ASSERT(ret >= 0);
	...
	memcpy(&curseg->sum_blk->journal.n_sits, kaddr + SUM_JOURNAL_SIZE, SUM_JOURNAL_SIZE);
	...
	for (i = CURSEG_HOT_DATA; i <= CURSEG_COLD_DATA; i++) {
		...
		if (curseg->alloc_type == SSR)		blk_off = sbi->blocks_per_seg;
		else								blk_off = curseg->next_blkoff;
		...
		for (j = 0; j < blk_off; j++) {
			...
			offset += SUMMARY_SIZE;
			if (offset + SUMMARY_SIZE <= PAGE_CACHE_SIZE - SUM_FOOTER_SIZE)		continue;
...
static void restore_node_summary(fsck_f2fs_sb_info *sbi, unsigned int segno, struct f2fs_summary_block *sum_blk)
{
	CF2fsFileSystem* fs = sbi->GetFs();
	...
	node_blk = (f2fs_node*)malloc(F2FS_BLKSIZE);
	ASSERT(node_blk);

	/* scan the node segment */
	addr = FSCK_START_BLOCK(sbi, segno);
	sum_entry = &sum_blk->entries[0];

	for (i = 0; i < sbi->blocks_per_seg; i++, sum_entry++) {
		ret = fs->dev_read_block(node_blk, addr);
		ASSERT(ret >= 0);
...
static void read_normal_summaries(fsck_f2fs_sb_info *sbi, int type)
{
	CF2fsFileSystem* fs = sbi->GetFs();
	...
	if (IS_DATASEG(type)) {
		...
		if (is_set_ckpt_flags(cp, CP_UMOUNT_FLAG))	blk_addr = sbi->sum_blk_addr( NR_CURSEG_TYPE, type);
		else										blk_addr = sbi->sum_blk_addr( NR_CURSEG_DATA_TYPE, type);
	} else {
		segno = get_cp(cur_node_segno[type - CURSEG_HOT_NODE]);
		if (is_set_ckpt_flags(cp, CP_UMOUNT_FLAG))	blk_addr = sbi->sum_blk_addr( NR_CURSEG_NODE_TYPE, type - CURSEG_HOT_NODE);
		else										blk_addr = GET_SUM_BLKADDR(sbi, segno);
	}

	sum_blk = (struct f2fs_summary_block *)malloc(PAGE_SIZE);
	ASSERT(sum_blk);

	ret = fs->dev_read_block(sum_blk, blk_addr);
	ASSERT(ret >= 0);
...
void update_sum_entry(fsck_f2fs_sb_info *sbi, block_t blk_addr, struct f2fs_summary *sum)
{
	CF2fsFileSystem* fs = sbi->GetFs();
	...
	sum_blk->footer.entry_type = IS_NODESEG(se->type) ? SUM_TYPE_NODE :	SUM_TYPE_DATA;

	/* write SSA all the time */
	ret = fs->dev_write_block((BYTE*)sum_blk, GET_SUM_BLKADDR(sbi, segno));
	ASSERT(ret >= 0);
...
static void restore_curseg_summaries(fsck_f2fs_sb_info *sbi)
{
	int type = CURSEG_HOT_DATA;

	if (is_set_ckpt_flags(sbi->F2FS_CKPT(), CP_COMPACT_SUM_FLAG)) {
...
static int build_curseg(fsck_f2fs_sb_info *sbi)
{
	f2fs_checkpoint *cp = sbi->F2FS_CKPT();
	struct fsck_curseg_info *array;
	unsigned short blk_off;
	unsigned int segno;
	int i;

	array = (fsck_curseg_info*)malloc(sizeof(*array) * NR_CURSEG_TYPE);
	if (!array) {
		MSG(1, "\tError: Malloc failed for build_curseg!\n");
		return -ENOMEM;
	}

	sbi->SM_I()->curseg_array = array;

	for (i = 0; i < NR_CURSEG_TYPE; i++) {
		array[i].sum_blk = (f2fs_summary_block*)calloc(PAGE_CACHE_SIZE, 1);
		if (!array[i].sum_blk) {
...
seg_cleanup:
	for(--i ; i >=0; --i)	free(array[i].sum_blk);
	free(array);

	return -ENOMEM;
}

static inline void check_seg_range(fsck_f2fs_sb_info *sbi, unsigned int segno)
{
	unsigned int end_segno = sbi->SM_I()->segment_count - 1;
	ASSERT(segno <= end_segno);
}

static inline block_t current_sit_addr(fsck_f2fs_sb_info *sbi,	unsigned int segno)
{
	fsck_sit_info *sit_i = SIT_I(sbi);
...
void get_current_sit_page(fsck_f2fs_sb_info *sbi, unsigned int segno, struct f2fs_sit_block *sit_blk)
{
	CF2fsFileSystem* fs = sbi->GetFs();
	block_t blk_addr = current_sit_addr(sbi, segno);
	ASSERT(fs->dev_read_block(sit_blk, blk_addr) >= 0);
}

void rewrite_current_sit_page(fsck_f2fs_sb_info *sbi, unsigned int segno, struct f2fs_sit_block *sit_blk)
{
	CF2fsFileSystem* fs = sbi->GetFs();
	block_t blk_addr = current_sit_addr(sbi, segno);
	ASSERT(fs->dev_write_block((BYTE*)sit_blk, blk_addr) >= 0);
}

void check_block_count(fsck_f2fs_sb_info *sbi, unsigned int segno, struct f2fs_sit_entry *raw_sit)
{
	fsck_f2fs_sm_info *sm_info = sbi->SM_I();
	...
	if (GET_SIT_VBLOCKS(raw_sit) > sbi->blocks_per_seg) {
		ASSERT_MSG("Invalid SIT vblocks: segno=0x%x, %u", segno, GET_SIT_VBLOCKS(raw_sit));
	}
	...
	if (GET_SIT_VBLOCKS(raw_sit) != valid_blocks) {
		ASSERT_MSG("Wrong SIT valid blocks: segno=0x%x, %u vs. %u", segno, GET_SIT_VBLOCKS(raw_sit), valid_blocks);
	}

	if (GET_SIT_TYPE(raw_sit) >= NO_CHECK_TYPE)
		ASSERT_MSG("Wrong SIT type: segno=0x%x, %u", segno, GET_SIT_TYPE(raw_sit));
}

void __seg_info_from_raw_sit(fsck_seg_entry *se, struct f2fs_sit_entry *raw_sit)
{
...
void seg_info_from_raw_sit(fsck_f2fs_sb_info *sbi, fsck_seg_entry *se,	struct f2fs_sit_entry *raw_sit)
{
	__seg_info_from_raw_sit(se, raw_sit);

	if (!need_fsync_data_record(sbi))		return;
	se->ckpt_valid_blocks = se->valid_blocks;
	memcpy(se->ckpt_valid_map, se->cur_valid_map, SIT_VBLOCK_MAP_SIZE);
	se->ckpt_type = se->type;
}

fsck_seg_entry *get_seg_entry(fsck_f2fs_sb_info *sbi, unsigned int segno)
{
	fsck_sit_info *sit_i = SIT_I(sbi);
	return &sit_i->sentries[segno];
}

unsigned short get_seg_vblocks(fsck_f2fs_sb_info *sbi, fsck_seg_entry *se)
{
	if (!need_fsync_data_record(sbi))	return se->valid_blocks;
	else								return se->ckpt_valid_blocks;
}

unsigned char *get_seg_bitmap(fsck_f2fs_sb_info *sbi, fsck_seg_entry *se)
{
	if (!need_fsync_data_record(sbi))	return se->cur_valid_map;
	else								return se->ckpt_valid_map;
}

unsigned char get_seg_type(fsck_f2fs_sb_info *sbi, fsck_seg_entry *se)
{
	if (!need_fsync_data_record(sbi))	return se->type;
	else								return se->ckpt_type;
}

struct f2fs_summary_block *get_sum_block(fsck_f2fs_sb_info *sbi,	unsigned int segno, int *ret_type)
{
	CF2fsFileSystem* fs = sbi->GetFs();
	...
	*ret_type= SEG_TYPE_MAX;
	...
	ret = fs->dev_read_block(sum_blk, ssa_blk);
	ASSERT(ret >= 0);
...
int get_sum_entry(fsck_f2fs_sb_info *sbi, u32 blk_addr, struct f2fs_summary *sum_entry)
{
	struct f2fs_summary_block *sum_blk;
	u32 segno, offset;
	int type;
...
static void get_nat_entry(fsck_f2fs_sb_info *sbi, nid_t nid,	struct f2fs_nat_entry *raw_nat)
{
	CF2fsFileSystem* fs = sbi->GetFs();
	...
	ret = fs->dev_read_block(nat_block, block_addr);
	ASSERT(ret >= 0);
...
void update_data_blkaddr(fsck_f2fs_sb_info *sbi, nid_t nid,	u16 ofs_in_node, block_t newaddr)
{
	CF2fsFileSystem* fs = sbi->GetFs();

	f2fs_node *node_blk = NULL;
	struct node_info ni;
	block_t oldaddr, startaddr, endaddr;
	int ret;

	node_blk = (f2fs_node *)calloc(BLOCK_SZ, 1);
	ASSERT(node_blk);

	get_node_info(sbi, nid, &ni);

	/* read node_block */
	ret = fs->dev_read_block(node_blk, ni.blk_addr);
	ASSERT(ret >= 0);

	/* check its block address */
	if (node_blk->footer.nid == node_blk->footer.ino) {
		int ofs = get_extra_isize(&node_blk->i);

		oldaddr = le32_to_cpu(node_blk->i._u.i_addr[ofs + ofs_in_node]);
		node_blk->i._u.i_addr[ofs + ofs_in_node] = cpu_to_le32(newaddr);
		ret = fs->write_inode(node_blk, ni.blk_addr);
		ASSERT(ret >= 0);
	} else {
		oldaddr = le32_to_cpu(node_blk->dn.addr[ofs_in_node]);
		node_blk->dn.addr[ofs_in_node] = cpu_to_le32(newaddr);
		ret = fs->dev_write_block((BYTE*)node_blk, ni.blk_addr);
		ASSERT(ret >= 0);
	}
	...
	startaddr = le32_to_cpu(node_blk->i.i_ext.blk);
	endaddr = startaddr + le32_to_cpu(node_blk->i.i_ext.len);
	if (oldaddr >= startaddr && oldaddr < endaddr) {
		node_blk->i.i_ext.len = 0;

		/* update inode block */
		ASSERT(fs->write_inode(node_blk, ni.blk_addr) >= 0);
	}
	free(node_blk);
}

void update_nat_blkaddr(fsck_f2fs_sb_info *sbi, nid_t ino, nid_t nid, block_t newaddr)
{
	CF2fsFileSystem* fs = sbi->GetFs();
	...
	if (c.func == FSCK)
		sbi->F2FS_FSCK()->entries[nid] = nat_block->entries[entry_off];

	ret = fs->dev_write_block((BYTE*)nat_block, block_addr);
	ASSERT(ret >= 0);
	free(nat_block);
}

void get_node_info(fsck_f2fs_sb_info *sbi, nid_t nid, struct node_info *ni)
{
	struct f2fs_nat_entry raw_nat;

	ni->nid = nid;
	if (c.func == FSCK && sbi->F2FS_FSCK()->nr_nat_entries) {
		node_info_from_raw_nat(ni, &(sbi->F2FS_FSCK()->entries[nid]));
		if (ni->blk_addr)	return;
...
static int build_sit_entries(fsck_f2fs_sb_info *sbi)
{
	fsck_sit_info *sit_i = SIT_I(sbi);
	struct fsck_curseg_info *curseg = CURSEG_I(sbi, CURSEG_COLD_DATA);
	struct f2fs_journal *journal = &curseg->sum_blk->journal;
	...
	sit_blk = (f2fs_sit_block*) calloc(BLOCK_SZ, 1);
	if (!sit_blk) {
	...
	do {
		readed = f2fs_ra_meta_pages(sbi, start_blk, MAX_RA_BLOCKS, META_SIT);
	...
	if (sits_in_cursum(journal) > SIT_JOURNAL_ENTRIES) {
		MSG(0, "\tError: build_sit_entries truncate n_sits(%u) to SIT_JOURNAL_ENTRIES(%llu)\n",
...
static int early_build_segment_manager(fsck_f2fs_sb_info *sbi)
{
	f2fs_super_block *sb = sbi->F2FS_RAW_SUPER();
	f2fs_checkpoint *cp = sbi->F2FS_CKPT();
	fsck_f2fs_sm_info *sm_info;

	sm_info =(fsck_f2fs_sm_info*) malloc(sizeof(fsck_f2fs_sm_info));
	if (!sm_info) {
...
static int late_build_segment_manager(fsck_f2fs_sb_info *sbi)
{
...
void build_sit_area_bitmap(fsck_f2fs_sb_info *sbi)
{
	f2fs_fsck *fsck = sbi->F2FS_FSCK();
	fsck_f2fs_sm_info *sm_i = sbi->SM_I();
	unsigned int segno = 0;
	char *ptr = NULL;
	u32 sum_vblocks = 0;
	u32 free_segs = 0;
	...
	// <YUAN> sit area bitmap 仅记录 main area 的bitmap, 在使用中已经把block调整为main block
	fsck->sit_area_bitmap_sz = sm_i->main_segments * SIT_VBLOCK_MAP_SIZE;
//	fsck->sit_area_bitmap = (char*)calloc(1, fsck->sit_area_bitmap_sz);
	fsck->sit_area_bitmap = new char[fsck->sit_area_bitmap_sz];
	ASSERT(fsck->sit_area_bitmap);
	ptr = fsck->sit_area_bitmap;
	int ptr_size = fsck->sit_area_bitmap_sz;

	ASSERT(fsck->sit_area_bitmap_sz == fsck->main_area_bitmap_sz);
//	LOG_DEBUG(L"total seg = %d", TOTAL_SEGS(sbi) )
//	for (segno = 0; segno < TOTAL_SEGS(sbi); segno++) {
	for (segno = 0; segno < sm_i->main_segments; segno++) {
		se = get_seg_entry(sbi, segno);
		memcpy_s(ptr, ptr_size, se->cur_valid_map, SIT_VBLOCK_MAP_SIZE);
		ptr += SIT_VBLOCK_MAP_SIZE;
		ptr_size -= SIT_VBLOCK_MAP_SIZE;
		JCASSERT(ptr_size >= 0);
		...
		if (se->valid_blocks == 0x0 && is_usable_seg(sbi, segno)) {
			if (le32_to_cpu(sbi->ckpt->cur_node_segno[0]) == segno ||
				...
				LOG_DEBUG(L"detect current segment, segno=%d,", segno)
				continue;
			} 
			else {	free_segs++; }
		}
		else {
			LOG_DEBUG(L"detect valid segment, segno=%d", segno);
	...
	LOG_DEBUG(L"Blocks [0x%x : %d] Free Segs [0x%x : %d]\n\n",	sum_vblocks, sum_vblocks, free_segs, free_segs);
}

void rewrite_sit_area_bitmap(fsck_f2fs_sb_info *sbi)
{
	struct f2fs_fsck *fsck = sbi->F2FS_FSCK();
	struct fsck_curseg_info *curseg = CURSEG_I(sbi, CURSEG_COLD_DATA);
	fsck_sit_info *sit_i = SIT_I(sbi);
	struct f2fs_sit_block *sit_blk;
	unsigned int segno = 0;
	...
	for (segno = 0; segno < TOTAL_SEGS(sbi); segno++) {
		...
		if (type >= NO_CHECK_TYPE) {
			ASSERT_MSG("Invalid type and valid blocks=%x,%x", segno, valid_blocks);
			type = 0;
		}
...
static int flush_sit_journal_entries(fsck_f2fs_sb_info *sbi)
{
	struct fsck_curseg_info *curseg = CURSEG_I(sbi, CURSEG_COLD_DATA);
	struct f2fs_journal *journal = &curseg->sum_blk->journal;
	fsck_sit_info *sit_i = SIT_I(sbi);
...
static int flush_nat_journal_entries(fsck_f2fs_sb_info *sbi)
{
	CF2fsFileSystem* fs = sbi->GetFs();
	struct fsck_curseg_info *curseg = CURSEG_I(sbi, CURSEG_HOT_DATA);
	struct f2fs_journal *journal = &curseg->sum_blk->journal;
...
next:
	...
	memcpy(&nat_block->entries[entry_off], &nat_in_journal(journal, i), sizeof(struct f2fs_nat_entry));
...
void flush_journal_entries(fsck_f2fs_sb_info *sbi)
{
...
void flush_sit_entries(fsck_f2fs_sb_info *sbi)
{
	fsck_sit_info *sit_i = SIT_I(sbi);
	struct f2fs_sit_block *sit_blk;
	unsigned int segno = 0;
...
int relocate_curseg_offset(fsck_f2fs_sb_info *sbi, int type)
{
	struct fsck_curseg_info *curseg = CURSEG_I(sbi, type);
	fsck_seg_entry *se = get_seg_entry(sbi, curseg->segno);
	unsigned int i;
...
void set_section_type(fsck_f2fs_sb_info *sbi, unsigned int segno, int type)
{
	unsigned int i;
	...
	for (i = 0; i < sbi->segs_per_sec; i++) {
		fsck_seg_entry *se = get_seg_entry(sbi, segno + i);
...
static bool write_pointer_at_zone_start(fsck_f2fs_sb_info *sbi,
					unsigned int zone_segno)
{
	u_int64_t sector;
	struct blk_zone blkz;
	block_t block = FSCK_START_BLOCK(sbi, zone_segno);
...
static bool write_pointer_at_zone_start(fsck_f2fs_sb_info *UNUSED(sbi),	unsigned int UNUSED(zone_segno))
{
	return true;
}
...
int find_next_free_block(fsck_f2fs_sb_info *sbi, u64 *to, int left,	int want_type, bool new_sec)
{
	f2fs_super_block *sb = sbi->F2FS_RAW_SUPER();
	fsck_seg_entry *se;
	u32 segno;
	...
	if (*to > 0)
		*to -= left;
	if (get_free_segments(sbi) <= sbi->SM_I()->reserved_segments + 1)
		not_enough = 1;

	while (*to >= sbi->SM_I()->main_blkaddr && *to < end_blkaddr) {
...
next_segment:
			*to = left ? FSCK_START_BLOCK(sbi, segno) - 1:	FSCK_START_BLOCK(sbi, segno + 1);
			continue;
		}
		if (!(get_sb(feature) & cpu_to_le32(F2FS_FEATURE_RO)) &&
...
static void move_one_curseg_info(fsck_f2fs_sb_info *sbi, u64 from, int left, int i)
{
	CF2fsFileSystem* fs = sbi->GetFs();

	f2fs_super_block *sb = sbi->F2FS_RAW_SUPER();
	struct fsck_curseg_info *curseg = CURSEG_I(sbi, i);
	if (!is_valid(curseg->segno)) return;
	...
	if ((get_sb(feature) & cpu_to_le32(F2FS_FEATURE_RO))) {
		if (i != CURSEG_HOT_DATA && i != CURSEG_HOT_NODE)	return;
		goto bypass_ssa;
	}

	/* update original SSA too */
	ssa_blk = GET_SUM_BLKADDR(sbi, curseg->segno);
	ret = fs->dev_write_block((BYTE*) curseg->sum_blk, ssa_blk);
	ASSERT(ret >= 0);
bypass_ssa:
	to = from;
	...
	/* update new segno */
	ssa_blk = GET_SUM_BLKADDR(sbi, curseg->segno);
	ret = fs->dev_read_block(&buf, ssa_blk);
	ASSERT(ret >= 0);
	...
	FIX_MSG("Move curseg[%d] %x -> %x after %lld\n", i, old_segno, curseg->segno, from);
}

void move_curseg_info(fsck_f2fs_sb_info *sbi, u64 from, int left)
{
	int i;
...
void update_curseg_info(fsck_f2fs_sb_info *sbi, int type)
{
	if (!relocate_curseg_offset(sbi, type))	return;
	move_one_curseg_info(sbi, sbi->SM_I()->main_blkaddr, 0, type);
}

void zero_journal_entries(fsck_f2fs_sb_info *sbi)
{
	int i;

	for (i = 0; i < NO_CHECK_TYPE; i++)
	{
		fsck_curseg_info * curseg = CURSEG_I(sbi, i);
		if (!is_valid(curseg->segno)) continue;
		curseg->sum_blk->journal.n_nats = 0;
	}
}

void write_curseg_info(fsck_f2fs_sb_info *sbi)
{
	f2fs_checkpoint *cp = sbi->F2FS_CKPT();
	int i;
...
//		if (!is_valid(curseg->segno)) continue;
		...
		if (i < CURSEG_HOT_NODE) {
			set_cp(cur_data_segno[i], curseg->segno);
			set_cp(cur_data_blkoff[i], curseg->next_blkoff);
		} else {
...
int lookup_nat_in_journal(fsck_f2fs_sb_info *sbi, u32 nid, struct f2fs_nat_entry *raw_nat)
{
	struct fsck_curseg_info *curseg = CURSEG_I(sbi, CURSEG_HOT_DATA);
	struct f2fs_journal *journal = &curseg->sum_blk->journal;
	int i = 0;
	...
	for (i = 0; i < nats_in_cursum(journal); i++) {
		if (le32_to_cpu(nid_in_journal(journal, i)) == nid) {
			memcpy(raw_nat, &nat_in_journal(journal, i),sizeof(struct f2fs_nat_entry));
...
void nullify_nat_entry(fsck_f2fs_sb_info *sbi, u32 nid)
{
	CF2fsFileSystem* fs = sbi->GetFs();
	...
	if (nid == sbi->F2FS_NODE_INO() || nid == sbi->F2FS_META_INO()) {
		FIX_MSG("nid [0x%x] block_addr= 0x%x -> 0x1", nid, le32_to_cpu(nat_block->entries[entry_off].block_addr));
...
void duplicate_checkpoint(fsck_f2fs_sb_info *sbi)
{
	CF2fsFileSystem* fs = sbi->GetFs();
	...
	ret = fs->dev_read((BYTE*)buf, src << F2FS_BLKSIZE_BITS, seg_size << F2FS_BLKSIZE_BITS);
	ASSERT(ret >= 0);

	ret = fs->dev_write((BYTE*)buf, dst << F2FS_BLKSIZE_BITS, seg_size << F2FS_BLKSIZE_BITS);
	ASSERT(ret >= 0);
	...
	MSG(0, "Info: Duplicate valid checkpoint to mirror position %llu -> %llu\n", src, dst);
}

void write_checkpoint(fsck_f2fs_sb_info *sbi)
{
	CF2fsFileSystem* fs = sbi->GetFs();
	...
	*((__le32 *)((unsigned char *)cp + get_cp(checksum_offset))) =
	...
	/* write the first cp */
	ret = fs->dev_write_block((BYTE*)cp, cp_blk_no++);
	ASSERT(ret >= 0);
...
//		if (!is_valid(curseg->segno)) continue;
		...
		ret = fs->dev_write_block((BYTE*)curseg->sum_blk, cp_blk_no++);
		ASSERT(ret >= 0);
	...
	/* Write nat bits */
	if (flags & CP_NAT_BITS_FLAG)		write_nat_bits(sbi, sb, cp, sbi->cur_cp);
...
void write_checkpoints(fsck_f2fs_sb_info *sbi)
{
...
void build_nat_area_bitmap(fsck_f2fs_sb_info *sbi)
{
	CF2fsFileSystem* fs = sbi->GetFs();
...
//	fsck->nat_area_bitmap = (char*)calloc(fsck->nat_area_bitmap_sz, 1);
	...
	//fsck->entries = (f2fs_nat_entry*)calloc(sizeof(f2fs_nat_entry), fsck->nr_nat_entries);
	fsck->entries = new f2fs_nat_entry[fsck->nr_nat_entries];
	memset(fsck->entries, 0, sizeof(f2fs_nat_entry) * fsck->nr_nat_entries);
	ASSERT(fsck->entries);
	...
	for (block_off = 0; block_off < nr_nat_blks; block_off++) {
		...
		for (i = 0; i < NAT_ENTRY_PER_BLOCK; i++) {
			...
			if ((nid + i) == sbi->F2FS_NODE_INO() || (nid + i) == sbi->F2FS_META_INO()) {
				/* block_addr of node/meta inode should be 0x1. Set this bit, and fsck_verify will fix it. */
			...
			if (ni.blk_addr == 0x0)
				continue;
			if (ni.ino == 0x0) {
				ASSERT_MSG("\tError: ino[0x%8x] or blk_addr[0x%16x] is invalid\n", ni.ino, ni.blk_addr);
			}
			if (ni.ino == (nid + i)) {
			...
			if (nid + i == 0) {
				/* nat entry [0] must be null.  If it is corrupted, set its bit in nat_area_bitmap, fsck_verify will nullify it	 */
...
static int check_sector_size(CF2fsFileSystem * fs, f2fs_super_block *sb)
{
//	CF2fsFileSystem* fs = sbi->GetFs();
	...
	if (log_sectorsize == get_sb(log_sectorsize) && log_sectors_per_block == get_sb(log_sectors_per_block))
		return 0;
...
static int tune_sb_features(fsck_f2fs_sb_info *sbi)
{
	CF2fsFileSystem* fs = sbi->GetFs();
	int sb_changed = 0;
	f2fs_super_block *sb = sbi->F2FS_RAW_SUPER();

	if (!(sb->feature & cpu_to_le32(F2FS_FEATURE_ENCRYPT)) && c.feature & cpu_to_le32(F2FS_FEATURE_ENCRYPT)) {
		sb->feature |= cpu_to_le32(F2FS_FEATURE_ENCRYPT);
		MSG(0, "Info: Set Encryption feature\n");
		sb_changed = 1;
	}
	if (!(sb->feature & cpu_to_le32(F2FS_FEATURE_CASEFOLD)) && c.feature & cpu_to_le32(F2FS_FEATURE_CASEFOLD)) {
		if (!c.s_encoding) {
	...
	if (!sb_changed)	return 0;

	update_superblock(fs, sb, SB_MASK_ALL);
	return 0;
}

fsck_fsync_inode_entry *get_fsync_inode(struct list_head *head, nid_t ino)
{
	fsck_fsync_inode_entry *entry;
	list_for_each_entry(fsck_fsync_inode_entry, entry, head, list) {
		if (entry->ino == ino)		return entry;
	}
	return NULL;
}

fsck_fsync_inode_entry *add_fsync_inode(struct list_head *head, nid_t ino)
{
	fsck_fsync_inode_entry *entry;

	entry = (fsck_fsync_inode_entry*)calloc(sizeof(fsck_fsync_inode_entry), 1);
...
static void del_fsync_inode(fsck_fsync_inode_entry *entry)
{
	list_del(&entry->list);
	free(entry);
}
...
{
	fsck_fsync_inode_entry *entry, *tmp;

	list_for_each_entry_safe(fsck_fsync_inode_entry, entry, tmp, head, list) {
		del_fsync_inode(entry);
	}
}

static int find_fsync_inode(fsck_f2fs_sb_info *sbi, struct list_head *head)
{
	CF2fsFileSystem* fs = sbi->GetFs();
	...
	unsigned int free_blocks = TOTAL_SEGS(sbi) * sbi->blocks_per_seg -	sbi->total_valid_block_count;
	int err = 0;

	/* get node pages in the current segment */
	curseg = CURSEG_I(sbi, CURSEG_WARM_NODE);
	blkaddr = FSCK_NEXT_FREE_BLKADDR(sbi, curseg);

	node_blk = (f2fs_node*)calloc(F2FS_BLKSIZE, 1);
	ASSERT(node_blk);

	while (1) {
		fsck_fsync_inode_entry *entry;
		if (!f2fs_is_valid_blkaddr(sbi, blkaddr, META_POR))		break;
		err = fs->dev_read_block(node_blk, blkaddr);
		if (err)			break;

		if (!is_recoverable_dnode(sbi, node_blk))				break;
		if (!is_fsync_dnode(node_blk))							goto next;
		...
		if (IS_INODE(node_blk) && is_dent_dnode(node_blk))		entry->last_dentry = blkaddr;
next:
		/* sanity check in order to detect looped node chain */
		if (++loop_cnt >= free_blocks || blkaddr == next_blkaddr_of_node(node_blk)) {
...
static int do_record_fsync_data(fsck_f2fs_sb_info *sbi,
					f2fs_node *node_blk,
					block_t blkaddr)
{
...
out:
	MSG(1, "recover_data: ino = %u, nid = %u, recorded = %d, err = %d\n", ino_of_node(node_blk), ofs_of_node(node_blk), recorded, err);
	return err;
}

static int traverse_dnodes(fsck_f2fs_sb_info *sbi, struct list_head *inode_list)
{
	CF2fsFileSystem* fs = sbi->GetFs();
...
static int record_fsync_data(fsck_f2fs_sb_info *sbi)
{
...
int f2fs_do_mount(fsck_f2fs_sb_info *sbi)
{
	CF2fsFileSystem* fs = sbi->GetFs();
	f2fs_checkpoint *cp = NULL;
	f2fs_super_block *sb = NULL;
	int ret;
	...
	sb = sbi->F2FS_RAW_SUPER();

	ret = check_sector_size(fs, sb);
	if (ret)	return -1;
	...
	cp = sbi->F2FS_CKPT();
	if (c.func != FSCK && c.func != DUMP &&	!is_set_ckpt_flags(sbi->F2FS_CKPT(), CP_UMOUNT_FLAG)) {
	...
	if (c.quota_fix) {
		if (get_cp(ckpt_flags) & CP_QUOTA_NEED_FSCK_FLAG)	c.fix_on = 1;
	}
	if (c.layout)		return 1;

	if (tune_sb_features(sbi))		return -1;
...
void f2fs_do_umount(fsck_f2fs_sb_info *sbi)
{
	fsck_sit_info *sit_i = SIT_I(sbi);
	fsck_f2fs_sm_info *sm_i = sbi->SM_I();
	fsck_f2fs_nm_info *nm_i = sbi->NM_I();
	...
	/* free nm_info */
	if (c.func == SLOAD || c.func == FSCK)	free(nm_i->nid_bitmap);
...
#ifdef WITH_ANDROID
int f2fs_sparse_initialize_meta(fsck_f2fs_sb_info *sbi)
{
	f2fs_super_block *sb = sbi->raw_super;
...
#else
int f2fs_sparse_initialize_meta(fsck_f2fs_sb_info *sbi) { return 0; }
#endif
