...
void f2fs_alloc_nid(fsck_f2fs_sb_info *sbi, nid_t *nid)
{
...
void f2fs_release_nid(fsck_f2fs_sb_info *sbi, nid_t nid)
{
	fsck_f2fs_nm_info *nm_i = sbi->NM_I();
...
void set_data_blkaddr(fsck_dnode_of_data *dn)
{
	__le32 *addr_array;
	f2fs_node *node_blk = dn->node_blk;
 ...
 */
block_t new_node_block(fsck_f2fs_sb_info *sbi, fsck_dnode_of_data *dn, unsigned int ofs)
{
	f2fs_super_block *sb = sbi->F2FS_RAW_SUPER();
	f2fs_node *f2fs_inode;
	f2fs_node *node_blk;
 ...
 */
static int get_node_path(f2fs_node *node, long block,
...
int get_dnode_of_data(fsck_f2fs_sb_info *sbi, fsck_dnode_of_data *dn, pgoff_t index, int mode)
{
	CF2fsFileSystem* fs = sbi->GetFs();

	int offset[4];
	unsigned int noffset[4];
	f2fs_node *parent = NULL;
	nid_t nids[4];
	block_t nblk[4];
	struct node_info ni;
	int level, i;
	...
	for (i = 1; i <= level; i++) {
		if (!nids[i] && mode == ALLOC_NODE) {
			...
			dn->node_blk = (f2fs_node*)calloc(BLOCK_SZ, 1);
			ASSERT(dn->node_blk);

			ret = fs->dev_read_block(dn->node_blk, ni.blk_addr);
			ASSERT(ret >= 0);
		...
		if (mode == ALLOC_NODE){
			...
			ret = fs->dev_write_block((BYTE*)parent, nblk[i - 1]);
			ASSERT(ret >= 0);
		}
		if (i != 1)
			free(parent);
