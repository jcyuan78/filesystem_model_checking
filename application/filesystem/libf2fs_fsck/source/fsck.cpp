...
int f2fs_set_main_bitmap(fsck_f2fs_sb_info *sbi, u32 blk, int type)
{
	...
	if (se->type >= NO_CHECK_TYPE)		fix = 1;
	else if (IS_DATASEG(se->type) != IS_DATASEG(type))		fix = 1;

	/* just check data and node types */
	if (fix) {
		LOG_DEBUG(L"Wrong segment type [0x%x] %x -> %x", GET_SEGNO(sbi, blk), se->type, type);
		se->type = type;
	}
	unsigned int main_blk = BLKOFF_FROM_MAIN(sbi, blk);
	LOG_DEBUG_(1, L"Set main block, blk=%d, seg_no=%d, main_blk=%d, type=%d, fix=%d", blk, GET_SEGNO(sbi, blk), main_blk, se->type, fix);
	JCASSERT(main_blk < fsck->main_area_bitmap_sz * 8);
	return f2fs_set_bit(main_blk, fsck->main_area_bitmap);
}

static inline int f2fs_test_main_bitmap(fsck_f2fs_sb_info *sbi, u32 blk)
{
	struct f2fs_fsck *fsck = sbi->F2FS_FSCK();
	unsigned int main_blk = BLKOFF_FROM_MAIN(sbi, blk);
	JCASSERT(main_blk < fsck->main_area_bitmap_sz * 8);

	return f2fs_test_bit(main_blk, fsck->main_area_bitmap);
}

static inline int f2fs_clear_main_bitmap(fsck_f2fs_sb_info *sbi, u32 blk)
{
	struct f2fs_fsck *fsck = sbi->F2FS_FSCK();
	unsigned int main_blk = BLKOFF_FROM_MAIN(sbi, blk);
	JCASSERT(main_blk < fsck->main_area_bitmap_sz * 8);

	return f2fs_clear_bit(main_blk, fsck->main_area_bitmap);
}

static inline int f2fs_test_sit_bitmap(fsck_f2fs_sb_info *sbi, u32 blk)
{
	struct f2fs_fsck *fsck = sbi->F2FS_FSCK();
	unsigned int main_blk = BLKOFF_FROM_MAIN(sbi, blk);
	JCASSERT(main_blk < fsck->sit_area_bitmap_sz * 8);

	return f2fs_test_bit(main_blk, fsck->sit_area_bitmap);
}

int f2fs_set_sit_bitmap(fsck_f2fs_sb_info *sbi, u32 blk)
{
	struct f2fs_fsck *fsck = sbi->F2FS_FSCK();
	unsigned int main_blk = BLKOFF_FROM_MAIN(sbi, blk);
	JCASSERT(main_blk < fsck->sit_area_bitmap_sz * 8);

	return f2fs_set_bit(main_blk, fsck->sit_area_bitmap);
}

static int add_into_hard_link_list(fsck_f2fs_sb_info *sbi, u32 nid, u32 link_cnt)
{
	struct f2fs_fsck *fsck = sbi->F2FS_FSCK();
	struct hard_link_node *node = NULL, *tmp = NULL, *prev = NULL;

	node = (hard_link_node*)calloc(sizeof(struct hard_link_node), 1);
	ASSERT(node != NULL);
...
static int find_and_dec_hard_link_list(fsck_f2fs_sb_info *sbi, u32 nid)
{
	struct f2fs_fsck *fsck = sbi->F2FS_FSCK();
...
static int is_valid_ssa_node_blk(fsck_f2fs_sb_info *sbi, u32 nid, u32 blk_addr)
{
	...
	if (need_fix && f2fs_dev_is_writable(c)) {
		u64 ssa_blk;
		int ret2;

		ssa_blk = GET_SUM_BLKADDR(sbi, segno);
		ret2 = fs->dev_write_block((BYTE*)sum_blk, ssa_blk);
		ASSERT(ret2 >= 0);
	}
out:
...
static int is_valid_summary(fsck_f2fs_sb_info *sbi, struct f2fs_summary *sum, u32 blk_addr)
{
	CF2fsFileSystem* fs = sbi->GetFs();

	u16 ofs_in_node = le16_to_cpu(sum->_u._s.ofs_in_node);
	...
	if (!IS_VALID_NID(sbi, nid))					goto out;
	get_node_info(sbi, nid, &ni);
	if (!IS_VALID_BLK_ADDR(sbi, ni.blk_addr))		goto out;

	/* read node_block */
	ret = fs->dev_read_block(node_blk, ni.blk_addr);
	ASSERT(ret >= 0);

	if (le32_to_cpu(node_blk->footer.nid) != nid)	goto out;

	/* check its block address */
	if (node_blk->footer.nid == node_blk->footer.ino) {
		int ofs = get_extra_isize(&node_blk->i);
		target_blk_addr = node_blk->i._u.i_addr[ofs + ofs_in_node];
	}
	else {
...
static int is_valid_ssa_data_blk(fsck_f2fs_sb_info *sbi, u32 blk_addr, u32 parent_nid, u16 idx_in_node, u8 version)
{
	CF2fsFileSystem* fs = sbi->GetFs();
	f2fs_super_block *sb = sbi->F2FS_RAW_SUPER();
	struct f2fs_summary_block *sum_blk;
	...
	if (le32_to_cpu(sum_entry->nid) != parent_nid || sum_entry->_u._s.version != version ||
			le16_to_cpu(sum_entry->_u._s.ofs_in_node) != idx_in_node) {
		if (!c.fix_on || type < 0) {
			DBG(0, "summary_entry.nid         [0x%x]\n",		le32_to_cpu(sum_entry->nid));
			DBG(0, "summary_entry.version     [0x%x]\n",		sum_entry->_u._s.version);
			DBG(0, "summary_entry.ofs_in_node [0x%x]\n",		le16_to_cpu(sum_entry->_u._s.ofs_in_node));
			...
			FIX_MSG("Set data summary 0x%x -> [0x%x] [0x%x] [0x%x]", segno, parent_nid, version, idx_in_node);
			sum_entry->nid = cpu_to_le32(parent_nid);
			sum_entry->_u._s.version = version;
			sum_entry->_u._s.ofs_in_node = cpu_to_le16(idx_in_node);
			need_fix = 1;
		}
	}
	if (need_fix && f2fs_dev_is_writable(c)) {
...
static int __check_inode_mode(u32 nid, enum FILE_TYPE ftype, u16 mode)
{
	...
	if (!S_ISLNK(mode) && !S_ISREG(mode) && !S_ISDIR(mode) &&
	    ...
	    !S_ISSOCK(mode)) {
		ASSERT_MSG("inode [0x%x] unknown file type i_mode [0x%x]", nid, mode);
		return -1;
	}
	...
	return 0;
err:
	ASSERT_MSG("inode [0x%x] mismatch i_mode [0x%x vs. 0x%x]", nid, ftype, mode);
	return -1;
}

static int sanity_check_nid(fsck_f2fs_sb_info *sbi, u32 nid, f2fs_node *node_blk, FILE_TYPE ftype, NODE_TYPE ntype, node_info *ni)
{
	CF2fsFileSystem* fs = sbi->GetFs();
	struct f2fs_fsck *fsck = sbi->F2FS_FSCK();
	int ret;
	...
	if (ftype != F2FS_FT_ORPHAN || f2fs_test_bit(nid, fsck->nat_area_bitmap) != 0) {
		f2fs_clear_bit(nid, fsck->nat_area_bitmap);
		/* avoid reusing nid when reconnecting files */
		f2fs_set_bit(nid, sbi->NM_I()->nid_bitmap);
	} else
...
int fsck_sanity_check_nid(fsck_f2fs_sb_info *sbi, u32 nid,
			f2fs_node *node_blk,
...
static int fsck_chk_xattr_blk(fsck_f2fs_sb_info *sbi, u32 ino, u32 x_nid, u32 *blk_cnt)
{
	f2fs_node *node_blk = NULL;
	struct node_info ni;
	int ret = 0;
...
int fsck_chk_node_blk(fsck_f2fs_sb_info *sbi, f2fs_inode *inode, u32 nid, enum FILE_TYPE ftype, enum NODE_TYPE ntype,
		u32 *blk_cnt, struct child_info *child)
{
	LOG_STACK_TRACE_EX(L"call depth=%d", ++sbi->m_call_depth);
	struct node_info ni;
	f2fs_node *node_blk = (f2fs_node *)calloc(BLOCK_SZ, 1);
	ASSERT(node_blk != NULL);
...
//		quota_add_inode_usage(fsck->qctx, nid, &node_blk->i);
		...
		case TYPE_DIRECT_NODE:
			...
			fsck_chk_dnode_blk(sbi, inode, nid, ftype, node_blk, blk_cnt, child, &ni);
			break;
...
void fsck_reada_node_block(fsck_f2fs_sb_info *sbi, u32 nid)
{
	CF2fsFileSystem* fs = sbi->GetFs();
	...
	if (nid != 0 && IS_VALID_NID(sbi, nid)) {
		get_node_info(sbi, nid, &ni);
		if (IS_VALID_BLK_ADDR(sbi, ni.blk_addr))	fs->dev_reada_block(ni.blk_addr);
	}
}

void fsck_reada_all_direct_node_blocks(fsck_f2fs_sb_info *sbi, f2fs_node *node_blk)
{
	int i;
...
void fsck_chk_inode_blk(fsck_f2fs_sb_info *sbi, u32 nid, FILE_TYPE ftype, f2fs_node *node_blk,
		u32 *blk_cnt, struct node_info *ni, struct child_info *child_d)
{
	LOG_STACK_TRACE_EX(L"call depth=%d", ++sbi->m_call_depth);

	CF2fsFileSystem* fs = sbi->GetFs();
	//char str_name[F2FS_NAME_LEN + 1]; /* parent name */
	char* str_name = new char[F2FS_NAME_LEN + 1];
	memset(str_name, 0, F2FS_NAME_LEN + 1);
	memcpy_s(str_name, F2FS_NAME_LEN, node_blk->i.i_name, node_blk->i.i_namelen);
	LOG_DEBUG(L"check inode blk: nid=%d, type=%d, length of name=%d, name=%S", nid, ftype, node_blk->i.i_namelen,str_name);
	MSG(1, "check inode for %s : nid=%d, type=%d, length of name=%d\n", str_name, nid, ftype, node_blk->i.i_namelen);
	...
	if (ftype == F2FS_FT_DIR) {
		...
		memcpy(child.p_name, node_blk->i.i_name, node_blk->i.i_namelen);
	} 
	else {
		if (f2fs_test_main_bitmap(sbi, ni->blk_addr) == 0) {
			f2fs_set_main_bitmap(sbi, ni->blk_addr,	CURSEG_WARM_NODE);
			if (i_links > 1 && ftype != F2FS_FT_ORPHAN && !is_qf_ino(sbi->F2FS_RAW_SUPER(), nid)) {
	...
	if (fsck_chk_xattr_blk(sbi, nid, i_xattr_nid, blk_cnt)) {
		if (c.fix_on) {
			...
			need_fix = 1;
			FIX_MSG("Remove xattr block: 0x%x, x_nid = 0x%x", nid, i_xattr_nid);
		}
	}
	...
	if (f2fs_has_extra_isize(&node_blk->i)) {
		if (c.feature & cpu_to_le32(F2FS_FEATURE_EXTRA_ATTR)) {
			unsigned int isize = le16_to_cpu(node_blk->i._u._s.i_extra_isize);
			if (isize > 4 * DEF_ADDRS_PER_INODE) {
				ASSERT_MSG("[0x%x] wrong i_extra_isize=0x%x", nid, isize);
				if (c.fix_on) {
					FIX_MSG("ino[0x%x] recover i_extra_isize from %u to %u", nid, isize, calc_extra_isize(c));
					node_blk->i._u._s.i_extra_isize = cpu_to_le16(calc_extra_isize(c));
					need_fix = 1;
				}
			}
		} else {
			...
			if (c.fix_on) {
				FIX_MSG("ino[0x%x] remove F2FS_EXTRA_ATTR flag in i_inline:%u",	nid, node_blk->i.i_inline);
		...
		if ((c.feature & cpu_to_le32(F2FS_FEATURE_FLEXIBLE_INLINE_XATTR)) &&
			...
			unsigned int inline_size = le16_to_cpu(node_blk->i._u._s.i_inline_xattr_size);
			if (!inline_size || inline_size > MAX_INLINE_XATTR_SIZE) {
				ASSERT_MSG("[0x%x] wrong inline_xattr_size:%u", nid, inline_size);
				if (c.fix_on) {
					FIX_MSG("ino[0x%x] recover inline xattr size from %u to %u", nid, inline_size, DEFAULT_INLINE_XATTR_ADDRS);
					node_blk->i._u._s.i_inline_xattr_size =	cpu_to_le16(DEFAULT_INLINE_XATTR_ADDRS);
					need_fix = 1;
				}
			}
		}
	...
	// <YUAN>此处有矛盾：
	//	(1)如果存在extra attr, 那么table的地址要从 i_addr+ extra size开始，但是extra size存放在i_addr[0]的位置，会引起混淆
	//	(2)checksum占用i_addr[5]的位置，使用checksum不能和no extra冲突
	if (f2fs_has_extra_isize(&node_blk->i))		ofs = get_extra_isize(&node_blk->i);
	else										ofs = 0;

	if ((node_blk->i.i_flags & cpu_to_le32(F2FS_CASEFOLD_FL)) && (ftype != F2FS_FT_DIR || !(c.feature & cpu_to_le32(F2FS_FEATURE_CASEFOLD)))) {
	...
	if ((node_blk->i.i_inline & F2FS_INLINE_DATA)) {
		unsigned int inline_size = (unsigned int)MAX_INLINE_DATA(&node_blk->i);
//		if (cur_qtype != -1)			qf_szchk_type[cur_qtype] = QF_SZCHK_INLINE;
		block_t blkaddr = le32_to_cpu(node_blk->i._u.i_addr[ofs]);

		if (blkaddr != 0) {
			ASSERT_MSG("[0x%x] wrong inline reserve blkaddr:%u", nid, blkaddr);
			if (c.fix_on) {
				FIX_MSG("inline_data has wrong 0'th block = %x", blkaddr);
				node_blk->i._u.i_addr[ofs] = 0;
				node_blk->i.i_blocks = cpu_to_le64(*blk_cnt);
				need_fix = 1;
			}
		}
		if (i_size > inline_size) {
			ASSERT_MSG("[0x%x] wrong inline size:%lu", nid, (unsigned long)i_size);
			if (c.fix_on) {
				node_blk->i.i_size = cpu_to_le64(inline_size);
				FIX_MSG("inline_data has wrong i_size %lu",	(unsigned long)i_size);
				need_fix = 1;
			}
		}
		if (!(node_blk->i.i_inline & F2FS_DATA_EXIST)) {
//			size_t max_inline_data = MAX_INLINE_DATA(&node_blk->i);
			char *buf = new char[inline_size];
			memset(buf, 0, inline_size);

			if (memcmp(buf, inline_data_addr(node_blk), inline_size)) {
	...
	if ((node_blk->i.i_inline & F2FS_INLINE_DENTRY)) {
		ofs = get_extra_isize(&node_blk->i);
		...
		if (blkaddr != 0) {
			ASSERT_MSG("[0x%x] wrong inline reserve blkaddr:%u", nid, blkaddr);
			if (c.fix_on) {
				FIX_MSG("inline_dentry has wrong 0'th block = %x", blkaddr);
				node_blk->i._u.i_addr[ofs] = 0;
				node_blk->i.i_blocks = cpu_to_le64(*blk_cnt);
				need_fix = 1;
			}
	...
	//}
	LOG_DEBUG(L"check blocks, blk_nr=%d", addrs_per_inode(&node_blk->i));
	for (idx = 0; idx < addrs_per_inode(&node_blk->i); idx++, child.pgofs++) {
		block_t blkaddr = le32_to_cpu(node_blk->i._u.i_addr[ofs + idx]);
		LOG_DEBUG_(1,L"check block, index=%d, blk addr=%d", idx, blkaddr);
		/* check extent info */
		check_extent_info(&child, blkaddr, 0);
		if (blkaddr == COMPRESS_ADDR) {
			if (node_blk->i._u._s.i_compr_blocks) {
				fsck->chk.valid_blk_cnt++;
				*blk_cnt = *blk_cnt + 1;
			}
			continue;
		...
		if (blkaddr != 0) {
			ret = fsck_chk_data_blk(sbi, IS_CASEFOLDED(&node_blk->i), blkaddr, &child, (i_blocks == *blk_cnt), 
					ftype, nid, idx, ni->version, /*file_is_encrypt(&node_blk->i)*/ false);
			if (!ret) {
				*blk_cnt = *blk_cnt + 1;
				//if (cur_qtype != -1 && blkaddr != NEW_ADDR)	qf_last_blkofs[cur_qtype] = child.pgofs;
			} 
			else if (c.fix_on) {
				node_blk->i._u.i_addr[ofs + idx] = 0;
				need_fix = 1;
	...
	for (idx = 0; idx < 5; idx++) {
		...
		if (idx == 0 || idx == 1)			ntype = TYPE_DIRECT_NODE;
		else if (idx == 2 || idx == 3)		ntype = TYPE_INDIRECT_NODE;
		else if (idx == 4)					ntype = TYPE_DOUBLE_INDIRECT_NODE;
		else								ASSERT(0);

		if (i_nid == 0x0)					goto skip;

		LOG_DEBUG(L"check direct node, index=%d, nid=%d", idx, nid);
		ret = fsck_chk_node_blk(sbi, &node_blk->i, i_nid, ftype, ntype, blk_cnt, &child);
		if (!ret) {	*blk_cnt = *blk_cnt + 1;} 
		else if (ret == -EINVAL) {
			if (c.fix_on) {
...
skip:
			if (ntype == TYPE_DIRECT_NODE)			child.pgofs += ADDRS_PER_BLOCK(&node_blk->i);
			else if (ntype == TYPE_INDIRECT_NODE)	child.pgofs += ADDRS_PER_BLOCK(&node_blk->i) * NIDS_PER_BLOCK;
			else				child.pgofs += ADDRS_PER_BLOCK(&node_blk->i) * NIDS_PER_BLOCK * NIDS_PER_BLOCK;
		}
	}
...
check:
	...
	if (child.state & FSCK_UNMATCHED_EXTENT) {
		ASSERT_MSG("ino: 0x%x has wrong ext: [pgofs:%u, blk:%u, len:%u]", nid, child.ei.fofs, child.ei.blk, child.ei.len);
		if (c.fix_on)	need_fix = 1;
	}

	// 在f2fs文件系统中，没有找到修改i_blocks的地方，这里默认必须修改
	if (i_blocks != *blk_cnt) {
		LOG_WARNING(L"ino: 0x%x has i_blocks: %08lld, but has %u blocks", nid, i_blocks, *blk_cnt);
//		ASSERT_MSG("ino: 0x%x has i_blocks: %08lld, but has %u blocks",	nid, i_blocks, *blk_cnt);
//		if (c.fix_on) {
			node_blk->i.i_blocks = cpu_to_le64(*blk_cnt);
//			need_fix = 1;
//			FIX_MSG("[0x%x] i_blocks=0x%08lld -> 0x%x",	nid, i_blocks, *blk_cnt);
//		}
	}
skip_blkcnt_fix:
	en = (char*)malloc(F2FS_PRINT_NAMELEN);
	ASSERT(en);

	namelen = le32_to_cpu(node_blk->i.i_namelen);
	if (namelen > F2FS_NAME_LEN) {
		if (child_d && child_d->i_namelen <= F2FS_NAME_LEN) {
			ASSERT_MSG("ino: 0x%x has i_namelen: 0x%x, but has %d characters for name",	nid, namelen, child_d->i_namelen);
			if (c.fix_on) {
		...
		} else	namelen = F2FS_NAME_LEN;
	}
	pretty_print_filename(node_blk->i.i_name, namelen, en, /*file_enc_name(&node_blk->i)*/false);
	if (ftype == F2FS_FT_ORPHAN)
		DBG(1, "Orphan Inode: 0x%x [%s] i_blocks: %u\n\n", le32_to_cpu(node_blk->footer.ino), en, (u32)i_blocks);

	if (is_qf_ino(sbi->F2FS_RAW_SUPER(), nid))
	...
	/* old kernel initialized i_gc_failures as 0x01, in preen mode 2, let's skip repairing.	 */
	if (ftype == F2FS_FT_REG_FILE && i_gc_failures &&
		(c.preen_mode != PREEN_MODE_2 || i_gc_failures != 0x01)) {

		DBG(1, "Regular Inode: 0x%x [%s] depth: %d\n\n", le32_to_cpu(node_blk->footer.ino), en, i_gc_failures);

		if (c.fix_on) {
			node_blk->i.i_gc_failures = cpu_to_le16(0);
			need_fix = 1;
			FIX_MSG("Regular: 0x%x reset i_gc_failures from 0x%x to 0x00",	nid, i_gc_failures);
		}
	}

	free(en);

	if (ftype == F2FS_FT_SYMLINK && i_size == 0 &&	i_blocks == (i_xattr_nid ? 3 : 2)) {
		ASSERT_MSG("ino: 0x%x i_blocks: %lu with zero i_size\n", nid, (unsigned long)i_blocks);
		if (c.fix_on) {
			node_blk->i.i_size = cpu_to_le64(F2FS_BLKSIZE);
			need_fix = 1;
			FIX_MSG("Symlink: recover 0x%x with i_size=%lu", nid, (unsigned long)F2FS_BLKSIZE);
		}
	}

	if (ftype == F2FS_FT_ORPHAN && i_links) {
		ASSERT_MSG("ino: 0x%x is orphan inode, but has i_links: %u", nid, i_links);
		if (c.fix_on) {
	...
	if (need_fix && f2fs_dev_is_writable(c))		node_blk->i.i_ext.len = 0;

	if ((c.feature & cpu_to_le32(F2FS_FEATURE_INODE_CHKSUM)) &&	f2fs_has_extra_isize(&node_blk->i)) {
		__u32 provided, calculated;

		provided = le32_to_cpu(node_blk->i._u._s.i_inode_checksum);
		calculated = CF2fsFileSystem::f2fs_inode_chksum(node_blk, c.chksum_seed);

		if (provided != calculated) {
			ASSERT_MSG("ino: 0x%x chksum:0x%x, but calculated one is: 0x%x", nid, provided, calculated);
			if (c.fix_on) {
				node_blk->i._u._s.i_inode_checksum =	cpu_to_le32(calculated);
				need_fix = 1;
...
int fsck_chk_dnode_blk(fsck_f2fs_sb_info *sbi, f2fs_inode *inode, u32 nid, enum FILE_TYPE ftype, f2fs_node *node_blk,
		u32 *blk_cnt, struct child_info *child, struct node_info *ni)
{
	LOG_DEBUG(L"call depth=%d", ++sbi->m_call_depth);
	...
	for (idx = 0; idx < ADDRS_PER_BLOCK(inode); idx++, child->pgofs++) {
		...
		if (blkaddr == 0x0)
			continue;
		if (blkaddr == COMPRESS_ADDR) {
			if (inode->_u._s.i_compr_blocks) {
				sbi->F2FS_FSCK()->chk.valid_blk_cnt++;
				*blk_cnt = *blk_cnt + 1;
			}
			continue;
		}
		ret = fsck_chk_data_blk(sbi, IS_CASEFOLDED(inode), blkaddr, child, le64_to_cpu(inode->i_blocks) == *blk_cnt, ftype,
			nid, idx, ni->version, /*file_is_encrypt(inode)*/false);
		if (!ret) {
			*blk_cnt = *blk_cnt + 1;
			//if (cur_qtype != -1 && blkaddr != NEW_ADDR)	qf_last_blkofs[cur_qtype] = child->pgofs;
		} 
...
int fsck_chk_idnode_blk(fsck_f2fs_sb_info *sbi, f2fs_inode *inode, enum FILE_TYPE ftype, f2fs_node *node_blk, u32 *blk_cnt,
		struct child_info *child)
{
	LOG_DEBUG(L"call depth=%d", ++sbi->m_call_depth);
	CF2fsFileSystem* fs = sbi->GetFs();
	...
	for (i = 0; i < NIDS_PER_BLOCK; i++) {
		if (le32_to_cpu(node_blk->in.nid[i]) == 0x0)			goto skip;
		ret = fsck_chk_node_blk(sbi, inode, le32_to_cpu(node_blk->in.nid[i]), ftype, TYPE_DIRECT_NODE, blk_cnt, child);
		if (!ret)		*blk_cnt = *blk_cnt + 1;
		else if (ret == -EINVAL) {
			if (!c.fix_on)	printf("should delete in.nid[i] = 0;\n");
			else {
				node_blk->in.nid[i] = 0;
				need_fix = 1;
...
int fsck_chk_didnode_blk(fsck_f2fs_sb_info *sbi, f2fs_inode *inode, enum FILE_TYPE ftype, f2fs_node *node_blk, u32 *blk_cnt,
		struct child_info *child)
{
	LOG_DEBUG(L"call depth=%d", ++sbi->m_call_depth);
	CF2fsFileSystem* fs = sbi->GetFs();
	...
	for (i = 0; i < NIDS_PER_BLOCK; i++) {
		if (le32_to_cpu(node_blk->in.nid[i]) == 0x0)			goto skip;
		ret = fsck_chk_node_blk(sbi, inode,	le32_to_cpu(node_blk->in.nid[i]), ftype, TYPE_INDIRECT_NODE, blk_cnt, child);
		if (!ret)			*blk_cnt = *blk_cnt + 1;
		else if (ret == -EINVAL) {
			if (!c.fix_on)		printf("should delete in.nid[i] = 0;\n");
			else {
...
skip:
			child->pgofs += ADDRS_PER_BLOCK(&node_blk->i) *	NIDS_PER_BLOCK;
		}
	}
...
static int base64_encode(const u8 *src, int len, char *dst)
{
	...
	if (bits)	*cp++ = lookup_table[ac & 0x3f];
	return (int)(cp - dst);
}
...
static void print_dentry(fsck_f2fs_sb_info *sbi, __u8 *name, u8 *bitmap, struct f2fs_dir_entry *dentry, int max, int idx, int last_blk, int enc_name)
{
	struct f2fs_fsck *fsck = sbi->F2FS_FSCK();
	...
	unsigned int i;
	int bit_offset;
	char new_name[F2FS_PRINT_NAMELEN];
	...
	bit_offset = (int)find_next_bit_le(bitmap, max, next_idx);
	...
	if (tree_mark_size <= depth) {
		...
		tree_mark = (char*)realloc(tree_mark, tree_mark_size);
		ASSERT(tree_mark != NULL);
	}

	if (last_de)	tree_mark[depth] = '`';
	else			tree_mark[depth] = '|';

	if (tree_mark[depth - 1] == '`')
		tree_mark[depth - 1] = ' ';

	pretty_print_filename(name, name_len, new_name, enc_name);
	...
	if (c.show_file_map) {
		...
		while (d) {
			if (d->depth > 1)		printf("/%s", d->name);
			d = d->next;
		}
		printf("/%s", new_name);
		if (dump_node(sbi, le32_to_cpu(dentry[idx].ino), 0)) printf("\33[2K\r");
	}
	else {
		for (i = 1; i < depth; i++)		printf("%c   ", tree_mark[i]);
		printf("%c-- %s <ino = 0x%x>, <encrypted (%d)>\n", last_de ? '`' : '|',	new_name, le32_to_cpu(dentry[idx].ino),
			enc_name);
	}
}
...
static int f2fs_check_hash_code(int encoding, int casefolded, struct f2fs_dir_entry *dentry, const unsigned char *name, u32 len, int enc_name)
{
	...
	if (dentry->hash_code != hash_code) {
		...
		pretty_print_filename(name, len, new_name, enc_name);
		FIX_MSG("Mismatch hash_code for \"%s\" [%x:%x]", new_name, le32_to_cpu(dentry->hash_code), hash_code);
...
static int f2fs_check_dirent_position(const struct f2fs_dir_entry *dentry, const char *printable_name, u32 pgofs, u8 dir_level, u32 pino)
{
	...
	bidx = dir_block_index(level, dir_level, le32_to_cpu(dentry->hash_code) % nbucket);
	end_block = bidx + nblock;

	if (pgofs >= bidx && pgofs < end_block)		return 0;

	ASSERT_MSG("Wrong position of dirent pino:%u, name:%s, level:%d, dir_level:%d, pgofs:%u, correct range:[%u, %u]\n",
		pino, printable_name, level, dir_level, pgofs, bidx, end_block - 1);
	return 1;
}

static int __chk_dots_dentries(fsck_f2fs_sb_info *sbi, int casefolded, f2fs_dir_entry *dentry,
			       struct child_info *child, u8 *name, int len, __u8 (*filename)[F2FS_SLOT_LEN], int enc_name)
{
	int fixed = 0;
	...
	if (name[0] == '.' && name[1] == '.' && len == 2) {
		if (child->p_ino == sbi->F2FS_ROOT_INO()) {
			if (le32_to_cpu(dentry->ino) != sbi->F2FS_ROOT_INO()) {
				ASSERT_MSG("Bad inode number[0x%x] for '..'\n", le32_to_cpu(dentry->ino));
				dentry->ino = cpu_to_le32(sbi->F2FS_ROOT_INO());
				fixed = 1;
			}
...
static int __chk_dentries(fsck_f2fs_sb_info *sbi, int casefolded, child_info *child, u8 *bitmap, f2fs_dir_entry *dentry, 
			__u8 (*filenames)[F2FS_SLOT_LEN], int max, int last_blk, int enc_name)
{
	LOG_STACK_TRACE_EX(L"call depth=%d, entry_nr=%d", ++sbi->m_call_depth, max);
	...
	for (i = 0; i < max; i++) {
		u32 ino;
		if (test_bit_le(i, bitmap) == 0)		continue;
		ino = le32_to_cpu(dentry[i].ino);
		if (IS_VALID_NID(sbi, ino)) {
			struct node_info ni;
			get_node_info(sbi, ino, &ni);
			if (IS_VALID_BLK_ADDR(sbi, ni.blk_addr)) {
				fs->dev_reada_block(ni.blk_addr);
				name_len = le16_to_cpu(dentry[i].name_len);
				if (name_len > 0)	i += (name_len + F2FS_SLOT_LEN - 1) / F2FS_SLOT_LEN - 1;
				// for debug
//				memcpy_s(en, F2FS_PRINT_NAMELEN, filenames[i], name_len);
//				en[name_len] = 0;
//				LOG_DEBUG(L"inode in dentry, index=%d, nid=%d, blk_addr=%d, fn=%s", i, ino, ni.blk_addr, en);
				// for debug end
			}
		}
	}
	...
	for (i = 0; i < max;) {
		...
		name_len = le16_to_cpu(dentry[i].name_len);
		ftype = (FILE_TYPE)dentry[i].file_type;
		name = new u8[name_len + 1];
		ASSERT(name);
		memset(name, 0, name_len + 1);
		if (name_len > 0 )		memcpy_s(name, name_len+1, filenames[i], name_len);
		LOG_DEBUG(L"check file, index=%d, fn=%S, inode=%d, name len=%d, type=%d", i, name, dentry[i].ino, name_len, ftype);

		if ((ftype <= F2FS_FT_UNKNOWN || ftype > F2FS_FT_LAST_FILE_TYPE)) {
			LOG_ERROR(L"Bad dentry 0x%x with unexpected ftype 0x%x", le32_to_cpu(dentry[i].ino), ftype);
...
//		name = (u8*)calloc(name_len + 1, 1);
		slots = (name_len + F2FS_SLOT_LEN - 1) / F2FS_SLOT_LEN;

		/* Becareful. 'dentry.file_type' is not imode. */
		if (ftype == F2FS_FT_DIR) {
			if ((name[0] == '.' && name_len == 1) || (name[0] == '.' && name[1] == '.' && name_len == 2)) {
				ret = __chk_dots_dentries(sbi, casefolded, &dentry[i], child, name, name_len, &filenames[i], enc_name);
				switch (ret) {
				case 1:
					fixed = 1;
				case 0:
...
//				free(name);
		...
		if (max == NR_DENTRY_IN_BLOCK) {
			ret = f2fs_check_dirent_position(dentry + i, en, child->pgofs, child->dir_level, child->p_ino);
			if (ret) {
				if (c.fix_on) {
					FIX_MSG("Clear bad dentry 0x%x", i);
					test_and_clear_bit_le(i, bitmap);
...
//				free(name);
		...
		if (ret && c.fix_on) {
			...
			FIX_MSG("Unlink [0x%x] - %s len[0x%x], type[0x%x]", le32_to_cpu(dentry[i].ino), en, name_len, dentry[i].file_type);
			fixed = 1;
		} 
		else if (ret == 0) {
			if (ftype == F2FS_FT_DIR)		child->links++;
			dentries++;
			child->files++;
		}
...
int fsck_chk_inline_dentries(fsck_f2fs_sb_info *sbi, f2fs_node *node_blk, struct child_info *child)
{
	LOG_STACK_TRACE_EX(L"call depth=%d", ++sbi->m_call_depth);

	struct f2fs_fsck *fsck = sbi->F2FS_FSCK();
	struct f2fs_dentry *cur_dentry = fsck->dentry_end;	// 一个链表结构，表示目前读取的，从root到到当前目录的路径
	...
	fsck->dentry_depth++;
	//new_dentry = (f2fs_dentry*) calloc(sizeof(struct f2fs_dentry), 1);
	new_dentry = new f2fs_dentry;
	ASSERT(new_dentry != NULL);

	new_dentry->depth = fsck->dentry_depth;
	//memcpy(new_dentry->name, child->p_name, F2FS_NAME_LEN);
	memcpy_s(new_dentry->name, F2FS_NAME_LEN + 1, child->p_name, F2FS_NAME_LEN);
	cur_dentry->next = new_dentry;
	fsck->dentry_end = new_dentry;

	dentries = __chk_dentries(sbi, IS_CASEFOLDED(&node_blk->i), child, (u8*)d.bitmap, d.dentry, d.filename, d.max, 1,
			/*file_is_encrypt(&node_blk->i)*/ false);// pass through
	if (dentries < 0) {	DBG(1, "[%3d] Inline Dentry Block Fixed hash_codes\n\n", fsck->dentry_depth);	} 
	else { DBG(1, "[%3d] Inline Dentry Block Done : dentries:%d in %lld slots (len:%d)\n\n", fsck->dentry_depth, dentries,
			d.max, F2FS_NAME_LEN);
	}
...
int fsck_chk_dentry_blk(fsck_f2fs_sb_info *sbi, int casefolded, u32 blk_addr, child_info *child, int last_blk, int enc_name)
{
	LOG_STACK_TRACE(L"call depth=%d", ++sbi->m_call_depth);
	...
	ret = fs->dev_read_block(de_blk, blk_addr);
	ASSERT(ret >= 0);
	...
	dentries = __chk_dentries(sbi, casefolded, child, de_blk->dentry_bitmap, de_blk->dentry, de_blk->filename,
			NR_DENTRY_IN_BLOCK, last_blk, enc_name);

	if (dentries < 0 && f2fs_dev_is_writable(c)) {
		ret = fs->dev_write_block((BYTE*)de_blk, blk_addr);
		ASSERT(ret >= 0);
...
int fsck_chk_data_blk(fsck_f2fs_sb_info *sbi, int casefolded, u32 blk_addr, struct child_info *child, int last_blk,
		enum FILE_TYPE ftype, u32 parent_nid, u16 idx_in_node, u8 ver, int enc_name)
{
	...
	struct f2fs_fsck *fsck = sbi->F2FS_FSCK();
	LOG_DEBUG(L"check data block, nid=%d, index=%d, type=%d, blk addr=%d", parent_nid, idx_in_node, ftype, blk_addr);
	...
	{
		LOG_DEBUG(L"[ASSERT] Duplicated data [%d]. pnid[%d] idx[%d]", blk_addr, parent_nid, idx_in_node);
		ASSERT_MSG("Duplicated data [0x%x]. pnid[0x%x] idx[0x%x]", blk_addr, parent_nid, idx_in_node);
	}
	...
	if (ftype == F2FS_FT_DIR) {
		...
		return fsck_chk_dentry_blk(sbi, casefolded, blk_addr, child, last_blk, enc_name);
	} 
	else {
...
int fsck_chk_orphan_node(fsck_f2fs_sb_info *sbi)
{
	CF2fsFileSystem* fs = sbi->GetFs();
	u32 blk_cnt = 0;
	...
	if (!is_set_ckpt_flags(sbi->F2FS_CKPT(), CP_ORPHAN_PRESENT_FLAG))
		return 0;
	...
	orphan_blk = (f2fs_orphan_block*)calloc(BLOCK_SZ, 1);
	ASSERT(orphan_blk);

	new_blk = (f2fs_orphan_block*)calloc(BLOCK_SZ, 1);
	ASSERT(new_blk);

	for (i = 0; i < orphan_blkaddr; i++) {
		int ret = fs->dev_read_block(orphan_blk, start_blk + i);
		...
		for (j = 0; j < entry_count; j++) {
			...
			ret = fsck_chk_node_blk(sbi, NULL, ino,	F2FS_FT_ORPHAN, TYPE_INODE, &blk_cnt, NULL);
			if (!ret)	new_blk->ino[new_entry_count++] = orphan_blk->ino[j];
			else if (ret && c.fix_on)	FIX_MSG("[0x%x] remove from orphan list", ino);
			else if (ret)				ASSERT_MSG("[0x%x] wrong orphan inode", ino);
		}
		if (f2fs_dev_is_writable(c) && c.fix_on && entry_count != new_entry_count) {
			new_blk->entry_count = cpu_to_le32(new_entry_count);
			ret = fs->dev_write_block((BYTE*)new_blk, start_blk + i);
			ASSERT(ret >= 0);
		}
...
#if 0
int fsck_chk_quota_node(fsck_f2fs_sb_info *sbi)
{
	f2fs_super_block *sb = sbi->F2FS_RAW_SUPER();
...
int fsck_chk_quota_files(fsck_f2fs_sb_info *sbi)
{
	struct f2fs_fsck *fsck = sbi->F2FS_FSCK();
	f2fs_super_block *sb = sbi->F2FS_RAW_SUPER();
	enum quota_type qtype;
...
int fsck_chk_meta(fsck_f2fs_sb_info *sbi)
{
	...
	for (i = 0; i < TOTAL_SEGS(sbi); i++) {
		...
		if (se->valid_blocks != 0)			sit_valid_segs++;
		else if (IS_CUR_SEGNO(sbi, i)) {
			/* curseg has not been written back to device */
			MSG(1, "\tInfo: curseg %u is counted in valid segs\n", i);
			sit_valid_segs++;
		}
		if (IS_NODESEG(se->type))			sit_node_blks += se->valid_blocks;
	}
	if (fsck->chk.sit_free_segs + sit_valid_segs !=	get_usable_seg_count(sbi)) {
		ASSERT_MSG("SIT usage does not match: sit_free_segs %u, sit_valid_segs %u, total_segs %u",
			fsck->chk.sit_free_segs, sit_valid_segs, get_usable_seg_count(sbi));
		return -EINVAL;
	}

	/* 2. check node count */
	if (fsck->chk.valid_nat_entry_cnt != sit_node_blks) {
		ASSERT_MSG("node count does not match: valid_nat_entry_cnt %u, sit_node_blks %u",
	...
	if (fsck->chk.sit_free_segs != le32_to_cpu(cp->free_segment_count)) {
		ASSERT_MSG("free segs does not match: sit_free_segs %u, free_segment_count %u",
				fsck->chk.sit_free_segs, le32_to_cpu(cp->free_segment_count));
//		return -EINVAL;		// TODO: ignor free segment mismatch
	}

	/* 4. check NAT with CP */
	if (fsck->chk.valid_nat_entry_cnt != le32_to_cpu(cp->valid_node_count)) {
		ASSERT_MSG("valid node does not match: valid_nat_entry_cnt %u, valid_node_count %u",
				fsck->chk.valid_nat_entry_cnt, le32_to_cpu(cp->valid_node_count));
		return -EINVAL;
	}

	/* 4. check orphan inode simply */
	if (fsck_chk_orphan_node(sbi))		return -EINVAL;
	...
	for (i = 0; i < fsck->nr_nat_entries; i++) {
		...
		if (!blk)
			/* skip entry whose ino is 0, otherwise, we will get a negative number by BLKOFF_FROM_MAIN(sbi, blk) */
			continue;
		...
		if (!f2fs_test_sit_bitmap(sbi, blk)) {
			MSG(0, "\tError: nat entry[ino %u block_addr 0x%x] not find it in sit_area_bitmap\n", ino, blk);
			return -EINVAL;
		}

		if (!IS_VALID_NID(sbi, ino)) {
			MSG(0, "\tError: nat_entry->ino %u exceeds the range of nat entries %u\n", ino, fsck->nr_nat_entries);
			return -EINVAL;
		}

		if (!f2fs_test_bit(ino, fsck->nat_area_bitmap)) {
			MSG(0, "\tError: nat_entry->ino %u is not set in nat_area_bitmap\n", ino);
			return -EINVAL;
		}
	}

	/* 6. check quota inode simply */
//	if (fsck_chk_quota_node(sbi))		return -EINVAL;

	if (fsck->nat_valid_inode_cnt != le32_to_cpu(cp->valid_inode_count)) {
		ASSERT_MSG("valid inode does not match: nat_valid_inode_cnt %u, valid_inode_count %u",
				fsck->nat_valid_inode_cnt, le32_to_cpu(cp->valid_inode_count));
		return -EINVAL;
	}
...
void fsck_init(fsck_f2fs_sb_info *sbi)
{
	struct f2fs_fsck *fsck = sbi->F2FS_FSCK();
	...
	LOG_DEBUG(L"main blk number=%d, main area bitmap size=%d", fsck->nr_main_blks, fsck->main_area_bitmap_sz);
//	fsck->main_area_bitmap = (char*)calloc(fsck->main_area_bitmap_sz, 1);
	...
	ASSERT(tree_mark_size != 0);
	tree_mark = (char*)calloc(tree_mark_size, 1);
	ASSERT(tree_mark != NULL);
	fsck->dentry = (f2fs_dentry*)calloc(sizeof(struct f2fs_dentry), 1);
	ASSERT(fsck->dentry != NULL);
	memcpy(fsck->dentry->name, "/", 1);
	fsck->dentry_end = fsck->dentry;
}

static void fix_hard_links(fsck_f2fs_sb_info *sbi)
{
	CF2fsFileSystem* fs = sbi->GetFs();
	struct f2fs_fsck *fsck = sbi->F2FS_FSCK();
...
static void fix_nat_entries(fsck_f2fs_sb_info *sbi)
{
	struct f2fs_fsck *fsck = sbi->F2FS_FSCK();
	u32 i;
...
static void flush_curseg_sit_entries(fsck_f2fs_sb_info *sbi)
{
	...
	ASSERT(sit_blk);
	/* update curseg sit entries, since we may change a segment type in move_curseg_info */
...
static void fix_checksum(fsck_f2fs_sb_info *sbi)
{
	f2fs_checkpoint *cp = sbi->F2FS_CKPT();
	...
	memcpy((BYTE*)bitmap_offset + nm_i->bitmap_size, sit_i->sit_bitmap, sit_i->bitmap_size);
}

static void fix_checkpoint(fsck_f2fs_sb_info *sbi)
{
	CF2fsFileSystem* fs = sbi->GetFs();
	...
	*((__le32 *)((unsigned char *)cp + get_cp(checksum_offset))) =
	...
	for (i = 0; i < get_sb(cp_payload); i++) {
		ret = fs->dev_write_block((BYTE*)((unsigned char *)cp) +
	...
	for (i = 0; i < NO_CHECK_TYPE; i++) {
		struct fsck_curseg_info *curseg = CURSEG_I(sbi, i);
		if (!is_valid(curseg->segno)) continue;
		if (!(flags & CP_UMOUNT_FLAG) && IS_NODESEG(i))		continue;
...
static void fix_checkpoints(fsck_f2fs_sb_info *sbi)
{
 ...
 */
static int last_vblk_off_in_zone(fsck_f2fs_sb_info *sbi,
...
static int check_curseg_write_pointer(fsck_f2fs_sb_info *sbi, int type)
{
	...
	cs_block = FSCK_START_BLOCK(sbi, curseg->segno) + curseg->next_blkoff;
	...
	zone_last_vblock = FSCK_START_BLOCK(sbi, zone_segno) +
...
static int check_curseg_write_pointer(fsck_f2fs_sb_info *UNUSED(sbi), int UNUSED(type))
{
	return 0;
}
...
int check_curseg_offset(fsck_f2fs_sb_info *sbi, int type)
{
	f2fs_super_block *sb = sbi->F2FS_RAW_SUPER();
	struct fsck_curseg_info *curseg = CURSEG_I(sbi, type);
	fsck_seg_entry *se;
	...
	if (get_sb(feature) & cpu_to_le32(F2FS_FEATURE_RO) && type != CURSEG_HOT_DATA && type != CURSEG_HOT_NODE)
		return 0;

	if ((curseg->next_blkoff >> 3) >= SIT_VBLOCK_MAP_SIZE) {
		ASSERT_MSG("Next block offset:%u is invalid, type:%d",	curseg->next_blkoff, type);
		return -EINVAL;
	}
	// 对于 inmem current seg，没有分配的话，segno=-1，无法获取se。
//	if (!IS_VALID_BLK_ADDR(sbi, curseg->segno)) return 0;
	if (!is_valid(curseg->segno)) return 0;

//	if (curseg->segno == (unsigned int)-1) return 0;
	...
	if (curseg->alloc_type == SSR)	return 0;

	nblocks = sbi->blocks_per_seg;
	for (j = curseg->next_blkoff + 1; j < nblocks; j++) {
		if (f2fs_test_bit(j, (const char *)se->cur_valid_map)) {
			ASSERT_MSG("For LFS curseg, space after .next_blkoff should be unused, type:%d", type);
			return -EINVAL;
		}
	}

	if (c.zoned_model == F2FS_ZONED_HM)	return check_curseg_write_pointer(sbi, type);
	return 0;
}

int check_curseg_offsets(fsck_f2fs_sb_info *sbi)
{
	int i, ret;
...
static void fix_curseg_info(fsck_f2fs_sb_info *sbi)
{
	int i, need_update = 0;
...
int check_sit_types(fsck_f2fs_sb_info *sbi)
{
	unsigned int i;
	int err = 0;
	...
	for (i = 0; i < TOTAL_SEGS(sbi); i++) {
		...
		if (se->orig_type != se->type) {
			if (se->orig_type == CURSEG_COLD_DATA &&
				...
				FIX_MSG("Wrong segment type [0x%x] %x -> %x", i, se->orig_type, se->type);
				err = -EINVAL;
			}
		}
	}
...
static f2fs_node *fsck_get_lpf(fsck_f2fs_sb_info *sbi)
{
	CF2fsFileSystem* fs = sbi->GetFs();
	f2fs_node *node;
	struct node_info ni;
	...
	node = (f2fs_node*)calloc(F2FS_BLKSIZE, 1);
	ASSERT(node);
	get_node_info(sbi, sbi->F2FS_ROOT_INO(), &ni);
	err = fs->dev_read_block(node, ni.blk_addr);
	ASSERT(err >= 0);
	...
	if (lpf_ino) { /* found */
		...
		de.name = (u8 *) LPF;
		de.len = (int)strlen(LPF);
		de.mode = 0x41c0;
		de.pino = sbi->F2FS_ROOT_INO(),
		de.file_type = F2FS_FT_DIR,
		//de.uid = getuid();
		//de.gid = getgid();
		de.mtime = (u32)time(NULL);
...
static int fsck_do_reconnect_file(fsck_f2fs_sb_info *sbi, f2fs_node *lpf, f2fs_node *fnode)
{
	CF2fsFileSystem* fs = sbi->GetFs();
	char name[80];
	size_t namelen;
	...
	ret = fs->dev_write_block((BYTE*)fnode, ni.blk_addr);
	ASSERT(ret >= 0);
...
static void fsck_failed_reconnect_file_dnode(fsck_f2fs_sb_info *sbi, nid_t nid)
{
	CF2fsFileSystem* fs = sbi->GetFs();
	struct f2fs_fsck *fsck = sbi->F2FS_FSCK();
	f2fs_node *node;
...
static void fsck_failed_reconnect_file_idnode(fsck_f2fs_sb_info *sbi, nid_t nid)
{
	CF2fsFileSystem* fs = sbi->GetFs();
	struct f2fs_fsck *fsck = sbi->F2FS_FSCK();
	f2fs_node *node;
...
static void fsck_failed_reconnect_file_didnode(fsck_f2fs_sb_info *sbi, nid_t nid)
{
	CF2fsFileSystem* fs = sbi->GetFs();
	struct f2fs_fsck *fsck = sbi->F2FS_FSCK();
	f2fs_node *node;
...
/* Counters and main_area_bitmap are already changed during checking inode block, so clear them. There is no need to clear new blocks allocted to lost+found. */
static void fsck_failed_reconnect_file(fsck_f2fs_sb_info *sbi, nid_t ino)
{
	CF2fsFileSystem* fs = sbi->GetFs();
	struct f2fs_fsck *fsck = sbi->F2FS_FSCK();
	f2fs_node *node;
	...
	if(!(node->i.i_inline & F2FS_INLINE_DATA)) {
		ofs = get_extra_isize(&node->i);
		for (i = 0; i < ADDRS_PER_INODE(&node->i); i++) {
			block_t addr = le32_to_cpu(node->i._u.i_addr[ofs + i]);
			if (!addr)				continue;
			fsck->chk.valid_blk_cnt--;
			if (addr == NEW_ADDR)	continue;
			f2fs_clear_main_bitmap(sbi, addr);
		}
	}
 ...
 * Since all unreachable nodes are already checked, we can allocate new blocks safely.
 * This function returns the number of files been reconnected. */
static int fsck_reconnect_file(fsck_f2fs_sb_info *sbi)
{
	CF2fsFileSystem* fs = sbi->GetFs();
	struct f2fs_fsck *fsck = sbi->F2FS_FSCK();
	f2fs_node *lpf_node, *node;
	struct node_info ni;
	...
	reconnect_bitmap = (char*)calloc(fsck->nat_area_bitmap_sz, 1);
	...
	for (nid = 0; nid < fsck->nr_nat_entries; nid++) {
		if (f2fs_test_bit(nid, fsck->nat_area_bitmap)) {
			...
			if (sanity_check_nid(sbi, nid, node, (FILE_TYPE)ftype, TYPE_INODE, &ni)) {
			...
			blk_cnt = 1;
			fsck_chk_inode_blk(sbi, nid, (FILE_TYPE)ftype, node, &blk_cnt, &ni, NULL);
...
static int chk_and_fix_wp_with_sit(int i, void *blkzone, void *opaque)
{
	...
	wp_blkoff = wp_block - FSCK_START_BLOCK(sbi, wp_segno);

	/* if a curseg points to the zone, skip the check */
	for (cs_index = 0; cs_index < NO_CHECK_TYPE; cs_index++) {
		cs = &sbi->SM_I()->curseg_array[cs_index];
	...
	/* When there is no valid block in the zone, check write pointer is at zone start. If not, reset the write pointer. */
	if (last_valid_blkoff < 0 &&
		...
		FIX_MSG("Reset write pointer of zone at segment 0x%x", zone_segno);
...
static void fix_wp_sit_alignment(fsck_f2fs_sb_info *sbi)
{
	unsigned int i;
...
static void fix_wp_sit_alignment(fsck_f2fs_sb_info *UNUSED(sbi))
{
	return;
}
...
/* Check and fix consistency with write pointers at the beginning of fsck so that following writes by fsck do not fail. */
void fsck_chk_and_fix_write_pointers(fsck_f2fs_sb_info *sbi)
{
	struct f2fs_fsck *fsck = sbi->F2FS_FSCK();
...
int fsck_chk_curseg_info(fsck_f2fs_sb_info *sbi)
{
	f2fs_super_block *sb = sbi->F2FS_RAW_SUPER();
	...
	for (i = 0; i < NO_CHECK_TYPE; i++) {
	// >= NR_PERSISNTENT_LOG 的cur_seg仅用于inmem，segno可能是0xFFFFFFFF
//	for (i = 0; i < NR_PERSISTENT_LOG; i++) {
		curseg = CURSEG_I(sbi, i);
//		if (!IS_VALID_BLK_ADDR(sbi, curseg->segno)) continue;
//		if (curseg->segno == (unsigned int)-1) continue;
		if (!is_valid(curseg->segno)) continue;

		se = get_seg_entry(sbi, curseg->segno);
		sum_blk = curseg->sum_blk;

		if ((get_sb(feature) & cpu_to_le32(F2FS_FEATURE_RO)) &&	(i != CURSEG_HOT_DATA && i != CURSEG_HOT_NODE))
			continue;

		if (se->type != i) {
			ASSERT_MSG("Incorrect curseg [%d]: segno [0x%x] type(SIT) [%d]", i, curseg->segno, se->type);
			if (c.fix_on || c.preen_mode)		se->type = i;
			ret = -1;
		}
		if (i <= CURSEG_COLD_DATA && IS_SUM_DATA_SEG(sum_blk->footer))		{	continue;	} 
		else if (i > CURSEG_COLD_DATA && IS_SUM_NODE_SEG(sum_blk->footer))	{	continue;} 
		else {
			ASSERT_MSG("Incorrect curseg [%d]: segno [0x%x] type(SSA) [%d]", i, curseg->segno, sum_blk->footer.entry_type);
			if (c.fix_on || c.preen_mode)
				sum_blk->footer.entry_type = i <= CURSEG_COLD_DATA ? SUM_TYPE_DATA : SUM_TYPE_NODE;
			ret = -1;
		}
	}
	return ret;
...
int fsck_verify(fsck_f2fs_sb_info *sbi)
{
	unsigned int i = 0;
	int ret = 0;
	int force = 0;
	...
	if (c.show_file_map)	return 0;

	printf("\n");

	if (c.zoned_model == F2FS_ZONED_HM) {
		printf("[FSCK] Write pointers consistency                    ");
		if (fsck->chk.wp_inconsistent_zones == 0x0) {	printf(" [Ok..]\n");} 
		else {
			printf(" [Fail] [0x%x]\n", fsck->chk.wp_inconsistent_zones);
			c.bug_on = 1;
		}

		if (fsck->chk.wp_fixed && c.fix_on)			force = 1;
	}

	if (c.feature & cpu_to_le32(F2FS_FEATURE_LOST_FOUND)) {
		for (i = 0; i < fsck->nr_nat_entries; i++)
			if (f2fs_test_bit(i, fsck->nat_area_bitmap) != 0)			break;
	...
	printf("[FSCK] Max image size: %lld MB, Free space: %u MB\n",
		c.max_size >> 20, (sbi->user_block_count - sbi->total_valid_block_count) >> (20 - F2FS_BLKSIZE_BITS));
	...
	LOG_DEBUG(L"SIT valid block bitmap checking, sit size=%d, main size=%d", fsck->sit_area_bitmap_sz, fsck->main_area_bitmap_sz);
	...
	if (le32_to_cpu(sbi->F2FS_CKPT()->free_segment_count) == fsck->chk.sit_free_segs) {
...
//		c.bug_on = 1;
	...
	if (nr_unref_nid && !c.ro) {
		...
		if (!strcmp(ans, "y")) {
			for (i = 0; i < fsck->nr_nat_entries; i++) {
				if (f2fs_test_bit(i, fsck->nat_area_bitmap)) dump_node(sbi, i, 1);
			}
		}
	}
#endif
	...
	/* fix global metadata */
	if (force || (c.fix_on && f2fs_dev_is_writable(c))) {
		f2fs_checkpoint *cp = sbi->F2FS_CKPT();
		...
		if (force || c.bug_on || c.bug_nat_bits) {
			...
			fix_checksum(sbi);
			fix_checkpoints(sbi);
		} else if (is_set_ckpt_flags(cp, CP_FSCK_FLAG) || is_set_ckpt_flags(cp, CP_QUOTA_NEED_FSCK_FLAG)) {
...
void fsck_free(fsck_f2fs_sb_info *sbi)
{
	struct f2fs_fsck *fsck = sbi->F2FS_FSCK();
	//if (fsck->qctx)				quota_release_context(&fsck->qctx);
	LOG_DEBUG(L"free main_area_bitmap");
//	if (fsck->main_area_bitmap)		free(fsck->main_area_bitmap);
	delete[] fsck->main_area_bitmap;
	LOG_DEBUG(L"free nat_area_bitmap");
//	if (fsck->nat_area_bitmap)		free(fsck->nat_area_bitmap);
	delete[] fsck->nat_area_bitmap;
	LOG_DEBUG(L"free sit_area_bitmap");
//	if (fsck->sit_area_bitmap)		free(fsck->sit_area_bitmap);
	delete[] fsck->sit_area_bitmap;
	LOG_DEBUG(L"free entries");
	//if (fsck->entries)				free(fsck->entries);
	delete[] fsck->entries;
	LOG_DEBUG(L"free tree_mark");
	if (tree_mark)					free(tree_mark);
	while (fsck->dentry) {
		struct f2fs_dentry *dentry = fsck->dentry;
		fsck->dentry = fsck->dentry->next;
		LOG_DEBUG(L"free dentry, name=%S", dentry->name);
		free(dentry);
	}
}
