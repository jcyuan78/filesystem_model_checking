...
next:
	zero_start = (int)find_next_zero_bit_le(bitmap, max_slots, bit_start);
	if (zero_start >= max_slots)
		return max_slots;

	zero_end = (int)find_next_bit_le(bitmap, max_slots, zero_start + 1);
...
void make_dentry_ptr(struct f2fs_dentry_ptr *d, f2fs_node *node_blk, void *src, int type)
{
	if (type == 1) {
		...
		int item_size = (SIZE_OF_DIR_ENTRY + F2FS_SLOT_LEN);
		LOG_DEBUG(L"max inline data=%lld, item size=%d", max_inline_data, item_size);
		int entry_cnt = (int)(max_inline_data * BITS_PER_BYTE / ( item_size * BITS_PER_BYTE + 1));
			//(int)NR_INLINE_DENTRY(&node_blk->i);
		int bitmap_size = DIV_ROUND_UP<int>(entry_cnt, BITS_PER_BYTE);
			//(int)INLINE_DENTRY_BITMAP_SIZE(&node_blk->i);
		int reserved_size = (max_inline_data - (item_size * entry_cnt + bitmap_size));
			//(int)INLINE_RESERVED_SIZE(&node_blk->i);

		d->max = entry_cnt;
		d->nr_bitmap = bitmap_size;
		d->bitmap = (u8 *)src;
		d->dentry = (struct f2fs_dir_entry *) ((char *)src + bitmap_size + reserved_size);
		d->filename = (__u8 (*)[F2FS_SLOT_LEN])((char *)src + bitmap_size + reserved_size +	SIZE_OF_DIR_ENTRY * entry_cnt);
	}
}

static struct f2fs_dir_entry *find_target_dentry(const u8 *name, unsigned int len, f2fs_hash_t namehash, int *max_slots,
...
static int find_in_level(fsck_f2fs_sb_info *sbi, f2fs_node *dir, unsigned int level, fsck_dentry *de)
{
	...
	for (; bidx < end_block; bidx++) {
		...
		ret = fs->dev_read_block(dentry_blk, dn.data_blkaddr);
		ASSERT(ret >= 0);

		dentry = find_in_block(dentry_blk, de->name, de->len, namehash, &max_slots);
		if (dentry) {
			ret = 1;
...
static int f2fs_find_entry(fsck_f2fs_sb_info *sbi,
				f2fs_node *dir, fsck_dentry *de)
{
...
nid_t f2fs_lookup(fsck_f2fs_sb_info *sbi, f2fs_node *dir, u8 *name, int len)
{
	int err;
	fsck_dentry de;
	de.name = name;
	...
	if (err == 1)		return de.ino;
	else				return 0;
}

static void f2fs_update_dentry(nid_t ino, int file_type, struct f2fs_dentry_ptr *d,
		...
		unsigned int bit_pos)
{
	...
	for (i = 0; i < slots; i++)
		test_and_set_bit_le(bit_pos + i, (char*)d->bitmap);
}

/*
 * f2fs_add_link - Add a new file(dir) to parent dir.
 */
int f2fs_add_link(fsck_f2fs_sb_info *sbi, f2fs_node *parent, const unsigned char *name, int name_len, nid_t ino,
			int file_type, block_t p_blkaddr, int inc_link)
{
	...
	f2fs_hash_t dentry_hash = f2fs_dentry_hash(get_encoding(sbi), IS_CASEFOLDED(&parent->i), name, name_len);
	...
	dentry_blk = (f2fs_dentry_block*)calloc(BLOCK_SZ, 1);
	ASSERT(dentry_blk);
...
add_dentry:
	...
	ret = fs->dev_write_block((BYTE*)dentry_blk, dn.data_blkaddr);
	ASSERT(ret >= 0);

	/* Parent inode needs updating, because its inode info may be changed. such as i_current_depth and i_blocks.	 */
	...
	if ((__u64)((block + 1) * F2FS_BLKSIZE) > le64_to_cpu(parent->i.i_size)) {
		parent->i.i_size = cpu_to_le64((block + 1) * F2FS_BLKSIZE);
		dn.idirty = 1;
	}

	if (dn.ndirty) {
		ret = fs->dev_write_block((BYTE*)dn.node_blk, dn.node_blkaddr);
		ASSERT(ret >= 0);
	}

	if (dn.idirty) {
		ASSERT(parent == dn.inode_blk);
		ret = fs->write_inode(dn.inode_blk, p_blkaddr);
		ASSERT(ret >= 0);
	}

	if (dn.node_blk != dn.inode_blk)	free(dn.node_blk);
	free(dentry_blk);
	return 0;
}

static void make_empty_dir(fsck_f2fs_sb_info *sbi, f2fs_node *inode)
{
	CF2fsFileSystem* fs = sbi->GetFs();
	...
	dent_blk = (f2fs_dentry_block*)calloc(BLOCK_SZ, 1);
	ASSERT(dent_blk);
	...
	ret = fs->dev_write_block((BYTE*)dent_blk, blkaddr);
	ASSERT(ret >= 0);

	inode->i._u.i_addr[get_extra_isize(&inode->i)] = cpu_to_le32(blkaddr);
	free(dent_blk);
}

static void page_symlink(fsck_f2fs_sb_info *sbi, f2fs_node *inode,	const char *symname, int symlen)
{
	CF2fsFileSystem* fs = sbi->GetFs();
	nid_t ino = le32_to_cpu(inode->footer.ino);
	struct f2fs_summary sum;
	...
	if ((unsigned long)(symlen + 1) <= MAX_INLINE_DATA(&inode->i)) {
	...
	data_blk = (char*)calloc(BLOCK_SZ, 1);
	ASSERT(data_blk);
	...
	ret = fs->dev_write_block((BYTE*)data_blk, blkaddr);
	ASSERT(ret >= 0);
...
static inline int is_extension_exist(const char *s, const char *sub)
{
	unsigned int slen = (unsigned int)strlen(s);
	unsigned int  sublen = (unsigned int)strlen(sub);

	/* filename format of multimedia file should be defined as:
	 * "filename + '.' + extension + (optional: '.' + temp extension)".	 */
	if (slen < sublen + 2)		return 0;

	for (unsigned int i = 1; i < slen - sublen; i++) {
		if (s[i] != '.')			continue;
		if (!strncmp(s + i + 1, sub, sublen))			return 1;
	}

	return 0;
}

static void set_file_temperature(fsck_f2fs_sb_info *sbi, f2fs_node *node_blk, const unsigned char *name)
{
	...
	for (i = 0; i < cold_count + hot_count; i++) {
		if (is_extension_exist((const char *)name, (const char *)extlist[i]))
			break;
	}
	...
	if (i < cold_count)		node_blk->i.i_advise |= FADVISE_COLD_BIT;
	else					node_blk->i.i_advise |= FADVISE_HOT_BIT;
}

static void init_inode_block(fsck_f2fs_sb_info *sbi, f2fs_node *node_blk, fsck_dentry *de)
{
	...
	if (de->file_type == F2FS_FT_DIR) {
		...
		ASSERT(de->link);
		mode |= S_IFLNK;
		size = (unsigned int)strlen(de->link);
		if (size + 1 > MAX_INLINE_DATA(&node_blk->i))			blocks++;
	} 
	else {		ASSERT(0);	}
	...
	if (de->file_type == F2FS_FT_DIR)		node_blk->i.i_current_depth = cpu_to_le32(1);
	else									node_blk->i.i_current_depth = cpu_to_le32(0);
	...
	if (c.feature & cpu_to_le32(F2FS_FEATURE_INODE_CHKSUM))
		node_blk->i._u._s.i_inode_checksum = cpu_to_le32(CF2fsFileSystem::f2fs_inode_chksum(node_blk, c.chksum_seed));
}

int convert_inline_dentry(fsck_f2fs_sb_info *sbi, f2fs_node *node, block_t p_blkaddr)
{
	CF2fsFileSystem* fs = sbi->GetFs();
	f2fs_inode *inode = &(node->i);
	unsigned int dir_level = node->i.i_dir_level;
	nid_t ino = le32_to_cpu(node->footer.ino);
	size_t inline_size = MAX_INLINE_DATA(&node->i);
	char *inline_data = new char[inline_size];
	fsck_dnode_of_data dn;
	struct f2fs_dentry_ptr d;
	unsigned long bit_pos = 0;
	int ret = 0;

	if (!(inode->i_inline & F2FS_INLINE_DENTRY))		return 0;

	memcpy(inline_data, inline_data_addr(node), inline_size);
	memset(inline_data_addr(node), 0, inline_size);
	inode->i_inline &= ~F2FS_INLINE_DENTRY;

	ret = fs->dev_write_block((BYTE*)node, p_blkaddr);
	ASSERT(ret >= 0);
	...
	if (!dir_level) {
		...
		if (dn.data_blkaddr == NULL_ADDR)	new_data_block(sbi, dentry_blk, &dn, CURSEG_HOT_DATA);
		...
		memset((char*)dst.bitmap + src.nr_bitmap, 0, dst.nr_bitmap - src.nr_bitmap);
	...
	while (bit_pos < (unsigned long)d.max) {
		...
		if (!test_bit_le(bit_pos, (char*)d.bitmap)) {
			bit_pos++;
			continue;
		}
		...
		ret = f2fs_add_link(sbi, node, filename, namelen, le32_to_cpu(de->ino), de->file_type, p_blkaddr, 0);
		if (ret)			MSG(0, "Convert file \"%s\" ERR=%d\n", filename, ret);
		else				MSG(1, "%s: add inline entry to block\n", __func__);
		bit_pos += GET_DENTRY_SLOTS(namelen);
	}
	delete[] inline_data;
...
struct hardlink_cache_entry *f2fs_search_hardlink(fsck_f2fs_sb_info *sbi, fsck_dentry *de)
{
	...
	void *search_result = nullptr;

	/* This might be a hardlink, try to find it in the cache */
	find_hardlink = (hardlink_cache_entry*)calloc(1, sizeof(struct hardlink_cache_entry));
...
int f2fs_create(fsck_f2fs_sb_info *sbi, fsck_dentry *de)
{
	CF2fsFileSystem* fs = sbi->GetFs();
	f2fs_node *parent, *child;
	struct hardlink_cache_entry *found_hardlink = NULL;
	...
	parent = (f2fs_node*)calloc(BLOCK_SZ, 1);
	ASSERT(parent);

	ret = fs->dev_read_block(parent, ni.blk_addr);
	ASSERT(ret >= 0);
	...
	if (ret) {
		...
		if (de->file_type == F2FS_FT_REG_FILE)		de->ino = 0;
		goto free_parent_dir;
	}

	child = (f2fs_node*)calloc(BLOCK_SZ, 1);
	ASSERT(child);
	...
	if (found_hardlink) {
		if (!found_hardlink->to_ino) {
			...
			ret = fs->dev_read_block(child, blkaddr);
			ASSERT(ret >= 0);
...
write_child_dir:
	ret = fs->dev_write_block((BYTE*)child, blkaddr);
	ASSERT(ret >= 0);

	update_free_segments(sbi);
	MSG(1, "Info: Create %s -> %s\n"
		"  -- ino=%x, type=%x, mode=%x, uid=%x, gid=%x, cap=%lld, size=%lu, link=%u blocks=%lld pino=%x\n",
...
int f2fs_mkdir(fsck_f2fs_sb_info *sbi, fsck_dentry *de)
{
	return f2fs_create(sbi, de);
}

int f2fs_symlink(fsck_f2fs_sb_info *sbi, fsck_dentry *de)
{
	return f2fs_create(sbi, de);
}

int f2fs_find_path(fsck_f2fs_sb_info *sbi, char *path, nid_t *ino)
{
	CF2fsFileSystem* fs = sbi->GetFs();
	f2fs_node *parent;
	struct node_info ni;
	...
	*ino = sbi->F2FS_ROOT_INO();
	parent = (f2fs_node*)calloc(BLOCK_SZ, 1);
	ASSERT(parent);

	char* context = nullptr;
	p = strtok_s(path, "/", &context);
	while (p) {
		de.name = (const u8 *)p;
		de.len = (int)strlen(p);
