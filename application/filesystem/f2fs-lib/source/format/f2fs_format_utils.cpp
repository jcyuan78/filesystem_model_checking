...
static int trim_device(int i)
{
	...
	UINT64 bytes = dev->total_sectors * dev->sector_size;
	int fd = dev->fd;
...
int CF2fsFileSystem::f2fs_trim_devices(f2fs_configuration& config)
{
	int i;
	for (i = 0; i < config.ndevs; i++) 
	{
		if (!is_wiped_device(i) && trim_device(i))	return -1;
	}
	config.trimmed = 1;
	return 0;
}
