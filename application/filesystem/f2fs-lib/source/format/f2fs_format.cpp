...
LOCAL_LOGGER_ENABLE(L"f2fs.format", LOGGER_LEVEL_DEBUGINFO);
...
void CF2fsFileSystem::cure_extension_list(f2fs_configuration & c)
{
	const char **extlist;
	char *ext_str;
	char *ue;
	...
	for (i = 0; i < 2; i++) {
		...
		if (i == 0) 		set_sb(extension_count, pos);
		else			sb->hot_ext_count = pos - get_sb(extension_count);;

		if (!ext_str)			continue;

		/* add user ext list */
		char* next_token = NULL;
		ue = strtok_s(ext_str, ", ", &next_token);
		while (ue != NULL) 
		{
			name_len = strlen(ue);
...
//				MSG(0, "\tWarn: Extension name (%s) is too long\n", ue);
				LOG_NOTICE(L"Warn: Extension name(%S) is too long", ue);
				goto next;
			}
			if (!is_extension_exist(ue)) memcpy(sb->extension_list[pos++], ue, name_len);
next:
			ue = strtok_s(NULL, ", ", &next_token);
			if (pos >= F2FS_MAX_EXTENSION)	break;
		}
...
static void verify_cur_segs(f2fs_configuration & c)
{
	int i, j;
	int reorder = 0;
...
int CF2fsFileSystem::f2fs_prepare_super_block(f2fs_configuration& c)
{
	UINT32 blk_size_bytes;
	UINT32 log_sectorsize, log_sectors_per_block;
	UINT32 log_blocksize, log_blks_per_seg;
	UINT32 segment_size_bytes, zone_size_bytes;
	UINT32 sit_segments, nat_segments;
	UINT32 blocks_for_sit, blocks_for_nat, blocks_for_ssa;
	UINT32 total_valid_blks_available;
	UINT64 zone_align_start_offset, diff;
	UINT64 total_meta_zones, total_meta_segments;
	UINT32 sit_bitmap_size, max_sit_bitmap_size;
	UINT32 max_nat_bitmap_size, max_nat_segments;
	UINT32 total_zones, avail_zones;
	...
	if (c.zoned_mode && c.ndevs > 1)
		zone_align_start_offset += (c.devices[0].total_sectors * c.sector_size) % zone_size_bytes;

	set_sb(segment0_blkaddr, boost::numeric_cast<UINT32>(zone_align_start_offset / blk_size_bytes));
	sb->cp_blkaddr = sb->segment0_blkaddr;

	MSG(0, "Info: zone aligned segment0 blkaddr: %u\n",	get_sb(segment0_blkaddr));

	if (c.zoned_mode &&
		((c.ndevs == 1 && (get_sb(segment0_blkaddr) + c.start_sector / DEFAULT_SECTORS_PER_BLOCK) % c.zone_blocks) ||
	...
	//<YUAN> 初始化device为无效状态
//	memset(sb->devs, 0xFF, sizeof(f2fs_device) * MAX_DEVICES);
//	memset(sb->devs, 0, sizeof(f2fs_device) * MAX_DEVICES);
	for (i = 0; i < c.ndevs; i++) 
	{
		sb->devs[i].path[0] = 'A'+i;	// 在windows上，path不再有用，保留空间兼容。设置有效性标志。	
		if (i == 0) 
		{
	...
	total_valid_blks_available = (get_sb(segment_count) -
			(get_sb(segment_count_ckpt) + get_sb(segment_count_sit))) * c.blks_per_seg;
	...
	/* The number of node segments should not be exceeded a "Threshold". This number resizes NAT bitmap area in 
	 a CP page. So the threshold is determined not to overflow one CP page	 */
	sit_bitmap_size = ((get_sb(segment_count_sit) / 2) << log_blks_per_seg) / 8;

	if (sit_bitmap_size > MAX_SIT_BITMAP_SIZE) 	max_sit_bitmap_size = MAX_SIT_BITMAP_SIZE;
	else		max_sit_bitmap_size = sit_bitmap_size;
	...
	{	/* use cp_payload if free space of f2fs_checkpoint is not enough */
		...
		{
			UINT32 diff =  max_sit_bitmap_size + max_nat_bitmap_size - MAX_BITMAP_SIZE_IN_CKPT;
	...
	{
		/* It should be reserved minimum 1 segment for nat. When sit is too large, we should expand cp area.
		...
		if (get_sb(segment_count_nat) > max_nat_segments) set_sb(segment_count_nat, max_nat_segments);
		set_sb(segment_count_nat, get_sb(segment_count_nat) * 2);
	}
	...
	if (c.feature & cpu_to_le32(F2FS_FEATURE_RO)) 	blocks_for_ssa = 0;
	else		blocks_for_ssa = total_valid_blks_available /	c.blks_per_seg + 1;

	set_sb(segment_count_ssa, SEG_ALIGN(blocks_for_ssa));

	total_meta_segments = get_sb(segment_count_ckpt) + get_sb(segment_count_sit) + get_sb(segment_count_nat) + get_sb(segment_count_ssa);
	diff = total_meta_segments % (c.segs_per_zone);
	if (diff) set_sb(segment_count_ssa, get_sb(segment_count_ssa) +	(c.segs_per_zone - boost::numeric_cast<UINT32>(diff)));
	...
	if (c.zoned_mode) 
	{
		/* Make sure there is enough randomly writeable space at the beginning of the disk. */
		...
		{
			MSG(0, "\tError: Device does not have enough random write zones for F2FS volume (%lu needed)\n",
					main_blkzone);
			return -1;
		}
		/* Check if conventional device has enough space to accommodate all metadata, zoned device should
		 * not overlap to metadata area.  */
		for (i = 1; i < c.ndevs; i++) 
		{
			if (c.devices[i].zoned_model == F2FS_ZONED_HM && c.devices[i].start_blkaddr < get_sb(main_blkaddr)) 
			{
				LOG_ERROR(L"[err] Conventional device %d is too small, (%lld MiB needes).", i, (get_sb(main_blkaddr) - c.devices[i].start_blkaddr) >> 8);
				//MSG(0, "\tError: Conventional device %s is too small, (%" PRIu64 " MiB needed).\n", 
				//	c.devices[0].path, (get_sb(main_blkaddr) - c.devices[i].start_blkaddr) >> 8);
				return -1;
			}
		}
	}
	...
	/* Let's determine the best reserved and overprovisioned space. For Zoned device, if zone capacity less than 
	zone size, the segments starting after the zone capacity are unusable in each zone. So get overprovision 
	ratio and reserved seg count based on avg usable segs_per_sec.	 */
	if (c.overprovision == 0) c.overprovision = get_best_overprovision(sb);

	c.reserved_segments = (2 * (100 / c.overprovision + 1) + NR_CURSEG_TYPE) *
			DIV_ROUND_UP(f2fs_get_usable_segments(sb), get_sb(section_count));
	...
	{
		LOG_ERROR(L"[err] Device size is not sufficient for F2FS volume");
		if ((!(c.feature & cpu_to_le32(F2FS_FEATURE_RO)) && c.overprovision == 0))
			LOG_ERROR(L" == F2FS_FEATURE_RO is enabled, but overprovision == 0");
		if (c.total_segments < F2FS_MIN_SEGMENTS) LOG_ERROR(L" == total_secments(%d) is less than request (%d)", c.total_segments, F2FS_MIN_SEGMENTS);
		if (c.devices[0].total_sectors * c.sector_size < zone_align_start_offset)
			LOG_ERROR(L" == total size (%X) is less than zone align size (%X)", c.devices[0].total_sectors * c.sector_size, zone_align_start_offset);
		if (get_sb(segment_count_main) - NR_CURSEG_TYPE < c.reserved_segments)
			LOG_ERROR(L" == segment count (%d) is less than reserved (%d) + NR_CURSEG_TYPE", get_sb(segment_count_main), c.reserved_segments, NR_CURSEG_TYPE);
	...
	else 
	{
		...
		memcpy_s(sb->uuid, 16, id.data, 16);
...
//	utf8_to_utf16(sb->volume_name, (const char *)c.vol_label, MAX_VOLUME_NAME, strlen(c.vol_label));
	wmemcpy_s(sb->volume_name, MAX_VOLUME_NAME, m_vol_name.c_str(), m_vol_name.size());
	set_sb(node_ino, 1);
	set_sb(meta_ino, 2);
	set_sb(root_ino, 3);
	c.next_free_nid = 4;
	...
	if (c.feature & cpu_to_le32(F2FS_FEATURE_QUOTA_INO)) 
	{
		...
		if (c.feature & cpu_to_le32(F2FS_FEATURE_PRJQUOTA))		quotatype_bits |= QUOTA_PRJ_BIT;
	}

	for (int iqtype = 0; iqtype < F2FS_MAX_QUOTAS; iqtype++) 
	{
		//qtype = reinterpret_cast<quota_type>(iqtype);
		qtype = (quota_type)(iqtype);
		if (!((1 << qtype) & quotatype_bits))	continue;
		sb->qf_ino[qtype] = cpu_to_le32(c.next_free_nid++);
		MSG(0, "Info: add quota type = %u => %u\n", qtype, c.next_free_nid - 1);
	}

	if (c.feature & cpu_to_le32(F2FS_FEATURE_LOST_FOUND))	c.lpf_ino = c.next_free_nid++;

	if (c.feature & cpu_to_le32(F2FS_FEATURE_RO))		avail_zones = 2;
	else		avail_zones = 6;

	if (total_zones <= avail_zones) 
	{
		MSG(1, "\tError: %d zones: Need more zones by shrinking zone size\n", total_zones);
		return -1;
	}
	...
	if (c.feature & cpu_to_le32(F2FS_FEATURE_RO)) {
		...
		c.cur_seg[CURSEG_COLD_NODE] = next_zone(CURSEG_WARM_NODE);
		c.cur_seg[CURSEG_HOT_DATA] = next_zone(CURSEG_COLD_NODE);
		c.cur_seg[CURSEG_COLD_DATA] = max(last_zone((total_zones >> 2)), next_zone(CURSEG_HOT_DATA));
		c.cur_seg[CURSEG_WARM_DATA] = max(last_zone((total_zones >> 1)), next_zone(CURSEG_COLD_DATA));
	}

	/* if there is redundancy, reassign it */
	if (!(c.feature & cpu_to_le32(F2FS_FEATURE_RO)))	verify_cur_segs(c);

	cure_extension_list(c);
...
//		get_kernel_uname_version(c.version);
		strcpy_s(m_config.version, "Windows");
	}

	memcpy_s(sb->version, VERSION_LEN, c.version, VERSION_LEN);
	memcpy_s(sb->init_version, VERSION_LEN, c.version, VERSION_LEN);
...
int CF2fsFileSystem::f2fs_init_sit_area(f2fs_configuration & c)
{
	UINT32 blk_size, seg_size;
	UINT32 index = 0;
	...
	zero_buf = (UINT8*)calloc(sizeof(UINT8), seg_size);
	if(zero_buf == NULL) 
	{
		LOG_ERROR(L"[err] Calloc Failed for sit_zero_buf!!!");	// log level 1
		return -1;
	}
	...
	for (index = 0; index < (get_sb(segment_count_sit) / 2); index++) 
	{
		...
		{
			LOG_ERROR(L"[err]: While zeroing out the sit area on disk!!!");
			free(zero_buf);
			return -1;
		}
...
int CF2fsFileSystem::f2fs_init_nat_area(f2fs_configuration & c)
{
	UINT32 blk_size, seg_size;
	UINT32 index = 0;
	...
	nat_buf = (UINT8*)calloc(sizeof(UINT8), seg_size);
	if (nat_buf == NULL) 
	{
		LOG_ERROR(L"[err] Calloc Failed for nat_zero_blk!!!");
		return -1;
	}
	...
	for (index = 0; index < get_sb(segment_count_nat) / 2; index++) 
	{
		...
		{
			LOG_ERROR(L"[err] While zeroing out the nat area on disk!!!");
			free(nat_buf);
			return -1;
		}
...
int CF2fsFileSystem::f2fs_write_check_point_pack(f2fs_configuration & c)
{
	struct f2fs_journal *journal;
	UINT32 blk_size_bytes;
	UINT32 nat_bits_bytes, nat_bits_blocks;
	...
	sum = (f2fs_summary_block*)calloc(F2FS_BLKSIZE, 1);
	if (sum == NULL) 
	{
	...
	sum_compact = (char*)calloc(F2FS_BLKSIZE, 1);
	if (sum_compact == NULL) 
	{
	...
	nat_bits = (UINT8*)calloc(F2FS_BLKSIZE, nat_bits_blocks);
	if (nat_bits == NULL) 
	{
	...
	cp_payload = (char*)calloc(F2FS_BLKSIZE, 1);
	if (cp_payload == NULL) {
	...
	srand((c.fake_seed) ? 0 : (UINT)(time(NULL)) );
	...
	if (c.feature & cpu_to_le32(F2FS_FEATURE_RO)) 
	{
		...
		set_cp(user_block_count, ((get_cp(free_segment_count) + 2 -	get_cp(overprov_segment_count)) * c.blks_per_seg));
	} 
	else 
	{
	...
	*/
	...
	for (int iqtype = 0, i = 1; iqtype < F2FS_MAX_QUOTAS; iqtype++) 
	{
		enum quota_type qtype = (quota_type)iqtype;

		if (sb->qf_ino[qtype] == 0)			continue;
		...
		journal->nat_j.entries[i].ne.block_addr = cpu_to_le32(get_sb(main_blkaddr) + get_cp(cur_node_segno[0]) *	c.blks_per_seg + i);
		i++;
	}
	...
	/* inode sit for root */
	if (c.feature & cpu_to_le32(F2FS_FEATURE_RO))		journal->n_sits = cpu_to_le16(2);
	else		journal->n_sits = cpu_to_le16(6);

	journal->sit_j.entries[0].segno = cp->cur_node_segno[0];
	journal->sit_j.entries[0].se.vblocks =	cpu_to_le16((CURSEG_HOT_NODE << 10) | (1 + c.quota_inum + c.lpf_inum));
	f2fs_set_bit(0, (char *)journal->sit_j.entries[0].se.valid_map);
	for (i = 1; i <= c.quota_inum; i++)		f2fs_set_bit(i, (char *)journal->sit_j.entries[0].se.valid_map);
	if (c.lpf_inum)		f2fs_set_bit(i, (char *)journal->sit_j.entries[0].se.valid_map);

	if (c.feature & cpu_to_le32(F2FS_FEATURE_RO)) 
	{
		/* data sit for root */
		journal->sit_j.entries[1].segno = cp->cur_data_segno[0];
		journal->sit_j.entries[1].se.vblocks = cpu_to_le16((CURSEG_HOT_DATA << 10) | (1 + c.quota_dnum + c.lpf_dnum));
		f2fs_set_bit(0, (char *)journal->sit_j.entries[1].se.valid_map);
		for (i = 1; i <= c.quota_dnum; i++)			f2fs_set_bit(i, (char *)journal->sit_j.entries[1].se.valid_map);
		if (c.lpf_dnum)			f2fs_set_bit(i, (char *)journal->sit_j.entries[1].se.valid_map);
	} 
	else 
	{
	...
	for (int iqtype = 0; iqtype < F2FS_MAX_QUOTAS; iqtype++) 
	{
		...
		for (j = 0; j < QUOTA_DATA(qtype); j++) 
		{
			...
			(sum_entry + off + j)->_u._s.ofs_in_node = cpu_to_le16(j);
		}
	...
	if (c.lpf_dnum) 
	{
		...
		(sum_entry + off)->_u._s.ofs_in_node = 0;
	}
	...
	if (dev_write_block((BYTE*)sum_compact, cp_seg_blk)) 
	{
	...
	sum->entries[0]._u._s.ofs_in_node = 0;
	for (int iqtype = i = 0; iqtype < F2FS_MAX_QUOTAS; iqtype++) 
	{
		enum quota_type qtype = (quota_type)(iqtype);
		if (sb->qf_ino[qtype] == 0)			continue;
		sum->entries[1 + i].nid = sb->qf_ino[qtype];
		sum->entries[1 + i]._u._s.ofs_in_node = 0;
		i++;
	}
	if (c.lpf_inum) 
	{
		...
		sum->entries[i]._u._s.ofs_in_node = 0;
	}
...
int CF2fsFileSystem::f2fs_write_super_block(f2fs_configuration & c)
{
	int index;
	UINT8 *zero_buff;
	f2fs_super_block* sb = &m_raw_sb;
	...
	for (index = 0; index < 2; index++) 
	{
		...
		{
			MSG(1, "\tError: While while writing super_blk on disk!!! index : %d\n", index);
			free(zero_buff);
			return -1;
		}
	}
...
int CF2fsFileSystem::f2fs_discard_obsolete_dnode(f2fs_configuration & c)
{
	...
	UINT64 start_inode_pos = get_sb(main_blkaddr);
	UINT64 last_inode_pos;

	if (c.zoned_mode || c.feature & cpu_to_le32(F2FS_FEATURE_RO))
		return 0;

	raw_node = (f2fs_node*)calloc(sizeof(struct f2fs_node), 1);
	if (raw_node == NULL) 
	{
	...
	last_inode_pos = start_inode_pos + c.cur_seg[CURSEG_HOT_NODE] * c.blks_per_seg + c.quota_inum + c.lpf_inum;

	do {
		if (offset < get_sb(main_blkaddr) || offset >= end_blkaddr) break;
		...
		if (dev_write_block((BYTE*)raw_node, offset)) 
		{
		...
		if (offset >= start_inode_pos || offset <= last_inode_pos) break;
	} while (1);
...
int get_extra_isize(f2fs_inode& iinode)
{
	return iinode._u._s.i_extra_isize / sizeof(__le32);
}

int CF2fsFileSystem::f2fs_write_root_inode(f2fs_configuration & c)
{
	...
	if (c.lpf_ino)		raw_node->i.i_links = cpu_to_le32(3);
	else		raw_node->i.i_links = cpu_to_le32(2);
	raw_node->i.i_uid = cpu_to_le32(c.root_uid);
	raw_node->i.i_gid = cpu_to_le32(c.root_gid);

	blk_size_bytes = (UINT64)1 << get_sb(log_blocksize);
	...
	if (c.feature & cpu_to_le32(F2FS_FEATURE_EXTRA_ATTR)) {
		...
		raw_node->i._u._s.i_extra_isize = cpu_to_le16(calc_extra_isize(c));
	}

	if (c.feature & cpu_to_le32(F2FS_FEATURE_PRJQUOTA))
		raw_node->i._u._s.i_projid = cpu_to_le32(F2FS_DEF_PROJID);

	if (c.feature & cpu_to_le32(F2FS_FEATURE_INODE_CRTIME)) {
		raw_node->i._u._s.i_crtime = cpu_to_le32(mkfs_time);
		raw_node->i._u._s.i_crtime_nsec = 0;
	}

	if (c.feature & cpu_to_le32(F2FS_FEATURE_COMPRESSION)) {
		raw_node->i._u._s.i_compress_algorithm = 0;
		raw_node->i._u._s.i_log_cluster_size = 0;
		raw_node->i._u._s.i_compress_flag = 0;
	}

	data_blk_nor = get_sb(main_blkaddr) + c.cur_seg[CURSEG_HOT_DATA] * c.blks_per_seg;
	raw_node->i._u.i_addr[get_extra_isize(raw_node->i)] = cpu_to_le32(data_blk_nor);

	raw_node->i.i_ext.fofs = 0;
	raw_node->i.i_ext.blk = 0;
	raw_node->i.i_ext.len = 0;

	main_area_node_seg_blk_offset = get_sb(main_blkaddr);
	main_area_node_seg_blk_offset += c.cur_seg[CURSEG_HOT_NODE] * c.blks_per_seg;

	DBG(1, "\tWriting root inode (hot node), %x %x %x at offset 0x%08" PRIu64 "\n", get_sb(main_blkaddr), c.cur_seg[CURSEG_HOT_NODE], c.blks_per_seg, main_area_node_seg_blk_offset);
...
int CF2fsFileSystem::f2fs_write_default_quota(int qtype, unsigned int blkaddr, __le32 raw_id)
{
	f2fs_configuration& c = m_config;
	char *filebuf = (char*) calloc(F2FS_BLKSIZE, 2);
	...
	if (c.lpf_ino) 	dqblk.dqb_curinodes = cpu_to_le64(2);
	else		dqblk.dqb_curinodes = cpu_to_le64(1);
	dqblk.dqb_bhardlimit = cpu_to_le64(0);
	dqblk.dqb_bsoftlimit = cpu_to_le64(0);
	if (c.lpf_ino)		dqblk.dqb_curspace = cpu_to_le64(8192);
	else		dqblk.dqb_curspace = cpu_to_le64(4096);
	...
	/* Write two blocks */
	if (dev_write_block((BYTE*)filebuf, blkaddr) || dev_write_block((BYTE*)filebuf + F2FS_BLKSIZE, blkaddr + 1)) 
	{
...
int CF2fsFileSystem::f2fs_write_qf_inode(f2fs_configuration & c, int qtype)
{
	struct f2fs_node *raw_node = NULL;
	UINT64 data_blk_nor;
	UINT64 main_area_node_seg_blk_offset = 0;
	...
	for (i = 0; i < qtype; i++)
		if (sb->qf_ino[i])	data_blk_nor += QUOTA_DATA(i);
	if (qtype == 0)			raw_id = raw_node->i.i_uid;
	else if (qtype == 1)	raw_id = raw_node->i.i_gid;
	else if (qtype == 2)	raw_id = raw_node->i._u._s.i_projid;
	else		ASSERT(0);
	...
	for (i = 0; i < QUOTA_DATA(qtype); i++)
		raw_node->i._u.i_addr[get_extra_isize(raw_node->i) + i] =	cpu_to_le32(data_blk_nor + i);
	raw_node->i.i_ext.fofs = 0;
	raw_node->i.i_ext.blk = 0;
	raw_node->i.i_ext.len = 0;
...
//	enum quota_type qtype;
	...
	if (dev_write_block((BYTE*)nat_blk, nat_seg_blk_offset)) 
	{
...
block_t CF2fsFileSystem::f2fs_add_default_dentry_lpf(f2fs_configuration & c)
{
	...
	if (dev_write_block((BYTE*)dent_blk, data_blk_offset)) {
...
int CF2fsFileSystem::f2fs_write_lpf_inode(f2fs_configuration & c)
{
	...
	data_blk_nor = f2fs_add_default_dentry_lpf(c);
	if (data_blk_nor == 0) 
	{
...
int CF2fsFileSystem::f2fs_create_root_dir(f2fs_configuration & c)
{
	...
	for (int iqtype = 0; iqtype < F2FS_MAX_QUOTAS; iqtype++)  
	{
		enum quota_type qtype = (quota_type)iqtype;
		if (sb->qf_ino[qtype] == 0)		continue;
		err = f2fs_write_qf_inode(c, qtype);
		if (err < 0) 
		{
...
#ifndef WITH_ANDROID
	err = f2fs_discard_obsolete_dnode(c);
	if (err < 0) 
	{
	...
	err = f2fs_add_default_dentry_root(c);
	if (err < 0) 
	{
...
exit:
	if (err) MSG(1, "\tError: Could not create the root directory!!!\n");

	return err;
}


int CF2fsFileSystem::f2fs_format_device(f2fs_configuration & c)
{
	int err = 0;

	err= f2fs_prepare_super_block(c);
	if (err < 0) 
	{
	...
	err = f2fs_write_check_point_pack(c);
	if (err < 0) 
	{
...
exit:
	if (err)	MSG(0, "\tError: Could not format the device!!!\n");
