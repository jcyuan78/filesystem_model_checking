...
static UINT16 *wchar_to_utf16(UINT16 *output, wchar_t wc, size_t outsize)
{
	if (wc <= 0xffff) {
		if (outsize == 0)
			return NULL;
...
int utf8_to_utf16(UINT16 *output, const char *input, size_t outsize, size_t insize)
{
	...
	while ((size_t)(inp - input) < insize && *inp) {
		...
		if (inp == NULL) 
		{
			LOG_ERROR(L"[err]illegal UTF-8 sequence");
			return -EILSEQ;
		}
		outp = wchar_to_utf16(outp, wc, outsize - (outp - output));
		if (outp == NULL) 
		{
			LOG_ERROR(L"[err] name is too long");
			return -ENAMETOOLONG;
		}
	}
	*outp = cpu_to_le16(0);
...
static const UINT16 *utf16_to_wchar(const UINT16 *input, wchar_t *wc, size_t insize)
{
...
//int utf16_to_utf8(char *output, const UINT16 *input, size_t outsize, size_t insize)
//{
//	const UINT16 *inp = input;
//	char *outp = output;
//	wchar_t wc;
//
...
//		if (inp == NULL) 
//		{
//			LOG_ERROR(L"[err] illegal UTF-16 sequence");
//			return -EILSEQ;
//		}
//		outp = wchar_to_utf8(outp, wc, outsize - (outp - output));
//		if (outp == NULL) 
//		{
//			LOG_DEBUG(L"[err] name is too long");
//			return -ENAMETOOLONG;
//		}
//	}
//	*outp = '\0';
...
int test_and_set_bit_le(UINT32 nr, UINT8 *addr)
{
	int mask, retval;
...
int test_and_clear_bit_le(UINT32 nr, UINT8 *addr)
{
	int mask, retval;
...
int test_bit_le(UINT32 nr, const UINT8 *addr)
{
...
static inline UINT64 __ffs(UINT8 word)
{
	int num = 0;
...
static UINT64 _find_next_bit_le(const UINT8 *addr, UINT64 nbits, UINT64 start, char invert)
{
	UINT8 tmp;
	...
	start = round_down<UINT64>(start, BITS_PER_BYTE);
...
UINT64 find_next_bit_le(const UINT8 *addr, UINT64 size, UINT64 offset)
{
	return _find_next_bit_le(addr, size, offset, 0);
}


UINT64 find_next_zero_bit_le(const UINT8 *addr, UINT64 size, UINT64 offset)
{
	return _find_next_bit_le(addr, size, offset, (char)0xff);
}
...
f2fs_hash_t f2fs_dentry_hash(int encoding, int casefolded, const unsigned char *name, int len)
{
	...
	if (len && casefolded) 
	{
		...
		buff = (UINT8*)malloc(sizeof(char) * PATH_MAX);
...
{
	if (!LINUX_S_ISREG(le16_to_cpu(i->i_mode)) || !(le32_to_cpu(i->i_flags) & F2FS_COMPR_FL))	return DEF_ADDRS_PER_BLOCK;
...
UINT32 f2fs_cal_crc32(UINT32 crc, const void *buf, size_t len)
{
	int i;
	...
	{
		crc ^= *p++;
		for (i = 0; i < 8; i++)			crc = (crc >> 1) ^ ((crc & 1) ? CRCPOLY_LE : 0);
	}
	return crc;
}

int f2fs_crc_valid(UINT32 blk_crc, void *buf, int len)
{
	UINT32 cal_crc = f2fs_cal_crc32(F2FS_SUPER_MAGIC, buf, len);
	if (cal_crc != blk_crc)	
	{
		LOG_ERROR(L"[err] CRC validation failed: cal_crc = %u, blk_crc = %u buff_size = 0x%x",	cal_crc, blk_crc, len);
		return -1;
	}
	return 0;
}
...
__u32 CF2fsFileSystem::f2fs_inode_chksum(struct f2fs_node *node)
{
	...
	offset += cs_size;
	chksum = f2fs_cal_crc32(chksum, (__u8 *)ri + offset, F2FS_BLKSIZE - offset);
	return chksum;
}

__u32 f2fs_checkpoint_chksum(f2fs_checkpoint *cp)
{
	...
	if (chksum_ofs < CP_CHKSUM_OFFSET) 
	{
		...
		chksum = f2fs_cal_crc32(chksum, (__u8 *)cp + chksum_ofs, F2FS_BLKSIZE - chksum_ofs);
	}
	return chksum;
}

#if 0 // move to fs2fs—filesystem.h
int write_inode(struct f2fs_node *inode, UINT64 blkaddr)
{
	if (c.feature & cpu_to_le32(F2FS_FEATURE_INODE_CHKSUM))
		inode->i.i_inode_checksum = cpu_to_le32(f2fs_inode_chksum(inode));
...
int f2fs_dev_is_writable(f2fs_configuration & c)
{
	return !c.ro || c.force;
}
...
int f2fs_devs_are_umounted(f2fs_configuration & c)
{
//	int i;
...
void get_kernel_version(char *version)
{
	int i;
...
void get_kernel_uname_version(char *version)
{
#ifdef HAVE_SYS_UTSNAME_H
	struct utsname buf;
...
static int win_get_device_size(const wchar_t *file, uint64_t *device_size)
{
	HANDLE dev;
	...
	if (dev == INVALID_HANDLE_VALUE) return EBADF;
...
int get_device_info(f2fs_configuration & c, int i)
{
	struct device_info *dev = c.devices + i;
#if 0
...
//	int32_t fd = 0;
	...
	jcvos::Utf8ToUnicode(str_path, dev->path);

	/* Block device target is not supported on Windows. */
	if (!c.sparse_mode) 
	{
		if (win_get_device_size(str_path.c_str(), &device_size)) 
		{
			MSG(0, "\tError: Failed to get device size!\n");
			return -1;
		}
	} 
	else {		device_size = c.device_size;	}
	
	FILE* fd = NULL;
	if (c.sparse_mode) 
	{
		_wfopen_s(&fd, str_path.c_str(), L"w+b");
...
//		fd = open((char *)dev->path, O_RDWR | O_BINARY);
...
#endif
	IVirtualDisk* disk = c.devices[i].m_fd;
	dev->total_sectors = disk->GetCapacity();
	...
	if (c.sparse_mode && f2fs_init_sparse_file())		return -1;

	return 0;
}
#endif

int CF2fsFileSystem::f2fs_get_device_info(f2fs_configuration & c)
{
	int i;
	// [原设计] f2fs_configuration::device 在parse operation中设置
	// [修改] 目前只能支持一个device。在MakeFileSystem中调用，需要调用此函数设置config中的全局信息。
	for (i = 0; i < c.ndevs; i++)		if (get_device_info(c, i))		return -1;
	...
	if (c.total_sectors * c.sector_size > (u_int64_t)F2FS_MAX_SEGMENT * 2 * 1024 * 1024) 
	{
	...
	for (i = 0; i < c.ndevs; i++) 
	{
		...
		{
		case F2FS_ZONED_NONE:	continue;
		case F2FS_ZONED_HM:		c.zoned_model = F2FS_ZONED_HM;			break;
		case F2FS_ZONED_HA:		if (c.zoned_model != F2FS_ZONED_HM)				c.zoned_model = F2FS_ZONED_HA;
			break;
		}
	}

	if (c.zoned_model != F2FS_ZONED_NONE) 
	{	/* For zoned model, the zones sizes of all zoned devices must be equal.	 */
		for (i = 0; i < c.ndevs; i++) {
		...
		/* Align sections to the device zone size and align F2FS zones to the device zones. For F2FS_ZONED_HA model without the BLKZONED feature set at format time, this is only an optimization as sequential writes will not be enforced. */
		c.segs_per_sec = boost::numeric_cast<UINT32>(c.zone_blocks / DEFAULT_BLOCKS_PER_SEGMENT);
		c.secs_per_zone = 1;
	} 
	else if(c.zoned_mode != 0) 
	{
		LOG_ERROR(L"[err] dev %d may not be a zoned block device",	0);
		return -1;
	}
	...
	MSG(0, "Info: total sectors = %" PRIu64 " (%" PRIu64 " MB)\n", c.total_sectors, (c.total_sectors * (c.sector_size >> 9)) >> 11);
	return 0;
}

unsigned int calc_extra_isize(f2fs_configuration & c)
{
	unsigned int size = offsetof(struct f2fs_inode, _u._s.i_projid);

	if (c.feature & cpu_to_le32(F2FS_FEATURE_FLEXIBLE_INLINE_XATTR))	size = offsetof(struct f2fs_inode, _u._s.i_projid);
	if (c.feature & cpu_to_le32(F2FS_FEATURE_PRJQUOTA))			size = offsetof(struct f2fs_inode, _u._s.i_inode_checksum);
	if (c.feature & cpu_to_le32(F2FS_FEATURE_INODE_CHKSUM))		size = offsetof(struct f2fs_inode, _u._s.i_crtime);
	if (c.feature & cpu_to_le32(F2FS_FEATURE_INODE_CRTIME))		size = offsetof(struct f2fs_inode, _u._s.i_compr_blocks);
	if (c.feature & cpu_to_le32(F2FS_FEATURE_COMPRESSION))		return sizeof(f2fs_inode::_u._s);
...
static const _f2fs_encoding_map f2fs_encoding_map[1] = {
	{"utf8", F2FS_ENC_UTF8_12_1, 0}, 
};
//= {
//	{
...
static const enc_flags encoding_flags[1] = {
...
const char *f2fs_encoding2str(const int encoding)
{
	for (int i = 0 ; i < ARRAY_SIZE(f2fs_encoding_map); i++)
		if (f2fs_encoding_map[i].encoding_magic == encoding)	return f2fs_encoding_map[i].name;
	return NULL;
}
...
{
	for (int i = 0 ; i < ARRAY_SIZE(f2fs_encoding_map); i++)
		if (f2fs_encoding_map[i].encoding_magic == encoding) 	return f2fs_encoding_map[encoding].default_flags;
	return 0;
}
...
int f2fs_str2encoding_flags(char **param, __u16 *flags)
{
	...
	char *f = strtok_s(*param, ",", &next_token);
