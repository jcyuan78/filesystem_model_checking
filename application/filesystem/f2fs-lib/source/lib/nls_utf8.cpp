...
static int utf8ncursor(utf8cursor *u8c, const struct utf8data *data, const char *s, size_t len)
{
	if (!data)
		return -1;
	if (!s)
	...
	u8c->ss = NULL;
	u8c->sp = NULL;
	u8c->len = boost::numeric_cast<unsigned int>(len);
	u8c->slen = 0;
	u8c->ccc = STOPPER;
	u8c->nccc = STOPPER;
...
static int utf8_casefold(const f2fs_nls_table *table,  const unsigned char *str, size_t len, unsigned char *dest, size_t dlen)
{
	...
	for (nlen = 0; nlen < dlen; nlen++) {
		...
		dest[nlen] = c;
		if (!c)			return boost::numeric_cast<int>(nlen);
		if (c == -1)	break;
	}
...
static const struct f2fs_nls_ops utf8_ops = { utf8_casefold };
//	.casefold = utf8_casefold,
//};

static const struct f2fs_nls_table nls_utf8 = { UNICODE_AGE(12, 1, 0),&utf8_ops };
//	.ops = &utf8_ops,
...
{
	if (encoding == F2FS_ENC_UTF8_12_1)		return &nls_utf8;
	return NULL;
}
