...
//	inode *new_inode;
	...
	f2fs_inode_info * new_inode = GetFs()->f2fs_new_inode<Cf2fsFileNode>(this, mode);
	if (IS_ERR(new_inode))		return PTR_ERR(new_inode);
	std::wstring fn;
#ifdef _DEBUG
	jcvos::Utf8ToUnicode(fn, entry->d_name.name);
	new_inode->m_description = L"file of " + fn;
	LOG_DEBUG(L"check size lock file=%s, lock=%d", fn.c_str(), new_inode->i_size_lock.LockCount);
#endif
...
//	new_inode->i_op = &f2fs_file_inode_operations;
//	new_inode->i_fop = &f2fs_file_operations;
//	new_inode->i_mapping->a_ops = &f2fs_dblock_aops;
	ino = new_inode->i_ino;
...
//	f2fs_free_filename(&fname);	//析构函数
	...
	if (!de) 
	{
		if (IS_ERR(page)) 
		{
//			return ERR_PTR<dentry>(PTR_ERR(page));
		}
		err = -ENOENT;
...
//		goto out_splice;
	...
	if (IS_ERR(new_node)) 
	{
		return ERR_PTR<dentry>(PTR_ERR(new_node));
...
//		goto out;
	...
	LOG_DEBUG(L"check size lock file=%s, lock=%d", fn.c_str(), new_node->i_size_lock.LockCount);
#endif
...
//		trace_f2fs_lookup_end(dir, src_entry, ino, err);
	...
	err = IS_ERR(new_entry)?reinterpret_cast<int>(new_entry):0;
...
//	trace_f2fs_unlink_enter(dir, ddentry);
	...
	f2fs_delete_entry(de, ppage, this, iinode);
#ifdef CONFIG_UNICODE
...
//	iinode->i_op = &f2fs_dir_inode_operations;
//	iinode->i_fop = &f2fs_dir_operations;
