...
LOCAL_LOGGER_ENABLE(L"f2fs.data", LOGGER_LEVEL_DEBUGINFO);
...
//	if (bio->bi_private)	mempool_free(bio->bi_private, bio_post_read_ctx_pool);
...
/* If the bio's data needs to be verified with fs-verity, then enqueue the verity work for the bio.  Otherwise finish
 the bio now.
 *
 * Note that to avoid deadlocks, the verity work can't be done on the decryption/decompression workqueue.  This is
 because verifying the data pages can involve reading verity metadata pages from the file, and these verity metadata
 pages may be encrypted and/or compressed. */
...
void CF2fsFileSystem::write_end_io(bio* bb)
{
	...
	bio_for_each_segment_all(bvec, bb, iter_all) 
	{
		...
		if (IS_DUMMY_WRITTEN_PAGE(page)) 
		{
			...
			unlock_page(page);
//			mempool_free(page, m_sb_info->write_io_dummy);
			if (unlikely(bb->bi_status))	f2fs_stop_checkpoint(m_sb_info, true);
			continue;
		}
...
#ifdef CONFIG_F2FS_FS_COMPRESSION
		if (f2fs_is_compressed_page(page)) {
			f2fs_compress_write_end_io(bb, page);
			continue;
		}
#endif
		...
		f2fs_bug_on(m_sb_info, page->mapping == NODE_MAPPING(m_sb_info) &&	page->index != nid_of_node(page));

		m_sb_info->dec_page_count(type);
		if (m_sb_info->f2fs_in_warm_node_list( page))	f2fs_del_fsync_node_entry(m_sb_info, page);
...
void CF2fsFileSystem::__submit_bio(bio *bio, enum page_type type)
{
	...
	if (!is_read_io(bio_op(bio))) 
	{
		...
		{
//			pp *page = mempool_alloc(sbi->write_io_dummy, GFP_NOIO | __GFP_NOFAIL);
			page* pp = new page;
			f2fs_bug_on(sbi, !pp);
			...
			lock_page(pp);
			LOG_DEBUG(L"add dummy page to bio, page=%llX, flag=%X", pp, pp->flags);
			if (bio_add_page(bio, pp, PAGE_SIZE, 0) < PAGE_SIZE)	f2fs_bug_on(sbi, 1);
		}
		/* In the NODE case, we lose next block address chain. So, we need to do checkpoint in f2fs_sync_file. */
		if (type == NODE)		sbi->set_sbi_flag(SBI_NEED_CP);
	}
submit_io:
...
{
	sbi->m_fs->__submit_bio( bio, type);
}
...
static void __submit_merged_bio(f2fs_bio_info *io)
{
	...
	if (!io->bio)	return;
	LOG_DEBUG(L"submit bio, sector=0x%llX, size=0x%llX", io->bio->bi_iter.bi_sector, io->bio->bi_iter.bi_size);
...
#endif
//	io->sbi->m_fs->__submit_bio(io->sbi, io->bio, fio->type);
	io->sbi->m_fs->__submit_bio(io->bio, fio->type);
	io->bio = NULL;
}

static bool __has_merged_page(struct bio *bio, struct inode *inode, struct page *page, nid_t ino)
{
	struct bio_vec *bvec;
	struct bvec_iter_all iter_all;
...
static void __submit_merged_write_cond(struct f2fs_sb_info *sbi, struct inode *inode, struct page *page,
	nid_t ino, enum page_type type, bool force)
{
	LOG_DEBUG(L"submit io, ino=%d, page=%llX, page_index=%d", ino, page, page?(page->index):-1);
	bool ret = true;
...
void f2fs_submit_merged_ipu_write(struct f2fs_sb_info *sbi, struct bio **bio, struct page *page)
{
	...
	if (found)	sbi->m_fs->__submit_bio(target, DATA);
	if (bio && *bio) 
	{
		bio_put(*bio);
		*bio = NULL;
...
bio * CF2fsFileSystem::f2fs_grab_read_bio(inode *inode, block_t blkaddr, unsigned nr_pages, unsigned op_flag,
				      pgoff_t first_idx, bool for_write)
{
	f2fs_sb_info *sbi = F2FS_I_SB(inode);
	unsigned int post_read_steps = 0;

	bio * ptr_bio = m_bio_set.bio_alloc_bioset(for_write ? GFP_NOIO : GFP_KERNEL, bio_max_segs(nr_pages)/*, &f2fs_bioset*/);
	...
	/*
	 * STEP_DECOMPRESS is handled specially, since a compressed file might contain both compressed and uncompressed
	 clusters.  We'll allocate a bio_post_read_ctx if the file is compressed, but the caller is responsible for 
	 enabling STEP_DECOMPRESS if it's actually needed.	 */
...
static int f2fs_submit_page_read(inode *inode, page *page, block_t blkaddr, int op_flags, bool for_write)
{
	f2fs_sb_info *sbi = F2FS_I_SB(inode);
	bio *bio;

	bio = sbi->m_fs->f2fs_grab_read_bio(inode, blkaddr, 1, op_flags, page->index, for_write);
...
//		zero_user_segment(page, 0, PAGE_SIZE);	// 将page数据清零
	...
	err = f2fs_submit_page_read(this, page, dn.data_blkaddr, op_flags, for_write);
	if (err) goto put_err;
	return page;
...
page* f2fs_inode_info::f2fs_get_new_data_page(page* ipage, pgoff_t index, bool new_i_size)
{
	...
	LOG_DEBUG(L"got page: page=%llX, addr=%llX, type=%s, index=%d",ppage, ppage->virtual_add,ppage->m_type.c_str(), ppage->index);
#endif
...
alloc:
	...
	f2fs_allocate_data_block(sbi, NULL, old_blkaddr, &dn->data_blkaddr,	&sum, seg_type, NULL);
...
//	struct inode *inode = file_inode(iocb->ki_filp);
	...
	if (f2fs_has_inline_data(inode))	return err;
...
map_blocks:
	...
	{
		if (!direct_io)	inode->set_inode_flag(FI_NO_PREALLOC);
		err = 0;
	}
	return err;
}
...
/* f2fs_map_blocks() tries to find or build mapping relationship which maps continuous logical blocks to physical
   blocks, and return such info via f2fs_map_blocks structure. */
int f2fs_map_blocks(f2fs_inode_info *iinode, struct f2fs_map_blocks *map, int create, int flag)
{
	...
	if (!create && f2fs_lookup_extent_cache(iinode, pgofs, &ei)) 
	{
		if (f2fs_lfs_mode(sbi) && flag == F2FS_GET_BLOCK_DIO &&	map->m_may_create)		goto next_dnode;
		...
		if (flag == F2FS_GET_BLOCK_DIO)	f2fs_wait_on_block_writeback_range(iinode, map->m_pblk, map->m_len);
		goto out;
	}

next_dnode:
	if (map->m_may_create)	f2fs_do_map_lock(sbi, flag, true);
	...
	end_offset = ADDRS_PER_PAGE(dn.node_page, iinode);

next_block:
	blkaddr = f2fs_data_blkaddr(&dn);

	if (__is_valid_data_blkaddr(blkaddr) && !sbi->f2fs_is_valid_blkaddr(blkaddr, DATA_GENERIC_ENHANCE))
	{
		err = -EFSCORRUPTED;
		goto sync_out;
	}
	...
	if (__is_valid_data_blkaddr(blkaddr)) 
	{
		...
		if (f2fs_lfs_mode(sbi) && flag == F2FS_GET_BLOCK_DIO &&	map->m_may_create) 
		{
			err = __allocate_data_block(&dn, map->m_seg_type);
			if (err)		goto sync_out;
			blkaddr = dn.data_blkaddr;
			set_inode_flag(iinode, FI_APPEND_WRITE);
		}
	}
	else 
	{
		if (create)
		{
			...
			else
			{
				...
				err = __allocate_data_block(&dn, map->m_seg_type);
				if (!err)	set_inode_flag(iinode, FI_APPEND_WRITE);
			}
			if (err)	goto sync_out;
...
sync_out:
	...
	if (flag == F2FS_GET_BLOCK_DIO && map->m_flags & F2FS_MAP_MAPPED)
		f2fs_wait_on_block_writeback_range(iinode, map->m_pblk, map->m_len);
...
submit_and_realloc:
	...
	if (bbio == NULL)
	{
		bbio = sbi->m_fs->f2fs_grab_read_bio(inode, block_nr, nr_pages,	is_readahead ? REQ_RAHEAD : 0, ppage->index,	false);
		if (IS_ERR(bbio)) 
		{
			ret = PTR_ERR(bbio);
			bbio = NULL;
...
static int f2fs_mpage_readpages(f2fs_inode_info*inode, readahead_control *rac, struct page *page)
{
	f2fs_sb_info* sbi = F2FS_I_SB(inode);
...
read_single_page:
		...
		ret = f2fs_read_single_page(inode, page, max_nr_pages, &map, &bio, &last_block_in_bio, rac);
		if (ret) 
		{
#ifdef CONFIG_F2FS_FS_COMPRESSION
set_error_page:
...
next_page:
	...
	if (bio) sbi->m_fs->__submit_bio(bio, DATA);
	return ret;
}
...
{
#if 1
	struct inode *ii = page_file_mapping(page)->host;
	f2fs_inode_info* inode = F2FS_I(ii);
	int ret = -EAGAIN;
...
//	trace_f2fs_readpages(inode, readahead_index(rac), readahead_count(rac));
	...
	if (!f2fs_is_compress_backend_ready(inode))		return;
	/* If the file has inline data, skip readpages */
	if (f2fs_has_inline_data(inode))		return;
	f2fs_mpage_readpages(F2FS_I(inode), rac, NULL);
#else
	JCASSERT(0)
#endif
}
				...
				struct f2fs_io_info *fio)
{
	...
	if (policy & (0x1 << F2FS_IPU_SSR_UTIL) && f2fs_need_SSR(sbi) &&
			utilization(sbi) > sbi->SM_I()->min_ipu_util)
		return true;
	...
	if (unlikely(fio && sbi->is_sbi_flag_set( SBI_CP_DISABLED) &&
...
static inline bool need_inplace_update(struct f2fs_io_info *fio)
{
	...
	if (f2fs_should_update_outplace(inode, fio))	return false;
...
{
	LOG_DEBUG(L"[IO] write date page, ino=%d", fio->ino);
	page *ppage = fio->page;
	...
	dn.set_new_dnode(iinode, NULL, NULL, 0);
	if (need_inplace_update(fio) &&	f2fs_lookup_extent_cache(iinode, ppage->index, &ei))
	{ // Inplace update
...
got_it:
	if (__is_valid_data_blkaddr(fio->old_blkaddr) && ! fio->sbi->f2fs_is_valid_blkaddr(fio->old_blkaddr, 
		...
		writeback_control *wbc, enum iostat_type io_type, int compr_blocks, bool allow_balance)
{
	...
	if (ppage->index < end_index ||	f2fs_verity_in_progress(iinode) || compr_blocks)
		goto write;
...
write:
	if (f2fs_is_drop_cache(iinode))		goto out;
	/* we should not write 0'th page having journal header */
	if (f2fs_is_volatile_file(iinode) && (!ppage->index ||
			(!wbc->for_reclaim && f2fs_available_free_memory(sbi, BASE_CHECK))))
		goto redirty_out;

	/* Dentry/quota blocks are controlled by checkpoint */
	if (S_ISDIR(iinode->i_mode) || IS_NOQUOTA(iinode))
	{
		/* We need to wait for node_write to avoid block allocation during checkpoint. This can only happen to quota 
		   writes which can cause the below discard race condition.	 */
		if (IS_NOQUOTA(iinode))		down_read(&sbi->node_write);

		fio.need_lock = LOCK_DONE;
		err = f2fs_do_write_data_page(&fio);
		LOG_DEBUG(L"after do write data page, page=%llX, flag=%X, writeback=%d", ppage,
			ppage ? ppage->flags : -1, ppage ? PageWriteback(ppage) : -1);
	...
	else if (has_not_enough_free_secs(sbi, 0, 0)) 	goto redirty_out;
	else	fi->set_inode_flag(FI_HOT_DATA);

	err = -EAGAIN;
	if (f2fs_has_inline_data(iinode))
	{
		err = f2fs_write_inline_data(iinode, ppage);
		if (!err) 	goto out;
	}
	...
	if (err == -EAGAIN)
	{
		...
		LOG_DEBUG(L"do write data page while err=EAGAIN, page=%llX, flag=%X, writeback=%d", ppage,
			ppage ? ppage->flags : -1, ppage ? PageWriteback(ppage) : -1);
...
redirty_out:
	 ...
	 * file_write_and_wait_range() will see EIO error, which is critical to return value of fsync() followed by 
	  atomic_write failure to user. */
...
static int f2fs_write_cache_pages(address_space *mapping, writeback_control *wbc, enum iostat_type io_type)
{
	...
	f2fs_inode_info* fi = F2FS_I(mapping->host);
	if (get_dirty_pages(mapping->host) <= (F2FS_M_SB(mapping))->SM_I()->min_hot_blocks)
		fi->set_inode_flag(FI_HOT_DATA);
		...
		retry_write:
			{
				...
				auto_lock<page_auto_lock> page_locker(*page);
				//	lock_page(page);
				...
				if (!PageDirty(page))	continue; /* someone wrote it for us */
				//{	
				//	goto continue_unlock;
				//}
				if (PageWriteback(page))
				{
					if (wbc->sync_mode != WB_SYNC_NONE) f2fs_wait_on_page_writeback(page, DATA, true, true);
					else	/*goto continue_unlock;*/ continue;
				}
				if (!clear_page_dirty_for_io(page)) continue;
					//goto continue_unlock;
...
#endif
			LOG_DEBUG(L"bio=%llX, page=%llX, flag=%X, writeback=%d", bio, page, 
				page?page->flags:-1, page?PageWriteback(page):-1);
				ret = f2fs_write_single_data_page(page, &submitted, &bio, &last_block, wbc, io_type, 0, true);
...
static inline bool __should_serialize_io(inode *iinode, writeback_control *wbc)
{
	/* to avoid deadlock in path of data flush */
//	if (F2FS_I(iinode)->cp_task) return false;
	f2fs_sb_info* sbi = F2FS_I(iinode)->m_sbi;
	if (sbi->cprc_info.IsRunning())
	{
		LOG_DEBUG(L"return due to CP thread is running");
		return false;
	}
...
{
//	inode *iinode = host;
//	return __f2fs_write_data_pages(this, wbc, F2FS_I(inode)->cp_task == current ? FS_CP_DATA_IO : FS_DATA_IO);
	...
	return __f2fs_write_data_pages(this, wbc, type);
}
...
//	trace_f2fs_write_begin(inode, pos, len, flags);
	...
	/* We should check this at this moment to avoid deadlock on inode page and #0 page. The locking rule for 
	inline_data conversion should be: lock_page(page #0) -> lock_page(inode_page) */
	if (index != 0) 
	{
		err = inode->f2fs_convert_inline_inode();
		if (err)	goto fail;
...
repeat:
	/* Do not use grab_cache_page_write_begin() to avoid deadlock due to wait_for_stable_page. Will wait that below with 
	our IO control.	 */
	...
	*pagep = ppage;
	...
	else
	{
		...
		err = f2fs_submit_page_read(inode, ppage, blkaddr, 0, true);
		if (err)	goto fail;
...
int CF2fsFileSystem::submit_bio_wait(bio* bbio)
{
//	DECLARE_COMPLETION_ONSTACK_MAP(done, bbio->bi_bdev->bd_disk->lockdep_map);
//	unsigned long hang_check;
	HANDLE wait_event = CreateEvent(NULL, FALSE, FALSE, NULL);
	bbio->bi_private = (void*)(wait_event);
	bbio->bi_end_io = submit_bio_wait_endio;
	...
	WaitForSingleObject(wait_event, -1);

	return blk_status_to_errno(bbio->bi_status);
}

void CF2fsFileSystem::submit_bio(bio* bb)
{
//	JCASSERT(bb && bb->bi_io_vec && bb->bi_bdev);	// bb->bi_io_vec可以为空
	...
	jcvos::auto_array<BYTE> buf(data_size);
	int op = bb->bi_opf & 0xFF;
	LOG_DEBUG(L"submit io: op=%s, sec=0x%llX, len=0x%llX", DebugOutReq(op), bb->bi_iter.bi_sector, data_secs);
	if (op == REQ_OP_READ)
	{
		m_sb_info->s_active;
