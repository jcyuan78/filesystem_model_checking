...
static int read_xattr_block(struct inode *inode, void *txattr_addr)
{
	...
	xpage = sbi->f2fs_get_node_page( xnid);
	if (IS_ERR(xpage))
				...
				void *txattr_addr, struct page *ipage)
{
	...
	if (inline_size) {
		if (ipage) {
			...
			in_page = sbi->f2fs_get_node_page( inode->i_ino);
			if (IS_ERR(in_page)) {
	...
	if (F2FS_I(inode)->i_xattr_nid) {
		xpage = sbi->f2fs_get_node_page( F2FS_I(inode)->i_xattr_nid);
		if (IS_ERR(xpage)) {
			err = PTR_ERR(xpage);
			f2fs_alloc_nid_failed(sbi, new_nid);
			goto in_page_out;
		...
		dn.set_new_dnode(inode, NULL, NULL, new_nid);
