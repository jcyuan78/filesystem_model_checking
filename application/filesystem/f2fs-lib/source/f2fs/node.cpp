...
{
	nat_entry *new_entry;
	//f2fs_kmem_cache_alloc(nat_entry_slab, GFP_F2FS_ZERO);
	if (no_fail)	new_entry = f2fs_kmem_cache_alloc<nat_entry>(nat_entry_slab,  GFP_F2FS_ZERO);
	// kmem_cache_alloc(nat_entry_slab, GFP_F2FS_ZERO);
	else			new_entry = kmem_cache_alloc<nat_entry>(nat_entry_slab, GFP_F2FS_ZERO);
...
nat_entry *f2fs_nm_info::__init_nat_entry(nat_entry *ne, f2fs_nat_entry *raw_ne, bool no_fail)
{
	...
	LOG_TRACK(L"nat", L"add nat to cache, ino=%d, nid=%d, block=0x%X", ne->ni.ino, ne->ni.nid, ne->ni.blk_addr);
...
nat_entry *f2fs_nm_info::__lookup_nat_cache(nid_t n)
{
	...
	if (ne && !get_nat_flag(ne, IS_DIRTY)) 
	{
		...
		spin_unlock(&nat_list_lock);
		LOG_TRACK(L"nat", L"found nat in cache, ino=%d, nid=%d, block=%X", ne->ni.ino, n, ne->ni.blk_addr);
	}
	return ne;
}
...
{
	LOG_TRACK(L"nat", L" delete nat from cache, ino=%d, nid=%d, block=0x%X", e->ni.ino, e->ni.nid, e->ni.blk_addr);
...
static void __set_nat_cache_dirty(f2fs_nm_info *nm_i, nat_entry *ne)
{
	...
	if (get_nat_flag(ne, IS_DIRTY))		goto refresh_list;
...
refresh_list:
	...
	if (new_ne)		list_del_init(&ne->list);
	else			list_move_tail(&ne->list, &head->entry_list);
	spin_unlock(&nm_i->nat_list_lock);
	LOG_TRACK(L"nat", L"set nat dirty, nid=%d, dirty_nr=%d", ne->ni.nid, nm_i->nat_cnt[DIRTY_NAT]);
}

static void __clear_nat_cache_dirty(struct f2fs_nm_info *nm_i, struct nat_entry_set *set, struct nat_entry *ne)
{
	spin_lock(&nm_i->nat_list_lock);
	...
	nm_i->nat_cnt[DIRTY_NAT]--;
	nm_i->nat_cnt[RECLAIMABLE_NAT]++;
	LOG_TRACK(L"nat", L"clear nat dirty, nid=%d, dirty_nr=%d", ne->ni.nid, nm_i->nat_cnt[DIRTY_NAT]);
}
...
static unsigned int f2fs_add_fsync_node_entry(f2fs_sb_info *sbi, page *ppage)
{
	fsync_node_entry *fn;
//	unsigned long flags;
	unsigned int seq_id;
...
void f2fs_del_fsync_node_entry(f2fs_sb_info *sbi, struct page *ppage)
{
	fsync_node_entry *fn;
//	unsigned long flags;
...
//static void set_node_addr(struct f2fs_sb_info *sbi, struct node_info *ni, block_t new_blkaddr, bool fsync_done)
void f2fs_sb_info::set_node_addr(node_info *ni, block_t new_blkaddr, bool fsync_done)
{
	...
	LOG_TRACK(L"nat", L"set node addr, ino=%d, nid=%d, block=%X", ni->ino, ni->nid, new_blkaddr);
	e = nm_i->__lookup_nat_cache(ni->nid);
	if (!e) 
	{
		e = nm_i->__init_nat_entry( new_entry_buf, NULL, true);
		copy_node_info(&e->ni, ni);
		f2fs_bug_on(this, ni->blk_addr == NEW_ADDR);
	} 
	else if (new_blkaddr == NEW_ADDR) 
	{
		/* when nid is reallocated, previous nat entry can be remained in nat cache. So, reinitialize it with new_entry_buf information. */
		copy_node_info(&e->ni, ni);
		f2fs_bug_on(this, ni->blk_addr != NULL_ADDR);
	}
	/* let's free early to reduce memory consumption */
	if (e != new_entry_buf) 	__free_nat_entry(new_entry_buf);

	/* sanity check */
	f2fs_bug_on(this, nat_get_blkaddr(e) != ni->blk_addr);
	f2fs_bug_on(this, nat_get_blkaddr(e) == NULL_ADDR && new_blkaddr == NULL_ADDR);
	f2fs_bug_on(this, nat_get_blkaddr(e) == NEW_ADDR &&	new_blkaddr == NEW_ADDR);
	f2fs_bug_on(this, __is_valid_data_blkaddr(nat_get_blkaddr(e)) && new_blkaddr == NEW_ADDR);
	...
	if (!__is_valid_data_blkaddr(new_blkaddr))		set_nat_flag(e, IS_CHECKPOINTED, false);
	__set_nat_cache_dirty(nm_i, e);

	/* update fsync_mark if its inode nat entry is still alive */
	if (ni->nid != ni->ino)							e = nm_i->__lookup_nat_cache(ni->ino);
	if (e) {
		if (fsync_done && ni->nid == ni->ino)		set_nat_flag(e, HAS_FSYNCED_INODE, true);
...
//	LOG_TRACK(L"nat", L"find node info: nid=%d", ni->nid);
	e = __lookup_nat_cache(nid);
	if (e) 
	{
...
//		LOG_TRACK(L"nat", L"found in cache: ino=%d, nid=%d, block=0x%X", ni->ino, ni->nid, ni->blk_addr);
		return 0;
	}
	...
	LOG_TRACK(L"nat", L" got node info from nat, ino=%d, nid=%d, block=0x%X", ni->ino, ni->nid, ni->blk_addr);
...
cache:
	...
	if (__is_valid_data_blkaddr(blkaddr) &&	!m_sbi->f2fs_is_valid_blkaddr(blkaddr, DATA_GENERIC_ENHANCE))
		return -EFAULT;
	/* cache nat entry */
	LOG_TRACK(L"nat", L"cache node, ino=%d, nid=%d, block=%d", ne.ino, nid, blkaddr);
...
static int get_node_path(f2fs_inode_info *inode, long block, int offset[4], unsigned int noffset[4])
{
...
static int truncate_node(struct dnode_of_data *dn)
{
	...
	sbi->set_node_addr(&ni, NULL_ADDR, false);
...
int f2fs_truncate_inode_blocks(f2fs_inode_info*iinode, pgoff_t from)
{
...
//	struct f2fs_sb_info *sbi = F2FS_I_SB(dn->inode);
	...
	new_ni.flag = 0;
	new_ni.version = 0;
	m_sbi->set_node_addr(&new_ni, NEW_ADDR, false);
...
int f2fs_sb_info::read_node_page(page *page, int op_flags)
{
	...
	LOG_TRACK(L"nat", L"got node info, ino=%d, nid=%d, block=%X", ni.ino, ni.nid, ni.blk_addr);
	...
	if (!err)	f2fs_update_iostat(FS_NODE_READ_IO, F2FS_BLKSIZE);
	return err;
}
...
#ifdef DEBUG_PAGE
	ptr_page->m_type = page::INODE_PAGE;
#endif
	LOG_TRACK(L"page", L"grab page for node, page=%p, nid=%d", ptr_page, ptr_page->index, nid);
...
{
	LOG_TRACK(L"node", L"write node page : nid=%d", ppage->index);
...
//	trace_f2fs_writepage(ppage, NODE);
	...
	if (__is_valid_data_blkaddr(ni.blk_addr) &&
		!sbi->f2fs_is_valid_blkaddr(ni.blk_addr, DATA_GENERIC_ENHANCE)) {
		up_read(&sbi->node_write);
		goto redirty_out;
	}
	...
	sbi->set_node_addr(&ni, fio.new_blkaddr, is_fsync_dnode(ppage));
	sbi->dec_page_count(F2FS_DIRTY_NODES);
	up_read(&sbi->node_write);
...
int f2fs_fsync_node_pages(f2fs_sb_info *sbi, f2fs_inode_info *inode, writeback_control *wbc, bool atomic, unsigned int *seq_id)
{
	pgoff_t index;
	struct pagevec pvec;
	int ret = 0;
...
retry:
	...
	while ((nr_pages = pagevec_lookup_tag(&pvec, NODE_MAPPING(sbi), &index,	PAGECACHE_TAG_DIRTY))) 
	{
		...
		for (i = 0; i < nr_pages; i++)
		{
			...
			if (!IS_DNODE(page) || !is_cold_node(page))		continue;
			if (ino_of_node(page) != ino)					continue;
...
continue_unlock:
			...
			if (!atomic || page == last_page) 
			{
				...
				if (IS_INODE(page)) 
				{
					if (is_inode_flag_set(inode, FI_DIRTY_INODE))	inode->f2fs_update_inode(page);
			...
			ret = __write_node_page(page, atomic && page == last_page, &submitted, wbc, true, FS_NODE_IO, seq_id);
			if (ret) 
			{
				unlock_page(page);
				f2fs_put_page(last_page, 0);
...
static int f2fs_match_ino(inode *iinode, unsigned long ino, void *data)
{
	...
	if (!is_inode_flag_set(F2FS_I(iinode), FI_DIRTY_INODE))		return 0;
...
next_step:
	index = 0;
	static const wchar_t* str_step[] = { L"indirect", L"dentry", L"file" };
	LOG_DEBUG(L"sync nodes: step=%d, sync %s nodes", step, str_step[step]);
	...
	while (!done && (nr_pages = pagevec_lookup_tag(&pvec, NODE_MAPPING(sbi), &index, PAGECACHE_TAG_DIRTY))) 
	{
		...
		for (i = 0; i < nr_pages; i++) {
			...
			if (atomic_read(&sbi->wb_sync_req[NODE]) &&	wbc->sync_mode == WB_SYNC_NONE) {
				done = 1;
				break;
			}
...
lock_node:
			if (wbc->sync_mode == WB_SYNC_ALL)		lock_page(page);
			else if (!trylock_page(page))			continue;
			if (unlikely(page->mapping != NODE_MAPPING(sbi))) {
//continue_unlock:
				unlock_page(page);
				continue;
...
//				goto continue_unlock;
			...
			set_fsync_mark(page, 0);
			set_dentry_mark(page, 0);
			LOG_DEBUG(L"write for node page, nid=%d", page->index);
			ret = __write_node_page(page, false, &submitted, wbc, do_balance, io_type, NULL);
...
//		cond_resched();
	...
	if (step < 2) {
		if (!sbi->is_sbi_flag_set( SBI_CP_DISABLED) &&	wbc->sync_mode == WB_SYNC_NONE && step == 1) goto out;
		step++;
		goto next_step;
	}
out:
	if (nwritten)	f2fs_submit_merged_write(sbi, NODE);
...
//	unsigned long flags;
	...
	while (seq_id && cur_seq_id < seq_id) {
		...
		f2fs_wait_on_page_writeback(ppage, NODE, true, false);
		if (TestClearPageError(ppage))
			ret = -EIO;
		TRACK_PAGE(ppage, L"put page");
...
#endif
int f2fs_recover_inline_xattr(f2fs_inode_info *inode, struct page *page)
{
...
//		congestion_wait(BLK_RW_ASYNC, DEFAULT_IO_TIMEOUT);
	...
	sbi->set_node_addr(&new_ni, NEW_ADDR, false);
...
int f2fs_flush_nat_entries(f2fs_sb_info *sbi, cp_control *cpc)
{
	...
	LOG_TRACK(L"nat", L"flush nat entries, dirty=%d", nm_i->nat_cnt[DIRTY_NAT]);
	...
	if (!nm_i->nat_cnt[DIRTY_NAT])		return 0;
...
//	LOG_DEBUG(L"nat journal: %d entry was found", nat_entry_nr);
	...
	LOG_TRACK(L"nat", L"flush nat entries completed, dirty=%d", nm_i->nat_cnt[DIRTY_NAT]);
	return err;
}
...
{
	f2fs_checkpoint *ckpt = m_sbi->F2FS_CKPT();
//	struct f2fs_nm_info *nm_i = NM_I(sbi);
	...
	{
		page *ppage;
		ppage = m_sbi->f2fs_get_meta_page(nat_bits_addr++);
		if (IS_ERR(ppage))		return PTR_ERR(ppage);
		memcpy(nat_bits + (i << F2FS_BLKSIZE_BITS), page_address<BYTE>(ppage), F2FS_BLKSIZE);
		f2fs_put_page(ppage, 1);
	}
...
//	sbi->nm_info = this;
	...
	// 源代码中没有初始化nat_cnt；
	memset(nat_cnt, 0, sizeof(nat_cnt));
		//return err;
...
//	if (!this)	return;
	...
	spin_lock(&nid_list_lock);
	list_for_each_entry_safe(free_nid, i, next_i, &free_nid_list, list)
	{
		__remove_free_nid(i, FREE_NID);
		spin_unlock(&nid_list_lock);
		kmem_cache_free(free_nid_slab, i);
	...
	f2fs_bug_on(m_sbi, nat_cnt[TOTAL_NAT]);
	...
	while ((found = __gang_lookup_nat_set(this, nid, SETVEC_SIZE, setvec)))
	{
		...
		for (idx = 0; idx < found; idx++) {
			...
			radix_tree_delete<nat_entry_set>(&nat_set_root, setvec[idx]->set);
			kmem_cache_free(nat_entry_set_slab, setvec[idx]);
		}
	}
	up_write(&nat_tree_lock);

	f2fs_kvfree(nat_block_bitmap);	//在init_free_nid()中alloc

	if (free_nid_bitmap)
	{
		int i;
		for (i = 0; i < nat_blocks; i++)		f2fs_kvfree(free_nid_bitmap[i]);
