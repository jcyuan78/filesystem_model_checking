...
bool CControlThread::Start(int priority)
{
	...
	LOG_DEBUG_(1,L"control thread stared, this=%p, data=%llX, thread id = %d", this, (UINT64)(data), m_thread_id);
	while (1)
	{	// 确保control线程已经开始运行
		Sleep(1);
//		if (InterlockedAdd(&m_running,0)) break;
		if (InterlockedAdd(&m_started, 0)) break;
	}
...
void CControlThread::Stop(void)
{
	...
	LOG_DEBUG_(1,L"stop thread, m_running=%d", m_running);
...
#ifdef _DEBUG
	LOG_DEBUG_(1,L"start control thread: %s, control=%p, data=%llX", control->m_thread_name.c_str(), control, (UINT64)data);
#endif
	JCASSERT(control);
