...
LOCAL_LOGGER_ENABLE(L"test_trace", LOGGER_LEVEL_DEBUGINFO);
...
void CTraceTester::Config(const boost::property_tree::wptree& pt, const std::wstring& root)
{
	...
	//m_trace_fn = pt.get<std::wstring>(L"trace_file");
...
int CTraceTester::PrepareTest(void)
{
	...
	while (1)
	{
		...
		TRACE_ENTRY& op = m_trace.back();

		UINT field_sn = 0;
		wchar_t* field_val = wcstok_s(line, L",\n", &context);
		TRACE_ENTRY::OP_CODE mode = TRACE_ENTRY::OP_NOP;
		for(; field_val; field_val = wcstok_s(nullptr, L",\n", &context), field_sn++)
		{
			...
			case FIELD_OPERATION:	// operation
				op.code = StringToOpCode(field_val); break;
			case FIELD_PATH: {	// path: 出去路径前缀，加入文件map
				std::wstring path = field_val + PATH_PREFIX; // , wcslen(field_val) - (2 + PATH_PREFIX));
				op.file_path = path;
			...
			//case 4: // Detail
			case FIELD_TID:	// TID
				swscanf_s(field_val/* + 1*/, L"%d", &(op.thread_id));
				break;
			case FIELD_PARAM1:	// Param1
				if (op.code == TRACE_ENTRY::OP_FILE_READ || op.code == TRACE_ENTRY::OP_FILE_WRITE)
				{	// offset by byte
					swscanf_s(field_val/* + 1*/, L"%zd", &(op.offset));
				}
				else if (op.code == TRACE_ENTRY::OP_FILE_CREATE)
				{	// mode: create, open, delete
					if (wcscmp(field_val, L"Open") == 0) mode = TRACE_ENTRY::OP_FILE_OPEN;
					else if (wcscmp(field_val, L"Create") == 0) mode = TRACE_ENTRY::OP_FILE_CREATE;
					else if (wcscmp(field_val, L"Overwrite") == 0) mode = TRACE_ENTRY::OP_FILE_OVERWRITE;
					else if (wcscmp(field_val, L"Delete") == 0) mode = TRACE_ENTRY::OP_FILE_DELETE;
//					else if (wcscmp(field_val, L"") == 0) op.mode = TRACE_ENTRY::OP_;
					//else op.mode = TRACE_ENTRY::OP_NOP;
				}	
				else if (op.code == TRACE_ENTRY::OP_THREAD_CREATE || op.code == TRACE_ENTRY::OP_THREAD_EXIT)
				{	// thread id of new
					swscanf_s(field_val /*+ 1*/, L"%d", &(op.new_thread_id));
				}
				break;
			...
			case FIELD_PARAM2: // Param2
				if (op.code == TRACE_ENTRY::OP_FILE_READ || op.code == TRACE_ENTRY::OP_FILE_WRITE)
				{	// length in byte
					swscanf_s(field_val/* + 1*/, L"%zd", &(op.length));
				}
				else if (op.code == TRACE_ENTRY::OP_FILE_CREATE)
				{	// dir or file
		...
		if (op.code == TRACE_ENTRY::OP_FILE_CREATE)
		{
			//DWORD fid = NewFileInfo(op);
			//op.fid = fid;

			if (mode == TRACE_ENTRY::OP_FILE_OPEN) op.code = TRACE_ENTRY::OP_FILE_OPEN;
			else if (mode == TRACE_ENTRY::OP_FILE_DELETE) op.code = TRACE_ENTRY::OP_FILE_DELETE;
			else if (mode == TRACE_ENTRY::OP_FILE_OVERWRITE) op.code = TRACE_ENTRY::OP_FILE_OVERWRITE;
			//else if (mode = TRACE_ENTRY::OP_FILE_) op.code = TRACE_ENTRY::OP_FILE;
			//else if (mode = TRACE_ENTRY::OP_FILE_) op.code = TRACE_ENTRY::OP_FILE;
			//else if (mode = TRACE_ENTRY::OP_FILE_) op.code = TRACE_ENTRY::OP_FILE;
		}
		if (op.code == TRACE_ENTRY::OP_FILE_CREATE)
		...
		else if (op.code == TRACE_ENTRY::OP_FILE_READ || op.code == TRACE_ENTRY::OP_FILE_WRITE)
		{
	...
	for (auto it = m_file_access.begin(); it != m_file_access.end(); ++it)
	{
		...
		else
		{
			...
			FillFile(file, info, 0, info.max_length);
			//			ReserveFile(file, info);
			CloseHandle(file);
		}
	}
	DWORD ii = 0;
...
int CTraceTester::FinishTest(void)
{
	...
	for (auto trace = m_traces.begin(); trace != m_traces.end(); ++trace)
	{
		...
		{
			if (op->code == TRACE_ENTRY::OP_FILE_READ)
			{
			...
			else if (op->code == TRACE_ENTRY::OP_FILE_WRITE)
			{
...
void CTraceTester::CalculateFileAccess(TRACE_ENTRY& op)
{
	...
	if (op.code == TRACE_ENTRY::OP_FILE_READ) finfo.total_read += op.length;
	else if (op.code == TRACE_ENTRY::OP_FILE_WRITE) finfo.total_write += op.length;
	else THROW_ERROR(ERR_APP, L"not read/write op, op=%d", op.code);
}
...
DWORD CTraceTester::FindOrNewFile(const std::wstring& path, bool is_dir)
{
	...
	if (found != m_path_map.end())
	{
		...
		LOG_DEBUG(L"found item=%s, id=%d", path.c_str(), fid);
		return fid;
	}
	// 没有找到 => 新建file info
	...
	DWORD fid = m_max_fid++;
	LOG_DEBUG(L"create item=%s, id=%d, parent_id=%d", path.c_str(), fid, parent_id);
	...
	while (1)
	{
		...
		for (; field_val; field_val = wcstok_s(nullptr, L",\n", &context), field_sn++)
		{
			...
			case FIELD_OPERATION:	// operation
				op.code = StringToOpCode(field_val); break;
			case FIELD_PATH: {	// path: 出去路径前缀，加入文件map
				if (field_val[0] == 0 /* || (op.code != TRACE_ENTRY::OP_FILE_CREATE && op.code != TRACE_ENTRY::OP_FILE_CLOSE && op.code != )*/
					) continue;
...
//	HANDLE* files = trace_info.m_files;
	for (auto it = trace.begin(); it != trace.end(); ++it)
	{
		...
		switch (op.code)
		{
		case TRACE_ENTRY::OP_FILE_CREATE: // continue
		case TRACE_ENTRY::OP_FILE_OPEN:
		case TRACE_ENTRY::OP_FILE_OVERWRITE: {
			DWORD flag = 0;
			...
			DWORD dipos;
			if (op.code == TRACE_ENTRY::OP_FILE_OVERWRITE)	
			{
				dipos = TRUNCATE_EXISTING; 
		...
		case TRACE_ENTRY::OP_FILE_CLOSE: {
			if (op.file_path == L"") continue;
			if (info.is_dir) continue;
		...
		case TRACE_ENTRY::OP_FILE_DELETE: {
			std::wstring path = m_root + op.file_path;
		...
		case TRACE_ENTRY::OP_FILE_WRITE: {
		...
		case TRACE_ENTRY::OP_FILE_FLUSH: {
			break;
		}
		case TRACE_ENTRY::OP_THREAD_CREATE: {break; }
		case TRACE_ENTRY::OP_THREAD_EXIT: {break; }
										//		case TRACE_ENTRY::OP_FILE: {break; }
		default: LOG_WARNING(L"Unknown operation code=%d", op.code);
		}
...
{
	LOG_DEBUG(L"add file=%s, dir=%d, ts=%lld", op.file_path.c_str(), op.is_dir, op.ts);
...
void CTraceTester::FillFile(HANDLE file, const FILE_ACCESS_INFO & info, size_t start, size_t len)
{
	...
	while (remain > 0)	// for each buffer
	{
		...
		if (!br || written < to_write) THROW_WIN32_ERROR(L"failed on write to file: %s", info.file_name.c_str());
		offset = 0;
		remain -= written;
	}
}

UINT64 CTraceTester::WriteTest(const FILE_ACCESS_INFO& info, HANDLE file, BYTE * _buf, size_t start, size_t len)
{
	JCASSERT(file);
	...
	//if (aligned_len > m_buffer_size)
	//{
	//	delete[] m_rw_buffer;
	//	m_rw_buffer = new BYTE[aligned_len];
	//}

//	jcvos::auto_array<FILE_FILL_DATA> buf(buffer_nr);
...
UINT64 CTraceTester::ReadTest(const FILE_ACCESS_INFO& info, HANDLE file, BYTE * _buf, size_t start, size_t len)
{
	JCASSERT(file);
	size_t aligned_start = round_down(start, sizeof(FILE_FILL_DATA));
	size_t aligned_end = round_up(start + len, sizeof(FILE_FILL_DATA));
...
void CTraceTester::ReserveFile(HANDLE file, const FILE_ACCESS_INFO& info)
{
...
TRACE_ENTRY::OP_CODE CTraceTester::StringToOpCode(const std::wstring& str)
{
	if (0) {}
	else if (str == L"Thread Create")	return TRACE_ENTRY::OP_THREAD_CREATE;
	else if (str == L"Thread Exit")		return TRACE_ENTRY::OP_THREAD_EXIT;
	else if (str == L"CreateFile")		return TRACE_ENTRY::OP_FILE_CREATE;
	else if (str == L"CloseFile")		return TRACE_ENTRY::OP_FILE_CLOSE;
	else if (str == L"ReadFile")		return TRACE_ENTRY::OP_FILE_READ;
	else if (str == L"WriteFile")		return TRACE_ENTRY::OP_FILE_WRITE;
	else if (str == L"FlushBuffersFile") return TRACE_ENTRY::OP_FILE_FLUSH;
	//else if (str == L"") return TRACE_ENTRY::OP_;
	//else if (str == L"") return TRACE_ENTRY::OP_;
	//else if (str == L"") return TRACE_ENTRY::OP_;
	//else if (str == L"") return TRACE_ENTRY::OP_;
	//else if (str == L"") return TRACE_ENTRY::OP_;
