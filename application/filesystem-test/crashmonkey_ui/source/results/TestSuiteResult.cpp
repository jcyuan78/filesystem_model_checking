...
using fs_testing::TestSuiteResult;
using std::endl;
//using std::ostream;
using std::string;
using std::vector;
...
void TestSuiteResult::TallyResult(SingleTestInfo& done, ResultSet& set)
{
	...
	{
	case SingleTestInfo::kPassed: 		++set.num_passed;			break;
	case SingleTestInfo::kFsckFixed:	++set.num_passed_fixed;		break;
	case SingleTestInfo::kFsckRequired:	++set.fsck_required;		break;
	case SingleTestInfo::kFailed:		++set.num_failed;

		switch (done.data_test.GetError())
		{
		case DataTestResult::kOldFilePersisted:			++set.old_file_persisted;			break;
		case DataTestResult::kFileMissing:				++set.file_missing;					break;
		case DataTestResult::kFileDataCorrupted:		++set.file_data_corrupted;			break;
		case DataTestResult::kFileMetadataCorrupted:	++set.file_metadata_corrupted;		break;
		case DataTestResult::kIncorrectBlockCount:		++set.incorrect_block_count;		break;
		case DataTestResult::kOther:					++set.other;						break;
		case DataTestResult::kAutoCheckFailed:			++set.auto_check_failed;			break;
		}
		break;
	}
}
...
void TestSuiteResult::PrintResults(std::wostream& os) const
{
