...
namespace fs_testing 
{
    namespace permuter 
    {
        ...
        protected:
           ...
           */
            std::vector<EpochOpSector> CoalesceSectors(std::vector<EpochOpSector> &sector_list);

            unsigned int sector_size_;

        private:
            virtual void init_data(std::vector<epoch> *data) = 0;
            virtual bool gen_one_state(std::vector<epoch_op>& res, fs_testing::PermuteTestResult &log_data) = 0;
