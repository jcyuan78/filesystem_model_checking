	...
	namespace user_tools {
		namespace api {
			...
			public:
				...
				virtual int FnRename(const std::string& old_path, const std::string& new_path) = 0;
			...
			{
			public:
				virtual int FnMknod(const std::string& pathname, mode_t mode, dev_t dev) override;
				...
				virtual int FnOpen2(const std::string& pathname, int flags, mode_t mode) override;
				...
				virtual int FnRename(const std::string& old_path, const std::string& new_path);
			...
			public:
				...
				virtual int CmMknod(const std::string& pathname, const mode_t mode, const dev_t dev) = 0;
				...
				virtual int CmOpen(const std::string& pathname, const int flags, const mode_t mode) = 0;
				...
				virtual ssize_t CmPwrite(const int fd, const void* buf, const size_t count, const off_t offset) = 0;
				...
				virtual int CmRename(const std::string& old_path, const std::string& new_path) = 0;
				...
				virtual int CmSyncFileRange(const int fd, size_t offset, size_t nbytes, unsigned int flags) = 0;
				virtual int CmCheckpoint() = 0;
			};
			...
			public:
				...
				ssize_t CmPwrite(const int fd, const void* buf, const size_t count, const off_t offset);
				void* CmMmap(void* addr, const size_t length, const int prot, const int flags, const int fd, const off_t offset);
				...
				virtual int CmSyncFileRange(const int fd, size_t offset, size_t nbytes, unsigned int flags);
			...
			private:
				...
				void CmOpenCommon(const int fd, const std::string& pathname, const bool exists, const int flags);

				/*
				 * Write data out to the given file descriptor. Automatically retires until
				 * all the requested data is written.
				 */
				int WriteWhole(const int fd, const unsigned long long size, std::shared_ptr<char> data);
			};
