 ...
 */
  ...
  */
shared_ptr<wchar_t> DiskMod::Serialize(DiskMod& dm, unsigned long long* size)
{
	...
	shared_ptr<wchar_t> res_ptr(new (std::nothrow) wchar_t[mod_size], [](wchar_t* c) {delete[] c; });
	wchar_t* buf = res_ptr.get();
	...
	if (res < 0)	{		return shared_ptr<wchar_t>(nullptr);	}
	buf_offset += res;

	// kCheckpointMod and kSyncMod don't need anything done after the type.
	if (!(dm.mod_type == DiskMod::kCheckpointMod ||	dm.mod_type == DiskMod::kSyncMod))
	{
		res = SerializeChangeHeader(buf, buf_offset, dm);
		if (res < 0)		{			return shared_ptr<wchar_t>(nullptr);		}

		if (dm.mod_type == DiskMod::kFsyncMod || dm.mod_type == DiskMod::kCreateMod)	{	return res_ptr;	}
...
int DiskMod::SerializeHeader(wchar_t* buf, const unsigned int buf_offset,
	DiskMod& dm)
{
	buf = buf + buf_offset;
...
size_t DiskMod::SerializeChangeHeader(wchar_t* buf, const unsigned int buf_offset, DiskMod& dm)
{
	buf += buf_offset;
	size_t size = dm.path.size() + 1;
	...
	memcpy(buf, dm.path.c_str(), size*sizeof(wchar_t));
...
int DiskMod::SerializeDataRange(wchar_t* buf, const unsigned int buf_offset,
	DiskMod& dm)
{
	buf += buf_offset;
...
int DiskMod::SerializeDirectoryMod(wchar_t* buf, const unsigned int buf_offset, DiskMod& dm)
{
	assert(0 && "Not implemented");
	return 0;
}

int DiskMod::Deserialize(shared_ptr<wchar_t> data, DiskMod& res)
{
	res.Reset();

	// Skip the first uint64 which is the size of this region. This is a blind
	// deserialization of the object!
	wchar_t* data_ptr = data.get();
	...
	while (data_ptr[0] != '\0')
	{
		...
		if (chars_read == tmp_size - 1)
		{
			...
			res.path += tmp;
			chars_read = 0;
			// Required because we just add the wchar_t[] to the string and we don't want
	...
	if (res.file_mod_len > 0)
	{
		...
		res.file_mod_data.reset(new (std::nothrow) wchar_t[res.file_mod_len],
			[](wchar_t* c) {delete[] c; });
