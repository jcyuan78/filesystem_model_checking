...
namespace fs_testing {
	namespace utils {
		...
		public:
			...
			static std::shared_ptr<wchar_t> Serialize(DiskMod& dm, unsigned long long* size);
			 ...
			 */
			static int Deserialize(std::shared_ptr<wchar_t> data, DiskMod& res);
			...
			std::shared_ptr<wchar_t> file_mod_data;
		...
		private:
			 ...
			 * that).
			 */
			static int SerializeHeader(wchar_t* buf, const unsigned int buf_offset, DiskMod& dm);
			/*
			 * Serializes stuff that is presesnt in file and directory operations.
			 */
			static size_t SerializeChangeHeader(wchar_t* buf, const unsigned int buf_offset, DiskMod& dm);
			static int SerializeDataRange(wchar_t* buf, const unsigned int buf_offset, DiskMod& dm);
			static int SerializeDirectoryMod(wchar_t* buf, const unsigned int len, DiskMod& dm);
		};
